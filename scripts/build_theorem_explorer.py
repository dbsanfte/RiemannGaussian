...
from numerical_certificate import partition as certificate_partition
...
def module_family(meta, module):
    """Use the same explicit override / longest-prefix rule in both views."""
    if module in meta["moduleFamilies"]:
        return meta["moduleFamilies"][module]
    matches = [(len(prefix), family["id"]) for family in meta["families"]
               for prefix in family["modulePrefixes"] if module.startswith(prefix)]
    assert matches, f"Assign a logical family to {module} in metadata.json"
    longest = max(length for length, _ in matches)
    winners = {family for length, family in matches if length == longest}
    assert len(winners) == 1, f"Ambiguous family for {module}: {sorted(winners)}"
    return winners.pop()


def build_family_index():
    """Index every source module, including families outside the explorer endpoints."""
    meta = json.loads((SITE / "metadata.json").read_bytes())
    source_root = ROOT / "RiemannGaussian"
    modules = {"RiemannGaussian." + ".".join(p.relative_to(source_root).with_suffix("").parts): p
               for p in sorted(source_root.rglob("*.lean"))}
    normal, optional = certificate_partition()
    groups = {f["id"]: [] for f in meta["families"] if f["id"] != "library"}
    for name, path in modules.items():
        short = name.removeprefix("RiemannGaussian.")
        family = module_family(meta, short)
        assert family in groups, f"Project module assigned to external library: {name}"
        groups[family].append({"module": name, "path": path.relative_to(ROOT).as_posix(),
                               "buildTarget": "RiemannGaussian" if name in normal
                               else "NumericalCertificate"})
    unknown = set(meta["moduleFamilies"]) - {n.removeprefix("RiemannGaussian.") for n in modules}
    assert not unknown, f"Stale module family overrides: {sorted(unknown)}"
    target = ROOT / "docs/theorem-families"
    header = "<!-- Generated by scripts/build_theorem_explorer.py; edit docs/theorem-explorer/metadata.json. -->\n\n"
    index = [header + "# Theorem-family index\n",
             f"Browse all **{len(modules):,} Lean source modules** by mathematical family. "
             "Every listed module is reachable from a declared build target. "
             "Modules marked **optional certificate build** require "
             "`lake build NumericalCertificate --wfail`; ordinary builds exclude them.\n",
             "The family names, colours and assignments share the explorer's "
             "[evergreen metadata](../theorem-explorer/metadata.json). "
             "[Open the interactive proof chain](https://dbsanfte.github.io/RiemannGaussian/) "
             "for theorem statements, dependency paths and axiom audits.\n",
             "| Theorem family | Modules | Contents |\n| --- | ---: | --- |"]
    outputs, catalog = {}, []
    for family in meta["families"]:
        if family["id"] == "library":
            continue
        entries = groups[family["id"]]
        assert entries, f"Empty project family: {family['id']}"
        filename = family["id"] + ".md"
        index.append(f"| [{family['label']}]({filename}) | {len(entries)} | {family['description']} |")
        lines = [header + f"# {family['label']}\n",
                 "[All theorem families](README.md) · [Family metadata](../theorem-explorer/metadata.json)\n",
                 family["description"] + "\n",
                 f"**{len(entries)} modules.** Source links open the definitions, hypotheses and proofs.\n"]
        for entry in entries:
            label = entry["module"].removeprefix("RiemannGaussian.")
            suffix = " — optional certificate build" if entry["buildTarget"] == "NumericalCertificate" else ""
            lines.append(f"- [{label}](../../{entry['path']}){suffix}")
        outputs[target / filename] = ("\n".join(lines) + "\n").encode()
        catalog.append({"id": family["id"], "label": family["label"], "modules": entries})
    index += ["", "The ordinary root is [RiemannGaussian.lean](../../RiemannGaussian.lean); "
              "the opt-in root is [NumericalCertificate.lean](../../NumericalCertificate.lean). "
              "See [certificate status and verification](../numerical-certificate.md). "
              "External dependency sources, licenses and provenance are kept under "
              "[vendor/zeta23](../../vendor/zeta23/); Lean and mathlib are external "
              "boundary nodes in the explorer, rather than project source modules.", ""]
    outputs[target / "README.md"] = "\n".join(index).encode()
    outputs[target / "index.json"] = json_bytes({
        "schemaVersion": 2, "metadata": "docs/theorem-explorer/metadata.json",
        "rootModule": "RiemannGaussian", "moduleCount": len(modules),
        "optionalRootModule": "NumericalCertificate",
        "allModulesReachableFromDeclaredRoots": True,
        "optionalModuleCount": len(set(modules) - normal.keys()), "families": catalog,
    })
    landing = [header + "# Lean theorem families\n",
               "[Browse the complete theorem-family index](../docs/theorem-families/README.md). "
               "It covers every source module in this directory and its subdirectories.\n",
               "Families include Gaussian heat, Suzuki, eta and reflection, phase positivity, "
               "hybrid matrices, finite Hardy models, arithmetic cancellation and zero-free regions. "
               "The index and interactive explorer use the same metadata.\n",
               "[Root imports](../RiemannGaussian.lean) · "
               "[Proof status](../docs/proof-status.json) · "
               "[Interactive theorem explorer](https://dbsanfte.github.io/RiemannGaussian/)\n"]
    outputs[source_root / "README.md"] = "\n".join(landing).encode()
    existing = set(target.glob("*.md")) | set(target.glob("*.json"))
    assert existing <= set(outputs), f"Remove obsolete generated family pages: {sorted(existing - set(outputs))}"
    return outputs
...
def build():
    ...
    for n in raw["nodes"]:
        ...
        if not n["project"]:
            ...
            family = module_family(meta, short_module)
...
def main():
    ...
    outputs = {SITE / name: content for name, content in build().items()}
    outputs.update(build_family_index())
