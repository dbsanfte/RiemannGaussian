...
def build_family_index():
    ...
    import_pattern = re.compile(r"^(?:public )?import (RiemannGaussian(?:\.[\w]+)*)", re.M)
    imports = {name: import_pattern.findall(path.read_text()) for name, path in modules.items()}
    imports["RiemannGaussian"] = import_pattern.findall((ROOT / "RiemannGaussian.lean").read_text())
    reachable, pending = set(), ["RiemannGaussian"]
    while pending:
        name = pending.pop()
        if name in reachable:
            continue
        assert name in imports, f"Missing imported project source: {name}"
        reachable.add(name)
        pending.extend(imports[name])
    assert set(modules) <= reachable, f"Unimported source modules: {sorted(set(modules) - reachable)}"
    ...
    index = [header + "# Theorem-family index\n",
             ...
             "Every listed module is reachable from the root library imports.\n",
    ...
    index += ["", "The root module is [RiemannGaussian.lean](../../RiemannGaussian.lean). "
    ...
    outputs[target / "index.json"] = json_bytes({
        "schemaVersion": 1, "metadata": "docs/theorem-explorer/metadata.json",
        "rootModule": "RiemannGaussian", "moduleCount": len(modules),
        "allModulesReachableFromRoot": True, "families": catalog,
    })
