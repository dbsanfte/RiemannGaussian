...
private def milestones : Array Milestone := #[
  ...
  {
    label := "The live near-edge frontier is exactly two xi-logarithmic-derivative vertical integrals minus the xi source bulk and retains the detector limit"
    lineOne := "literal xi'/xi terms"
    lineTwo := "vertical-bulk limit"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_separatedSelectedLaplaceXiLogDerivativeVerticalSubBulkHeat_im
  }
]
