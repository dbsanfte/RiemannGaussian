...
private def milestones : Array Milestone := #[
  ...
  {
    label := "Actual nontrivial zeros satisfy 1/450000<Re(rho)<1-1/450000 whenever 1000000<=abs(Im(rho)) and log(abs(Im(rho))+2)<=320000. Literal zeta nonvanishing includes the closed right edge. The complete all-family Gaussian cost is at most 47500 against a selected source at least 48000; the existing exact family discharges all premises. Exact comparisons cover the unique Littlewood crossover L_* through log(exp(320000)-2), with equality at L_* and strict improvement above it. Classical, Littlewood, VK and intermediate literature expressions are compared with exact rational constants. The existing eventual component remains, and overlaps combine by maximum with all height conditions. Full literature and historical novelty review remains; RH is open."
...
private def gaussianPoleRemainderToolkit : Json :=
  Json.mkObj [
    ...
    ("nextProofTarget", .str "The gaussianPhaseBandToolkit now combines the full family and closes a rational cost surplus on an explicit height band. The exact comparison interval and uniform squarefree transport are now proved. Next resolve remaining literature-source coverage, widen the actual region and attack the independent ordinary-prime tail; no coefficient search."),
...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ...
    ("nextProofTarget", .str "Resolve remaining literature-source coverage, widen or sharpen the actual region using the full all-family budget, and attack the independent ordinary-prime tail using the proved uniform squarefree transport."),
