...
private def milestones : Array Milestone := #[
  ...
  {
    label := "A growing diagonal sequence of finite paired-eta Dirichlet-polynomial fluxes converges to the complete detector"
    lineOne := "finite eta flux"
    lineTwo := "limit = detector"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_pairedEtaBoundaryHeatFiniteArithmeticRadialFluxDiagonal
  }
]
...
private def renderSvg (moduleCount declarationCount theoremCount : Nat) : String :=
  ...
  "<svg xmlns=\"http://www.w3.org/2000/svg\" role=\"img\" " ++
    ...
    "  <text class=\"frontier\" x=\"20\" y=\"220\">Inventory, not proximity meter. Open: " ++
      "global arithmetic control of finite eta L1 flux; no current theorem implies RH.</text>\n" ++
    "</svg>\n"
  ...
  let statusJson := Json.mkObj [
    ...
    ("frontier", Json.mkObj [
      ...
      ("target", .str (
        ...
        "for the selected growing finite paired-eta Dirichlet-polynomial flux " ++
