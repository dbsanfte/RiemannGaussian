...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ...
    ("regionUnion", .str "The pointwise maximum retains both all-height Gaussian branches and the earlier signed-pole/reserve region, every admissible eventual Littlewood coefficient, the earlier fixed VK component and the stronger proved Vinogradov–Korobov family C/(L^(2/3)*(log L)^(1/3)), L=log(abs(t)+2), for every 0<C<3*pi/10640. The concrete coefficient 1/1150 is checked, and summing the cubic profile raises the coefficient ceiling by exactly 20/19 over the preceding cubic family. The explicit component is unconditional at every height. Eventual components keep finite unevaluated coefficient-dependent starting heights. The family eventually strictly improves every fixed earlier log-log coefficient; this does not establish published benchmark constants or a world record. RH remains open."),
    ...
    ("unionTheorem", .str "RiemannGaussian.ZetaVinogradovSummedZeroFree.exists_eventual_union"),
    ...
    ("nextProofTarget", .str "The complete original dyadic prefix now retains summable cubic decay and gives actual zeta growth 1048576*|t|^(40*Delta_n^(3/2))*log(|t|)^(2/3), throughout the closed strip through 3/2, above the same T_(8n). The resulting zero-free family has 0<C<3*pi/10640, including 1/1150. Next strengthen incomplete-system moment and asymmetric short-block estimates toward the published leading growth constants, sharpen the zero detector and evaluate finite starting heights. The independent fixed-height Riesz floor and harmonic Type-II estimate remain open."),
    ...
    ("vkScope", .str "Actual Dirichlet product-shift averaging, signed Gaussian resonance, prime-power conditioning and finite descent are proved. The unweighted critical moment exponent plus any fixed positive defect is unconditional; the 1/(128*k^2) Dirichlet saving remains available with existential degree costs. The quantitative relative-defect continuation uses 2*q profile steps and polynomial depth (k+2*k^3)^(2*q). It proves J_{k(k+1),k}(X) <= momentMultiplier(k,q)*X^(3*k*(k+1)/2+k^2/q) for k>=4, 1<=q<=k^2 and every X>=1, with every coefficient specified and no moment premise. Taking q=128 pays both moments and every Gaussian, smoothing, Taylor and averaging cost. The actual Dirichlet block and its literal damped zeta coefficients have saving 1/(256*k^2) for k>=12 and M>=k*(k+1), with defined costs. Decreasing weights pay their actual mass and the endpoint pays only its initial weight. The chosen moment coefficient is at least 2^(packetDepth^2); this is a cost audit of this construction, not a lower bound on the true moment. The polynomial-system continuation now proves exact type preservation under finite differences, weighted collision equivalence and the actual factorial-times-triangular-prime-power congruence count. The L-to-next-K integral step is now proved: literal residue energy, bounded boundary masks and interpolation give the diagonal alternative or 2*(2*floor(P/q))^m times a complete lower-order mixed-moment allowance at one fixed displacement. Its next polynomial type and T<=T'<=P*T are proved, and every original tail coordinate remains. The actual nonsingular prime-dilated count now satisfies S_ns<=m!*p^((r-d)*(r-d-1)/2)*L with unchanged endpoints and every tail divisibility proved. It reaches the attained next polynomial type by the checked differencing inequality; the full dilated tail moment equals ordinary J. Low-degree tail conditioning is also proved on the original nonsingular count: it is at most d!*p^(2s-d) times one fixed original tail residue-class count. Newton rigidity includes repeated entries; integer dilation is cancelled before prime reduction, and class selection follows integration. Endpoint deletion is now proved on actual mixed counts: K(N+1)<=2*K(N) for N>=16*s^2, using the diagonal tail reserve. The original residue tail reaches floor(Q/p), with its extra endpoint paid. Literal binomial rows use shift q*(p-xi), preserve the exact polynomial type and transport the full solution count to prime dilation. The original nonsingular count now reaches L at floor(Q/p) with explicit coefficient 2*d!*m!*p^(2s-d+(r-d)*(r-d-1)/2), and then an attained next polynomial type. The unrestricted entry point is now proved. An attained type-family maximum compares the doubled polynomial block with the original fixed tail. For m>=2 and P>=4*m^4, unordered repeated-pair counting and the actual diagonal reserve retain at least half the maximizing count with both blocks distinct. One uniform R-prime packet simultaneously avoids T and separates both blocks, with exact budget P^(d+2*choose(m,2))<M^R. Its proved Bertrand endpoint is 2^R*M; no dense (M,2*M] packet is assumed. The complete original K count reaches L at floor(Q/p) with coefficient 4*R*d!*m!*p^(2s-d+(r-d)*(r-d-1)/2), under the displayed packet, type and endpoint size conditions. The direct diagonal branch proves the actual s-to-s+k iteration at every positive integer endpoint, with defect ratio 1-1/k and an independent small-endpoint maximum. The fixed-width continuation now constructs exactly R primes in (M,8M] for M>=4096 and M>=9*R^2, using the existing Suzuki proper-prime-power bound and Chebyshev estimates. With R=2*k^3, the full actual homogeneous moment has coefficient (2^62*k^6)^(k^3) and defect k^2/256 at order (7k+1)k, for every k>=2 and P>=1 without a moment premise. Both moment coefficients and all Gaussian costs fit the actual high-moment root with multiplier two. The actual polynomial product has coefficient two and saving 1/(8192*k^2); its original Dirichlet product has coefficient three, and the full original block has bound 5*M^(4-1/(8192*k^2)). The range is k>=12, M>=(7k+1)k, M^(2k-2)<=t<=M^(2k) and M^4<=z<=2*M^4. Partial blocks retain the 2*M^2 boundary, while decreasing weights pay their actual mass and only their initial weight at that boundary. Literal damped zeta terms at nonnegative real part inherit this bound. All constants and the degree-dependent starting condition are explicit. All-scale coverage is now proved for the actual canonical dyadic blocks. Integer fourth-root rounding gives 6*X^(1-sigma-1/(32768*k^2))+2*X^(1/2-sigma). For n>=12 and t>=(16*(2*n+1)^2)^(2*n), the middle-window degree and root are constructed, the small blocks pay their damped mass, and sixth- and fourth-derivative profiles pay every longer block through 4*t. With Delta_n=1/(65536*(2*n+1)^2) and a_n=2*Delta_n/n, the actual zeta function on sigma=1-Delta_n satisfies |zeta(s)|<=32768*|t|^a_n*log|t|/Delta_n. The full strip sigma>=1-Delta_n, sigma<1 has the same exponent with denominator 1-sigma. On each adjacent band the exponent is at most 12288*(1-sigma)^(3/2). Every eta endpoint, infinite tail and division cost is paid; both signs of height are covered. Direct Euler reconstruction now improves the coefficient to 8192 and removes the whole 1/(1-sigma) factor. This uniform bound holds on the closed strip 1-Delta_n<=sigma<=1, including the one-line, and retains the same three-halves exponent on adjacent bands. The band-dependent height threshold remains explicit. The earlier complete zero detector and signed angular continuation remain proved. The shorter actual moments now have orders (3*k+1)k and (4*k+1)k, defects k^2/40 and k^2/100, and coefficient (2^41*k^6)^(k^3), at every positive endpoint. Both moment coefficients and the entire variable-order Gaussian factor fit the true root with multiplier two. The original full Dirichlet block has bound 5*M^(4-saving(k)), where saving(k)=1/(512*k^2) for k>=48 and 1/(1600*k^2) for 12<=k<48; its full range requires M>=(4*k+1)k and retains the original time rectangle. Partial and damped blocks retain the complete boundary. Every dyadic scale is paid, including the lower-degree window. For n>=48, Delta_n=1/(4096*(2*n+1)^2) and a_n=2*Delta_n/n give |zeta(s)|<=8192*|t|^a_n*log|t| on the closed strip 1-Delta_n<=Re(s)<=1 above the unchanged T_n=(16*(2*n+1)^2)^(2*n). The strip is sixteen times wider at a fixed index, with its increased exponent paid; adjacent bands have exponent at most 3072*(1-sigma)^(3/2). The generic signed angular detector retains multiplicity and radial correction; both actual analytic-disc bounds are discharged. The single natural degree floor((L/log L)^(1/3)/32) pays the threshold. The normalized radius tends to 1/16, both oscillatory profiles divided by log L tend to 5, and the full Euler-center logarithm divided by log L tends to 2/3. The entire original cost tends to 38080*C/(3*pi), giving actual closed-edge nonvanishing and both zero-strip edges for 0<C<3*pi/38080. The ceiling increases by exactly 44/17; C=1/4500 is checked and exceeds the whole earlier signed coefficient range. All previous explicit and eventual shapes remain in the proved family and union. The finite starting height is unevaluated. Every positive member eventually strictly exceeds each fixed earlier log-log coefficient. At C>=1/4500 the older fixed VK component is absorbed exactly in the compact union. The retained physical-scale continuation now keeps v=log(X)/log(t) coupled to the actual selected degree. For high-degree blocks, k*v<=9/4 gives the unmaximized exponent delta*v-v^3/10368; its numerical maximum is at most 40*delta*sqrt(delta). The lower-degree range, original shift boundary, all long blocks and the expanded small-block range are paid independently. The actual whole zeta bound is now 8192*|t|^(40*Delta_n^(3/2))*log|t| on the same closed strip for n>=48, above the explicit larger T_(8n). The exponent is strictly less than 5/32 of the preceding exponent at a fixed displacement; do not discard the changed height threshold. Continuous adjacent bands have exponent at most 320*(1-sigma)^(3/2). The unmaximized profile remains a named theorem for further scale summation. On the actual natural degree floor((L/log L)^(1/3)/64), the full radius has normalized limit 1/4, both growth profiles have normalized limit six, and the Euler-center cost remains 2/3. T_(8n)+1 is paid on this same schedule. The full signed cost tends to 11200*C/(3*pi). Actual nonvanishing and both zero-strip edges now hold for 0<C<3*pi/11200, including 1/1250. The coefficient ceiling increases by exactly 17/5 over the previous shorter-moment family; the new rational member exceeds that entire earlier coefficient interval. The compact union holds for C>=1/1250 and the eventual threshold is still unevaluated. The complete scale summation is now proved. Coefficient forty retains the extra cubic reserve -v^3/2097152, and the small blocks, finite lower-degree range, shift boundary and both long derivative profiles all pay that same decay. The exact derivative-profile shift keeps both complementary terms. Gaussian summation gives an absolute 1024*L^(2/3) bound for the complete dyadic cubic tail. Actual zeta therefore satisfies 1048576*|t|^(40*Delta_n^(3/2))*log(|t|)^(2/3) on the full closed strip 1-Delta_n<=Re(s)<=3/2 above the same T_(8n). This has a larger constant but the sharper classical logarithmic exponent. Both complete analytic profiles now have normalized limit 17/3; including the unchanged Euler-center cost gives 19/3. The entire signed cost tends to 10640*C/(3*pi), proving actual nonvanishing and both edges for 0<C<3*pi/10640, including 1/1150. The coefficient ceiling increases by exactly 20/19 over the preceding cubic family, whose entire interval is below the new rational member. The compact union holds for C>=1/1150. Its starting height remains finite and unevaluated. Sharper incomplete-system moments, asymmetric block estimates, published leading constants and finite starting-height evaluation remain open. This growing-height theorem does not discharge the independent fixed-height Riesz signed floor or harmonic Type-II premise. RH remains open and no world-record claim is made."),
    ("fordSelectedMomentTheorem", .str "RiemannGaussian.VinogradovFordSelectedIteration.selected_moment_bound"),
    ("fordSelectedDefectRateTheorem", .str "RiemannGaussian.VinogradovFordDefectRate.selectedDefect_rate"),
    ("fordSelectedPositiveDefectTheorem", .str "RiemannGaussian.VinogradovFordLowerDefect.selectedDefect_pos"),
    ("fordPotentialIterationTheorem", .str "RiemannGaussian.VinogradovFordPotentialIteration.potential_cumulative"),
    ("fordClosedDefectTheorem", .str "RiemannGaussian.VinogradovFordClosedDefect.selected_defect_bound"),
    ("fordClosedDefectMomentTheorem", .str "RiemannGaussian.VinogradovFordClosedDefect.selected_moment_closed_defect"),
    ("fordCoefficientScaleTheorem", .str "RiemannGaussian.VinogradovFordCoefficientScale.published_height_le"),
    ("fordEarlyDefectDropTheorem", .str "RiemannGaussian.VinogradovFordEarlyDefect.early_defect_drop"),
    ("fordEarlyCoefficientStepTheorem", .str "RiemannGaussian.VinogradovFordCoefficientStep.early_step_le"),
    ("fordCoefficientBudgetTheorem", .str "RiemannGaussian.VinogradovFordCoefficientBudget.coefficient_after_early"),
    ("fordClosedCoefficientTheorem", .str "RiemannGaussian.VinogradovFordClosedCoefficient.selected_coefficient_bound"),
    ("fordClosedMomentTheorem", .str "RiemannGaussian.VinogradovFordClosedCoefficient.selected_moment_closed"),
    ("fordOrderInterpolationTheorem", .str "RiemannGaussian.VinogradovMomentInterpolation.meanValue_bound"),
    ("fordAllOrdersTheorem", .str "RiemannGaussian.VinogradovFordAllOrders.published_moment_bound_real"),
    ("rosserZeroPoissonMassTheorem", .str "RiemannGaussian.RosserSchoenfeldZeroMass.hasSum_poisson"),
    ("rosserOrdinateSquareMassTheorem", .str "RiemannGaussian.RosserSchoenfeldZeroMass.ordinate_mass_lt"),
    ("rosserNormSquareMassTheorem", .str "RiemannGaussian.RosserSchoenfeldZeroMass.norm_square_mass_lt"),
    ("rosserCubicLogLowerTheorem", .str "RiemannGaussian.RosserSchoenfeldSharpConstants.log_cubic_lower"),
    ("rosserSharpEulerUpperTheorem", .str "RiemannGaussian.RosserSchoenfeldSharpConstants.euler_upper"),
    ("rosserSharpLogPiLowerTheorem", .str "RiemannGaussian.RosserSchoenfeldSharpConstants.log_pi_lower"),
    ("rosserNormCubeMassTheorem", .str "RiemannGaussian.RosserSchoenfeldCubeMass.norm_cube_mass_lt"),
    ("rosserNormFourthMassTheorem", .str "RiemannGaussian.RosserSchoenfeldFourthMass.norm_fourth_mass_lt"),
    ("rosserZetaTwoLogDerivativeTheorem", .str "RiemannGaussian.RosserSchoenfeldZetaTwo.log_derivative_upper"),
    ("rosserSmoothedPrimeFormulaTheorem", .str "RiemannGaussian.RosserSchoenfeldExplicitFormula.finite_prime_explicitFormula"),
    ("rosserHighZeroTailTheorem", .str "RiemannGaussian.RosserSchoenfeldZeroTail.value_le"),
    ("rosserLargeSmoothedErrorTheorem", .str "RiemannGaussian.RosserSchoenfeldLargeSmoothed.abs_prime_sub_main_lt"),
    ("rosserLargePsiErrorTheorem", .str "RiemannGaussian.RosserSchoenfeldLargeChebyshev.abs_psi_sub_exp_le"),
    ("rosserLargeThetaErrorTheorem", .str "RiemannGaussian.RosserSchoenfeldLargeChebyshev.abs_theta_sub_exp_le"),
    ("rosserPublishedLargeThetaTheorem", .str "RiemannGaussian.RosserSchoenfeldLargeChebyshev.abs_theta_sub_self_lt_published"),
    ("rosserFinitePrimeCountBoundsTheorem", .str "RiemannGaussian.RosserSchoenfeldFiniteBounds.bounds_through_sixteen_thousand"),
    ("rosserFinitePrimeCountTheorem", .str "RiemannGaussian.RosserSchoenfeldFiniteBounds.primeCounting_sixteen_thousand"),
    ("rosserCompletePrimeCatalogTheorem", .str "RiemannGaussian.RosserSchoenfeldFiniteBounds.catalog_mem_iff"),
    ("rosserPrimeCatalogLookupTheorem", .str "RiemannGaussian.RosserSchoenfeldFiniteBounds.smallPrimeCount_eq"),
    ("rosserRemainingPrimeSupplyTheorem", .str "RiemannGaussian.VinogradovRosserPrimeSupply.shortPrimeSupply_of_primeCounting_above_sixteen_thousand"),
    ("rosserFiniteThetaTheorem", .str "RiemannGaussian.RosserSchoenfeldFiniteTheta.bounds_through_sixteen_thousand"),
    ("rosserLargePrimeCountTheorem", .str "RiemannGaussian.RosserSchoenfeldLargePrimeCounting.bounds_above_exp_5100"),
    ("rosserIntermediatePrimeSupplyTheorem", .str "RiemannGaussian.VinogradovRosserLargeSupply.shortPrimeSupply_of_intermediate_counts"),
    ("fordLargeBaseTheorem", .str "RiemannGaussian.VinogradovRosserLargeSupply.publishedBase_large_of_log_ge"),
    ("fordLargePrimeSupplyTheorem", .str "RiemannGaussian.VinogradovRosserLargeSupply.shortPrimeSupply_of_log_ge"),
    ("fordLargeFixedWidthSupplyTheorem", .str "RiemannGaussian.VinogradovRosserLargeSupply.fixedWidth_shortPrimeSupply"),
    ("fordUnconditionalLargeMomentTheorem", .str "RiemannGaussian.VinogradovFordLargeDegree.moment_bound_real"),
    ("rosserProvedPrimeRangesScope", .str "Unconditionally, the original theta allowances with constants -47/100 and 31/100 hold for every real 1451<=x<=16000, using the complete prime catalog and 69 closed kernel-checked cells. Both original strict prime-count inequalities hold on 67<=x<=16000 and x>=exp(5100). The infinite count range follows from the proved 41/100 theta error, exact Abel summation and an explicit bound for the entire earlier prefix. The unchanged full-width ShortPrimeSupply statement is now conditional only on the two count inequalities for 16000<x<exp(5100); this middle interval remains open. Separate large-degree packet and moment theorems discharge that arithmetic premise when log(k)>=1700. The full all-degree literature theorem, published VK benchmark, larger zero-free region and RH signed floor are not claimed."),
    ("fordUnconditionalLargeMomentScope", .str "Unconditionally, when log(k)>=1700 the original published packet base is at least exp(5100), and the actual prime-count theorem constructs exactly k^3 primes in every original interval (M,(1+omega)*M]. This proves ShortPrimeSupply throughout 1/(3*log(k))<=omega<=1/2, including omega=3/50, without a prime-count or supply premise. The actual complete Ford moment bound therefore holds for every integer 2*k^2<=s<=(k^2/2)*(1/2+log(3*k/8)) and every real P>=1, retaining the original coefficient, 1.7/k defect allowance and literal floor cutoff. This is an explicit large-degree part of Ford Theorem 3, not its full k>=1000 range. Intermediate prime counts, smaller degrees, incomplete-system moments, sharp zeta transport and the published VK benchmark remain open. No new zero-free region, RH signed floor or numerical certificate follows."),
    ("rosserFinitePrimeCountScope", .str "Unconditionally, the original strict inequalities x/(log x-1/2)<pi(x)<x/log x*(1+3/(2*log x)) hold for every real 67<=x<=16000, where pi(x) is the actual Nat.primeCounting of floor(x). Every integer in the finite enumeration is checked, all 136 closed logarithmic cells cover their endpoints, and each rational margin is reduced by the Lean kernel. The exact endpoint count is 1862. The retained complete prime catalog contains precisely all primes through 16000, without duplicates, and every filtered lookup agrees with the actual smaller count. The unchanged full-width ShortPrimeSupply conclusion now follows conditionally from the remaining actual count estimates above 16000; those estimates are not assumed as a theorem. The separate proved-range endpoint now pays theta on [1451,16000] and both count bounds for x>=exp(5100). Intermediate counts, full all-degree prime supply, published VK benchmarks and any larger zero-free region remain open. No RH floor or numerical zero-certificate improvement is claimed."),
    ("rosserPrimeErrorScope", .str "Unconditionally, the literal finite sum Q(t)=sum_{n<=exp(t)} Lambda(n)*(t-log n) equals exp(t)-1-log(2*pi)*t-Z(t)+H(t) for every t>=0. Z retains the entire nontrivial zero divisor with multiplicities and both signs; H retains every trivial zero. Their actual Laplace transforms, continuity, absolute convergence and every sum-integral exchange are proved. The complete inverse-square high-zero tail is at most (8+2*log T)/T for T>=2, by the exact positive xi Poisson mass. The already proved signed-pole edge and the explicit cutoff exp(3*sqrt(t)/8) give |Q(t)-(exp(t)-1-log(2*pi)*t)|<exp(t)/(64*t^2) for every t>=4900. Monotone desmoothing at step 1/(4*t) proves |psi(x)-x|<=0.40*x/log x and |theta(x)-x|<=0.41*x/log x for every x>=exp(5000). All proper prime powers are paid. The strict source allowance 0.47*x/log x is therefore proved on its whole final range, with no prime-error premise or additional low-zero verification. The complete published prime-count inequalities and full VK benchmark reproduction remain open. Separate range theorems now prove finite theta, the count tail from exp(5100), and unchanged ShortPrimeSupply with actual complete Ford moments when log(k)>=1700; the smaller degrees remain open. This is not Theorem 11's sharper exponential error, a new zero-free region, an RH floor or a changed numerical certificate."),
    ("rosserZeroMassScope", .str "Unconditionally, the complete multiplicity-weighted reciprocal zero-norm sums are strictly below 463/10000 for squares, 167/100000 for cubes and 744/10000000 for fourth powers, retaining both ordinate signs. All three series are proved summable. The fourth-power bound uses an exact difference of reflected xi Poisson masses at one and two, complete critical-line verification through height 54, the absence of zeros through 14 and a kernel-checked bound on the actual zeta logarithmic derivative at two. The inverse-cube comparison subtracts a strict saving from the rigorously isolated first conjugate pair. No RH, external zero table, prime-count estimate or short-prime supply is assumed. The actual smoothed explicit formula and final-range theta bound are now proved in the separate Rosser prime-error endpoint. The larger low-zero verification, smaller source ranges and full prime-count estimates remain open. These classical Rosser dependencies supply no enlarged zero-free region or independent RH signed floor."),
    ("fordSelectedIterationScope", .str "For k>=1000, the actual selected defect has the original 1.69/k exponential allowance throughout the full published upper order range. The original coefficient product is now proved with constants 2.055, 5.91 and 9.7278, using V^(k+1)<=W=k^(4.11*k), the early 0.01916*k defect decrease and exact telescoping of all later packet costs. Actual torus Holder interpolation covers every integer order 2*k^2<=s<=(k^2/2)*(1/2+log(3*k/8)), retaining the original 1.7/k all-order defect constant and every real endpoint P>=1 via the literal floor cutoff. ShortPrimeSupply k (3/50) remains the explicit arithmetic premise on the full k>=1000 range; a separate theorem discharges it when log(k)>=1700. No moment, coefficient, schedule or interpolation estimate is assumed. All scalar estimates are independent of prime supply. Dense short-prime supply for the remaining degrees, the lower-degree numerical tables and the incomplete-moment/zeta transport remain open. No new zero-free region, RH signed floor or numerical zero certificate follows."),
    ...
    ("vkGaussianCenterCostTheorem", .str "RiemannGaussian.VinogradovKorobovMoment.interval_centered_cost_le_quarter"),
    ...
    ("vkExplicitProfileDepthTheorem", .str "RiemannGaussian.VinogradovLinearProfile.bounded_depth_profile_iteration"),
    ("vkRelativeProfileDepthTheorem", .str "RiemannGaussian.VinogradovRelativeProfile.relative_defect_conditioned_bound"),
    ("vkQuantitativeRelativeMomentTheorem", .str "RiemannGaussian.VinogradovQuantitativeDescent.relative_moment_bound"),
    ("vkQuantitativeMomentCostAuditTheorem", .str "RiemannGaussian.VinogradovQuantitativeDescent.packet_square_power_le_momentMultiplier"),
    ("vkRelativePolynomialSavingTheorem", .str "RiemannGaussian.VinogradovRelativeResonance.polynomial_bound"),
    ("vkQuantitativeDirichletBlockTheorem", .str "RiemannGaussian.VinogradovQuantitativeDirichlet.block_bound"),
    ("vkQuantitativeDampedFeatureTheorem", .str "RiemannGaussian.VinogradovQuantitativeDirichlet.feature_block_bound"),
    ("vkPolynomialCollisionTheorem", .str "RiemannGaussian.VinogradovPolynomialRigidity.weighted_equations_iff"),
    ("vkPolynomialSystemTypeTheorem", .str "RiemannGaussian.VinogradovPolynomialSystems.hasType_difference"),
    ("vkPolynomialSystemCongruenceTheorem", .str "RiemannGaussian.VinogradovPolynomialSystems.type_integer_congruence_card_le"),
    ("vkMixedBoundaryMomentTheorem", .str "RiemannGaussian.VinogradovMixedMoments.intermediate_power_le"),
    ("vkResidueDifferenceEnergyTheorem", .str "RiemannGaussian.VinogradovDifferenceEnergy.residueEnergy_eq_differences"),
    ("vkMixedDifferencingTheorem", .str "RiemannGaussian.VinogradovMixedDifferencing.exists_difference_bound"),
    ("vkTypedDifferencingTheorem", .str "RiemannGaussian.VinogradovPolynomialDifferencing.exists_typed_difference_bound"),
    ("vkPolynomialConditioningEnergyTheorem", .str "RiemannGaussian.VinogradovPolynomialConditioning.configuration_integral_le"),
    ("vkNonsingularCountTheorem", .str "RiemannGaussian.VinogradovPolynomialNonsingular.prime_dilated_count_le"),
    ("vkDilatedTailMomentTheorem", .str "RiemannGaussian.VinogradovNonsingularDescent.monomialTail_moment"),
    ("vkNonsingularDescentTheorem", .str "RiemannGaussian.VinogradovNonsingularDescent.exists_nonsingular_descent"),
    ("vkLowDegreeTailFibreTheorem", .str "RiemannGaussian.VinogradovLowDegreeTail.lowSignature_fibre_le"),
    ("vkLowDegreeTailConditioningTheorem", .str "RiemannGaussian.VinogradovLowDegreeTail.exists_tail_class_bound"),
    ("vkTailEndpointDeletionTheorem", .str "RiemannGaussian.VinogradovEndpointDeletion.mixedMoment_succ_le"),
    ("vkResidueFloorEndpointTheorem", .str "RiemannGaussian.VinogradovResidueEndpoint.exists_floor_tail_bound"),
    ("vkBinomialRowTypeTheorem", .str "RiemannGaussian.VinogradovBinomialRows.hasType_rowSystem"),
    ("vkBinomialRowCollisionTheorem", .str "RiemannGaussian.VinogradovRowTransport.shifted_count_eq"),
    ("vkOriginalNonsingularConditioningTheorem", .str "RiemannGaussian.VinogradovPolynomialConditionedDescent.exists_conditioning"),
    ("vkOriginalNonsingularDescentTheorem", .str "RiemannGaussian.VinogradovPolynomialConditionedDescent.exists_descent"),
    ("vkTypeMaximumTheorem", .str "RiemannGaussian.VinogradovTypeMaximum.exists_maximizer"),
    ("vkMixedRepeatedBoundTheorem", .str "RiemannGaussian.VinogradovMixedRepeated.repeated_integral_le"),
    ("vkMixedRepeatedAbsorptionTheorem", .str "RiemannGaussian.VinogradovMixedExceptional.mixedMoment_le_twice_distinct"),
    ("vkDistinctTypeReductionTheorem", .str "RiemannGaussian.VinogradovPolynomialExceptional.exists_count_reduction"),
    ("vkTwoBlockPacketTheorem", .str "RiemannGaussian.VinogradovTwoBlockPacket.exists_uniform_two_block_packet"),
    ("vkUnrestrictedPrimeTransferTheorem", .str "RiemannGaussian.VinogradovPolynomialPrimeTransfer.exists_prime_carrying_count"),
    ("vkUnrestrictedConditioningTheorem", .str "RiemannGaussian.VinogradovUnrestrictedConditioning.exists_conditioning"),
    ("vkDiagonalMomentStepTheorem", .str "RiemannGaussian.VinogradovDiagonalMoment.exists_diagonal_moment_step"),
    ("vkDiagonalAllEndpointStepTheorem", .str "RiemannGaussian.VinogradovDiagonalThreshold.all_endpoint_defect_step"),
    ("vkDiagonalIterationTheorem", .str "RiemannGaussian.VinogradovDiagonalIteration.iterated_moment_bound"),
    ("vkDiagonalDefectTheorem", .str "RiemannGaussian.VinogradovDiagonalIteration.defect_seven_degree_le"),
    ("vkDiagonalCoefficientTheorem", .str "RiemannGaussian.VinogradovDiagonalCost.coefficient_seven_degree_le"),
    ("vkDiagonalExplicitMomentTheorem", .str "RiemannGaussian.VinogradovDiagonalCost.explicit_relative_moment_bound"),
    ("vkShortPrimePacketTheorem", .str "RiemannGaussian.VinogradovShortPacket.exists_short_packet"),
    ("vkPacketConditioningTheorem", .str "RiemannGaussian.VinogradovUnrestrictedConditioning.exists_conditioning_of_packet"),
    ("vkNarrowAllEndpointTheorem", .str "RiemannGaussian.VinogradovNarrowThreshold.all_endpoint_defect_step"),
    ("vkNarrowMomentTheorem", .str "RiemannGaussian.VinogradovNarrowCost.explicit_relative_moment_bound"),
    ("vkNarrowCoefficientRootTheorem", .str "RiemannGaussian.VinogradovNarrowCost.two_moments_gaussian_le"),
    ("vkNarrowActualGaussianTheorem", .str "RiemannGaussian.VinogradovNarrowGaussian.actual_gaussian_resonance_le"),
    ("vkNarrowPolynomialTheorem", .str "RiemannGaussian.VinogradovNarrowResonance.polynomial_bound"),
    ("vkNarrowDirichletBlockTheorem", .str "RiemannGaussian.VinogradovNarrowDirichlet.block_bound"),
    ("vkNarrowWeightedBlockTheorem", .str "RiemannGaussian.VinogradovNarrowDirichlet.weighted_block_bound"),
    ("vkNarrowDampedFeatureTheorem", .str "RiemannGaussian.VinogradovNarrowDirichlet.feature_block_bound"),
    ("vkDyadicBlockTheorem", .str "RiemannGaussian.VinogradovDyadic.original_block_bound"),
    ("vkScaleSelectionTheorem", .str "RiemannGaussian.VinogradovScaleSelection.exists_window_parameters"),
    ("vkLongSixthTheorem", .str "RiemannGaussian.VinogradovLongBlocks.sixth_profile_le"),
    ("vkLongFourthTheorem", .str "RiemannGaussian.VinogradovLongBlocks.fourth_profile_le"),
    ("vkAllScaleBlockTheorem", .str "RiemannGaussian.VinogradovNearOneBudget.canonical_block_bound"),
    ("vkDisplacementPowerTheorem", .str "RiemannGaussian.VinogradovNearOneBudget.growth_le_displacement_power"),
    ("vkNearOneZetaTheorem", .str "RiemannGaussian.ZetaVinogradovBound.bound_abs"),
    ("vkNearOneStripTheorem", .str "RiemannGaussian.ZetaVinogradovBound.bound_strip_abs"),
    ("vkNearOneBandTheorem", .str "RiemannGaussian.ZetaVinogradovBound.bound_band_abs"),
    ("vkEulerVinogradovLineTheorem", .str "RiemannGaussian.ZetaVinogradovEulerBound.bound_abs"),
    ("vkEulerVinogradovStripTheorem", .str "RiemannGaussian.ZetaVinogradovEulerBound.bound_strip_abs"),
    ("vkEulerVinogradovBandTheorem", .str "RiemannGaussian.ZetaVinogradovEulerBound.bound_band_abs"),
    ("vkRightStripTheorem", .str "RiemannGaussian.ZetaEulerRightStrip.bound_abs"),
    ("vkLocalDiscTheorem", .str "RiemannGaussian.ZetaVinogradovLocalDisc.disc_norm_bound"),
    ("vkCanonicalTheorem", .str "RiemannGaussian.ZetaVinogradovCanonical.exists_controlled_decomp"),
    ("vkSignedBudgetTheorem", .str "RiemannGaussian.ZetaVinogradovSignedBudget.margin_of_budget"),
    ("vkMarginPaidTheorem", .str "RiemannGaussian.ZetaVinogradovMargin.budget_paid"),
    ("vkFixedMarginTheorem", .str "RiemannGaussian.ZetaVinogradovMargin.exact_margin"),
    ("vkFixedNonvanishingTheorem", .str "RiemannGaussian.ZetaVinogradovMargin.nonvanishing"),
    ("vkHeightScheduleTheorem", .str "RiemannGaussian.VinogradovHeightSchedule.eventually_threshold"),
    ("vkScheduledReserveTheorem", .str "RiemannGaussian.ZetaVinogradovScheduledMargin.reserve_le"),
    ("vkScheduledWidthTheorem", .str "RiemannGaussian.ZetaVinogradovScheduledMargin.width_le_margin"),
    ("vkScheduledParametersTheorem", .str "RiemannGaussian.ZetaVinogradovScheduledMargin.eventually_parameters"),
    ("vkLogLogImprovementTheorem", .str "RiemannGaussian.ZetaVinogradovAngularZeroFree.eventually_dominates_previous_loglog"),
    ("vkZeroFreeTheorem", .str "RiemannGaussian.ZetaVinogradovSummedZeroFree.exists_eventual_strip"),
    ("vkNonvanishingTheorem", .str "RiemannGaussian.ZetaVinogradovSummedZeroFree.exists_eventual_nonvanishing"),
    ("vkUnionTheorem", .str "RiemannGaussian.ZetaVinogradovSummedZeroFree.exists_eventual_union"),
    ...
    ("vkScaledParametersTheorem", .str "RiemannGaussian.ZetaVinogradovBalancedScale.eventually_parameters"),
    ("vkCenterCostTheorem", .str "RiemannGaussian.ZetaVinogradovBalancedBudget.center_log_normalized"),
    ("vkExactCostTheorem", .str "RiemannGaussian.ZetaVinogradovBalancedBudget.cost_tendsto"),
    ("vkCoefficientTheorem", .str "RiemannGaussian.ZetaVinogradovSummedZeroFree.rational_coefficient_lt"),
    ("vkMillionfoldTheorem", .str "RiemannGaussian.ZetaVinogradovAngularZeroFree.millionfold_improvement"),
    ("vkCompactUnionTheorem", .str "RiemannGaussian.ZetaVinogradovSummedZeroFree.exists_eventual_union_compact"),
    ("vkSummedReserveTheorem", .str "RiemannGaussian.VinogradovCubicDecay.cubic_reserve"),
    ("vkSummedSmallTheorem", .str "RiemannGaussian.VinogradovCubicDecay.small_reserve"),
    ("vkSummedLowDegreeTheorem", .str "RiemannGaussian.VinogradovCubicDecay.low_degree_reserve"),
    ("vkSummedShiftTheorem", .str "RiemannGaussian.VinogradovCubicDecay.profile_shift"),
    ("vkSummedCoverageTheorem", .str "RiemannGaussian.VinogradovCubicDecay.block_bound"),
    ("vkSummedGaussianTheorem", .str "RiemannGaussian.VinogradovCubicSummation.sum_le"),
    ("vkSummedPrefixTheorem", .str "RiemannGaussian.ZetaVinogradovSummedBound.prefix_bound"),
    ("vkSummedStripTheorem", .str "RiemannGaussian.ZetaVinogradovSummedBound.bound_strip_abs"),
    ("vkSummedDiscTheorem", .str "RiemannGaussian.ZetaVinogradovSummedDisc.norm_translated_le"),
    ("vkSummedActualBudgetTheorem", .str "RiemannGaussian.ZetaVinogradovSummedCost.margin_of_budget"),
    ("vkSummedCostTheorem", .str "RiemannGaussian.ZetaVinogradovSummedCost.cost_tendsto"),
    ("vkSummedGainTheorem", .str "RiemannGaussian.ZetaVinogradovSummedZeroFree.coefficient_gain"),
    ("vkSummedPreviousCeilingTheorem", .str "RiemannGaussian.ZetaVinogradovSummedZeroFree.previous_limit_lt_rational"),
    ("vkCubicScaleCouplingTheorem", .str "RiemannGaussian.VinogradovCubicSaving.degree_log_bound"),
    ("vkCubicProfileTheorem", .str "RiemannGaussian.VinogradovCubicSaving.power_le_cubic_profile"),
    ("vkCubicMaximumTheorem", .str "RiemannGaussian.VinogradovCubicSaving.cubic_le"),
    ("vkCubicCoverageTheorem", .str "RiemannGaussian.VinogradovCubicBudget.canonical_block_bound"),
    ("vkCubicGrowthGainTheorem", .str "RiemannGaussian.VinogradovCubicBudget.growth_lt_previous"),
    ("vkCubicStripTheorem", .str "RiemannGaussian.ZetaVinogradovCubicBound.bound_strip_abs"),
    ("vkCubicBandTheorem", .str "RiemannGaussian.ZetaVinogradovCubicBound.bound_band_abs"),
    ("vkCubicDiscTheorem", .str "RiemannGaussian.ZetaVinogradovCubicDisc.norm_translated_le"),
    ("vkCubicScheduleTheorem", .str "RiemannGaussian.VinogradovCubicSchedule.eventually_threshold"),
    ("vkCubicParametersTheorem", .str "RiemannGaussian.ZetaVinogradovCubicScale.eventually_parameters"),
    ("vkCubicActualBudgetTheorem", .str "RiemannGaussian.ZetaVinogradovCubicCost.margin_of_budget"),
    ("vkCubicCostTheorem", .str "RiemannGaussian.ZetaVinogradovCubicCost.cost_tendsto"),
    ("vkCubicGainTheorem", .str "RiemannGaussian.ZetaVinogradovCubicZeroFree.coefficient_gain"),
    ("vkCubicPreviousCeilingTheorem", .str "RiemannGaussian.ZetaVinogradovCubicZeroFree.previous_limit_lt_rational"),
    ("vkExponentialDefectTheorem", .str "RiemannGaussian.VinogradovExponentialDefect.defect_le_exp"),
    ("vkThreeMomentTheorem", .str "RiemannGaussian.VinogradovShortMoment.moment_three_bound"),
    ("vkFourMomentTheorem", .str "RiemannGaussian.VinogradovShortMoment.moment_four_bound"),
    ("vkShortMomentCostTheorem", .str "RiemannGaussian.VinogradovShortMoment.two_moments_gaussian_le"),
    ("vkVariableGaussianTheorem", .str "RiemannGaussian.VinogradovVariableGaussian.actual_gaussian_resonance_le"),
    ("vkShortPolynomialTheorem", .str "RiemannGaussian.VinogradovShortResonance.polynomial_bound"),
    ("vkShortDirichletTheorem", .str "RiemannGaussian.VinogradovShortDirichlet.block_bound"),
    ("vkShortDampedTheorem", .str "RiemannGaussian.VinogradovShortDirichlet.feature_block_bound"),
    ("vkShortDyadicTheorem", .str "RiemannGaussian.VinogradovShortDyadic.original_block_bound"),
    ("vkSharperCoverageTheorem", .str "RiemannGaussian.VinogradovSharperBudget.canonical_block_bound"),
    ("vkSharperStripTheorem", .str "RiemannGaussian.ZetaVinogradovSharperBound.bound_strip_abs"),
    ("vkSharperBandTheorem", .str "RiemannGaussian.ZetaVinogradovSharperBound.bound_band_abs"),
    ("vkSharperDiscTheorem", .str "RiemannGaussian.ZetaVinogradovSharperDisc.norm_translated_le"),
    ("vkGenericAngularBudgetTheorem", .str "RiemannGaussian.ZetaAngularDiscBudget.margin_of_budget"),
    ("vkSharperActualBudgetTheorem", .str "RiemannGaussian.ZetaVinogradovSharperCost.margin_of_budget"),
    ("vkSharperHeightTheorem", .str "RiemannGaussian.VinogradovSharperSchedule.eventually_threshold"),
    ("vkSharperRadiusTheorem", .str "RiemannGaussian.VinogradovSharperSchedule.radius_scaled"),
    ("vkSharperParametersTheorem", .str "RiemannGaussian.ZetaVinogradovSharperScale.eventually_parameters"),
    ("vkSharperCostTheorem", .str "RiemannGaussian.ZetaVinogradovSharperCost.cost_tendsto"),
    ("vkSharperGainTheorem", .str "RiemannGaussian.ZetaVinogradovSharperZeroFree.coefficient_gain"),
    ("vkEarlierCeilingTheorem", .str "RiemannGaussian.ZetaVinogradovSharperZeroFree.previous_limit_lt_rational"),
    ...
    ("rieszRenewalBoundaryTheorem", .str "RiemannGaussian.ZetaRieszTypeII.reflectedOuterPair_discrepancy"),
    ("rieszRenewalInnerTheorem", .str "RiemannGaussian.ZetaRieszTypeII.pairDiscrepancy_inner_boundary"),
    ("rieszRenewalFiniteTheorem", .str "RiemannGaussian.ZetaRieszTypeII.renewalHinge_eq_riesz"),
    ("rieszRenewalPairTheorem", .str "RiemannGaussian.ZetaRieszTypeII.pairForm_eq_minFac_renewal"),
    ("rieszRenewalBranchTheorem", .str "RiemannGaussian.ZetaRieszTypeII.inner_layer_renewal"),
    ("rieszRenewalCellTheorem", .str "RiemannGaussian.ZetaRieszTypeII.cellBand_not_closed"),
    ("rieszRenewalKernelTheorem", .str "RiemannGaussian.ZetaRieszTypeII.no_strict_density_contraction"),
    ("rieszWideOwnerCompletionTheorem", .str "RiemannGaussian.ZetaRieszWideOwnerAudit.completion_rate"),
    ("rieszWideOwnerAllocationTheorem", .str "RiemannGaussian.ZetaRieszWideOwnerAudit.ownerCompanion_eq_fibres"),
    ("rieszWideOwnerTailTheorem", .str "RiemannGaussian.ZetaRieszWideOwnerAudit.unallocatedTriples_bound"),
    ("rieszWideOwnerProductTheorem", .str "RiemannGaussian.ZetaRieszWideOwnerAudit.wide_product_scalar"),
    ("rieszWideOwnerJointTheorem", .str "RiemannGaussian.ZetaRieszWideOwnerAudit.exists_wide_completion_bound"),
    ("rieszWideOwnerCarrierTheorem", .str "RiemannGaussian.ZetaRieszWideOwnerAudit.narrow_completed_decomposition"),
    ("rieszWideOwnerSourceTheorem", .str "RiemannGaussian.ZetaRieszWideOwnerAudit.eventually_re_wideComplete_le"),
    ("rieszWideOwnerReserveTheorem", .str "RiemannGaussian.ZetaRieszWideOwnerAudit.not_tendsto_wideComplete_add_reserve"),
    ("rieszWideOwnerScope", .str "Concrete unique-largest-prime allocation on 1/2<u<=10001/20000, with cofactor orders 13N/40<k<=27N/40 and the unchanged original masks and allocation factor. Completion uses q=27/55, ell=11/8 and sigma=1+1/262144. The original exponent is <=-1/25000; the newly unallocated actual triple mass decays geometrically, uniformly in height. Complementary product and saturation corrections are paid at exp(1-N/200000). No-go for discarding the new complete companion: under the original exposed-zero hypotheses its normalized real part is <=-m^2/40, and after returning the old reserve it is <=-m^2/5000. Nondecay holds on the actual dyadic schedule. The exact signed correction back to owned finite fibres remains explicit and has no independent decay bound. This is not a zero exclusion or a general impossibility theorem for future masked cancellation. Stop the direct completion route; retain LocalizedTypeIIBound and the public frontier unchanged."),
    ("rieszSkewRowTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.second_composite_convolution"),
    ("rieszSkewFactorialTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.skewAtom_eq_factorials"),
    ("rieszSkewPartitionTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.correction_eq_nonowner_rows"),
    ("rieszSkewJointTailTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.literal_missing_skew_bound"),
    ("rieszSkewBoundTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.skewResponse_bound"),
    ("rieszSkewDecayTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.tendsto_skewResponse"),
    ("rieszSkewNoReserveTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.not_eventually_norm_skewResponse_ge"),
    ("rieszSkewAllocationTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.tendsto_raw_sub_allocated"),
    ("rieszRectangleAllocationTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.boundedShare_mul_rectangleMass"),
    ("rieszRectangleMaskTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.tendsto_rectangleMaskError"),
    ("rieszRectangleComparisonTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.tendsto_separate_sub_rectangle"),
    ("rieszRectanglePhaseTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.eventually_rectangle_triple_phase"),
    ("rieszRectangleWeightTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.sum_rectangleSourceWeight_lower"),
    ("rieszRectangleReserveTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.eventually_rectangleReserve_ge"),
    ("rieszRectangleCorrectionTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.correction_eq_rectangle_rest"),
    ("rieszRectangleLedgerTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.narrow_rectangle_completion_ledger"),
    ("rieszRectangleOverlapTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.completion_rest_rectangle_ledger"),
    ("rieszRectangleNondecayTheorem", .str "RiemannGaussian.ZetaRieszSkewAllocation.not_tendsto_owned_rectangle_zero"),
    ("rieszParityHingeTheorem", .str "RiemannGaussian.ZetaRieszParityPacket.quintuple_coefficient_nonpos"),
    ("rieszParityLedgerTheorem", .str "RiemannGaussian.ZetaRieszParityPacket.packet_direct_ledger"),
    ("rieszParityCellTheorem", .str "RiemannGaussian.ZetaRieszParityPacket.re_frozenPacket"),
    ("rieszParityPhaseErrorTheorem", .str "RiemannGaussian.ZetaRieszParityPacket.tendsto_packet_sub_frozen"),
    ("rieszParityWindowTheorem", .str "RiemannGaussian.ZetaRieszParityPacket.exists_core_window_error"),
    ("rieszParityAllocationTheorem", .str "RiemannGaussian.ZetaRieszParityPacket.tendsto_quintuple_allocation"),
    ("rieszParityCountingAuditTheorem", .str "RiemannGaussian.ZetaRieszParityPacket.actual_prime_count_thin_cell"),
    ("rieszCascadeTransformTheorem", .str "RiemannGaussian.ZetaRieszContinuumCascade.cascadeTransform_zero"),
    ("rieszCascadeRenewalTheorem", .str "RiemannGaussian.ZetaRieszContinuumCascade.hasDerivAt_cascadeTransform"),
    ("rieszCascadePrimeVariationTheorem", .str "RiemannGaussian.ZetaRieszContinuumCascade.cutoff_contrast_prime_variation_ne_zero"),
    ("rieszCascadeFibreTheorem", .str "RiemannGaussian.ZetaRieszContinuumCascade.interior_variation_fibre"),
    ("rieszFullParityLedgerTheorem", .str "RiemannGaussian.ZetaRieszParityPacket.fullParity_direct_ledger"),
    ("rieszFullParityCountTheorem", .str "RiemannGaussian.ZetaRieszParityPacket.fullParityBox_count_le"),
    ("rieszCascadeAbelAuditTheorem", .str "RiemannGaussian.ZetaRieszCascadeAbelAudit.normalized_coreAbelAllowance_tendsto"),
    ("rieszZeroParityTransformTheorem", .str "RiemannGaussian.ZetaRieszZeroParityCascade.parityTransform_zero"),
    ("rieszZeroParitySupportTheorem", .str "RiemannGaussian.ZetaRieszZeroParityCascade.supportKernel_core"),
    ("rieszZeroParityInsertionTheorem", .str "RiemannGaussian.ZetaRieszZeroParityCascade.nonzero_insertion_count"),
    ("rieszZeroParityPhaseTheorem", .str "RiemannGaussian.ZetaRieszZeroParityCascade.simple_zero_product_phase"),
    ("rieszZeroParityOrderWitnessTheorem", .str "RiemannGaussian.ZetaRieszZeroParityOrders.concrete_rectangle_split"),
    ("rieszZeroParityOrderAuditTheorem", .str "RiemannGaussian.ZetaRieszZeroParityOrders.not_eventually_uniform_literal_split"),
    ("rieszParityBadOrderDecayTheorem", .str "RiemannGaussian.ZetaRieszParityOrderTail.tendsto_badPacket"),
    ("rieszParityGoodPhaseTheorem", .str "RiemannGaussian.ZetaRieszParityOrderTail.eventually_good_product_phase"),
    ("rieszParityFirstInsertionBoundTheorem", .str "RiemannGaussian.ZetaRieszParityFirstInsertion.firstInsertion_bounds"),
    ("rieszParityRectangleTailTheorem", .str "RiemannGaussian.ZetaRieszParityOrderTail.rectangleOmittedMass_bounds"),
    ("rieszParityAllocationTailTheorem", .str "RiemannGaussian.ZetaRieszParityOrderTail.fullParityBox_boundedShare"),
    ("rieszParityFactorialErrorsTheorem", .str "RiemannGaussian.ZetaRieszParityOrderTail.tendsto_coefficientBox_sub_fullParity"),
    ("rieszParitySupportedTailTheorem", .str "RiemannGaussian.ZetaRieszParityInsertionPoisson.nested_tail_core_bound"),
    ("rieszParityMaskedPhaseAuditTheorem", .str "RiemannGaussian.ZetaRieszParityMaskedPhaseAudit.scaled_defect_not_eventually_bounded"),
    ("rieszRadialMaskedRateAuditTheorem", .str "RiemannGaussian.ZetaRieszParityMaskedPhaseAudit.radial_growth_bounds"),
    ("rieszHalfPlaneMaskedBoundTheorem", .str "RiemannGaussian.ZetaRieszHalfPlaneModes.twoBand_bound"),
    ("rieszFilteredPacketBridgeTheorem", .str "RiemannGaussian.ZetaRieszFilteredMaskAudit.filteredPacket_eq_frozen_total_filter"),
    ("rieszPoleJetFilterMaskAuditTheorem", .str "RiemannGaussian.ZetaRieszFilteredMaskAudit.total_filter_ne_legwise_on_pole_mix"),
    ("rieszPoleJetInnerFilterTheorem", .str "RiemannGaussian.ZetaRieszFilteredMaskAudit.poleJetFilter_eval_exposed_inner_ne_zero"),
    ("rieszRightwardResonanceDepthTheorem", .str "RiemannGaussian.ZetaRieszHalfPlaneModes.resonance_depth_forces_rightward"),
    ("rieszRightwardModeObstructionTheorem", .str "RiemannGaussian.ZetaRieszRightwardModeAudit.no_uniform_successor_bound"),
    ("rieszShiftedPoleCancellationTheorem", .str "RiemannGaussian.ZetaRieszShiftedCenter.pole_cancellation"),
    ("rieszShiftedArithmeticLegTheorem", .str "RiemannGaussian.ZetaRieszShiftedCenter.leg_eq_multiplier"),
    ("rieszShiftedGlobalZeroTheorem", .str "RiemannGaussian.ZetaRieszShiftedCenter.completeLeg_global_error_bound"),
    ("rieszShiftedProductErrorTheorem", .str "RiemannGaussian.ZetaRieszShiftedCenter.product_multiplier_error_linear"),
    ("rieszNegativeModeCountTheorem", .str "RiemannGaussian.ZetaRieszNegativeModeCascade.hasSum_zero_cutoff_counts"),
    ("rieszMovingInsertionBoundTheorem", .str "RiemannGaussian.ZetaRieszMovingInsertion.movingInsertion_exp_bound"),
    ("rieszAdaptiveMarginalTheorem", .str "RiemannGaussian.ZetaRieszShiftedCenter.zero_order_full_marginal"),
    ("rieszAdaptiveSourceAuditTheorem", .str "RiemannGaussian.ZetaRieszShiftedCenter.shrinking_share_source_mass_tendsto"),
    ("rieszFiniteModeSupportTheorem", .str "RiemannGaussian.ZetaRieszNegativeModeSupport.inversePrimitive_core_patch"),
    ("rieszInfiniteModeAuditTheorem", .str "RiemannGaussian.ZetaRieszInfiniteModeAudit.no_dominated_cone_primitive"),
    ("rieszLocalDivisorTheorem", .str "RiemannGaussian.ZetaRieszLocalXiDivisor.fullGenerating_local"),
    ("rieszAnalyticFactorAuditTheorem", .str "RiemannGaussian.ZetaRieszAnalyticFactorAudit.leakage_inside_core_geometry"),
    ("rieszArtificialModeAuditTheorem", .str "RiemannGaussian.ZetaRieszArtificialModeAudit.frequently_residual_pole"),
    ("rieszFiniteModeAuditScope", .str "Every finite all-negative mode family has weak inverse support in d>=s, with all diagonal and low-order derivative channels retained; the literal strict core misses this support. A genuine finite local zeta divisor plus an analytic remainder of normalized radius greater than 4/3 is constructed. These are not a masked prime-packet estimate. Fixed ordinary primitives cannot pass to the complete xi product because of Gamma growth. A concrete analytic factor creates support below the cone, and finite artificial-mode normalization of a one-variable slice does not cancel the full two-variable singular surface. All prior no-go results and the open independent arithmetic floor are preserved."),
    ("rieszLiteralPrimeExpansionTheorem", .str "RiemannGaussian.ZetaRieszLeastBoundary.rawInteriorPacket_riesz_expansion"),
    ("rieszJointCountTailTheorem", .str "RiemannGaussian.ZetaRieszLeastBoundary.jointHighCountPacket_bound"),
    ("rieszJointCountRateTheorem", .str "RiemannGaussian.ZetaRieszLeastBoundary.jointCountTailRate_bounds"),
    ("rieszJointLowComparisonTheorem", .str "RiemannGaussian.ZetaRieszLeastBoundary.fullPacket_sub_jointLow_bound"),
    ...
    ("rieszCutoffPlateauTheorem", .str "RiemannGaussian.ZetaRieszCutoffProfile.riesz_reflected_plateau"),
    ("rieszPrimeCompensationIdentityTheorem", .str "RiemannGaussian.ZetaRieszPrimeCompensation.riesz_eq_unit_sub_prime_tents"),
    ("rieszPrimeCompensationMagnitudeTheorem", .str "RiemannGaussian.ZetaRieszPrimeCompensation.coefficient_le_neg_plateau_count_lowSupport"),
    ("rieszPrimeCompensationNormTheorem", .str "RiemannGaussian.ZetaRieszPrimeCompensation.norm_coefficient_le_prime_count"),
    ("rieszPrimeCompensationSupplyTheorem", .str "RiemannGaussian.ZetaRieszCompensationSupply.eventually_retained_positive_supply"),
    ("rieszMarkedPrimeHeadTheorem", .str "RiemannGaussian.ZetaRieszMarkedPrimeHead.tendsto_head"),
    ("rieszEulerQuotientErrorTheorem", .str "RiemannGaussian.ZetaRieszOrderedEulerBound.norm_scaled_errorResponse_le"),
    ("rieszEulerQuotientComparisonTheorem", .str "RiemannGaussian.ZetaRieszRoughEulerTransfer.tendsto_main_sub_current"),
    ("rieszMainFrequencyTheorem", .str "RiemannGaussian.ZetaRieszMainFrequency.norm_scaled_exterior_le"),
    ("rieszMarkedSeparationIdentityTheorem", .str "RiemannGaussian.ZetaRieszMarkedSeparation.separatedSymbol_factor"),
    ("rieszMarkedSeparationTheorem", .str "RiemannGaussian.ZetaRieszMarkedSeparation.norm_scaled_response_difference_le"),
    ("rieszMarkedPrimeCompletionIntegralTheorem", .str "RiemannGaussian.ZetaRieszMarkedPrimeCompletion.norm_integral_completionPair_le"),
    ("rieszMarkedPrimeCompletionBoundTheorem", .str "RiemannGaussian.ZetaRieszMarkedPrimeCompletion.norm_scaled_errorResponse_le"),
    ("rieszMarkedPrimeCompletionTheorem", .str "RiemannGaussian.ZetaRieszMarkedPrimeCompletion.tendsto_completed_sub_current"),
    ...
    ("rieszMarkedEulerComparisonTheorem", .str "RiemannGaussian.ZetaRieszOrderedEulerCompletion.tendsto_completion_sub_current"),
    ("rieszMarkedExteriorBoundTheorem", .str "RiemannGaussian.ZetaRieszMarkedCompletion.exteriorPacket_bound"),
    ("rieszPhysicalCompletionBoundTheorem", .str "RiemannGaussian.ZetaRieszPhysicalCompletion.exists_eventual_physical_bound"),
    ("rieszUnrestrictedPacketTheorem", .str "RiemannGaussian.ZetaRieszPhysicalCompletion.tendsto_packet_sub_current"),
    ("rieszPrimePrefixEulerLimitTheorem", .str "RiemannGaussian.ZetaRieszPhysicalCompletion.tendsto_prefixEuler"),
    ("rieszOrderedCompletionScope", .str "The existing factorial rectangle and paired Riesz Fourier response exactly sum all middle-prime subsets into an ordered least-prime Euler product. The empty-middle contribution is zero. Unrestricted-count share exteriors have an independent C*(N+1)*r^N bound with r<1, and radial tails are paid. Both physical prime endpoints can then be removed with a height-uniform geometric error: every label in the core/share interior already satisfies them. On the original dyadic schedule, for 1/2<u<=10001/20000 and any fixed height, the resulting unrestricted marked arithmetic series differs from lowerThresholdPacket (counts 3..55) minus shortOverflowPacket (counts 3..13) by source-scale o(1). For each fixed N>=21, the arithmetic series is also the limit of the actual finite ordered Euler integrals over primesLE X, using absolute convergence on Re s=3/2. No zero hypothesis, density replacement, discarded low orders, infinite zero-divisor inversion or exchange of the N and X limits is used. These are independently paid completion errors, not a bound for the common signed main term. Its signed estimate and the complementary carrier's independent floor remain open; no zero exclusion or whole-carrier frontier change follows."),
    ("rieszPrimeCompensationSupplyScope", .str "Actual five-prime products in fixed-width logarithmic boxes with slopes 1/25, 1/10, 11/50, 27/50, 11/10. Their exact coefficient is -log(n)*log(minFac(n))/L. The proved PNT counts distinct labels, and a bounded translation supplies favorable full product phase at each fixed nonzero height. On the original dyadic order/count schedule, with 1/2<u<=10001/20000, all current lowSupport masks hold and the original correlated rectangle retains at least half its factorial mass. The source-normalized real subfamily sum is eventually at least c(u,y)*(2u)^N/(N+1)^6, with c>0 and an unevaluated starting index. This is an independent signed estimate for a genuine suballocation, not a net cancellation bound for lowerThresholdPacket(3..55)-shortOverflowPacket(3..13). Remaining labels can have either phase sign and are not discarded or bounded. Neither the joint signed target nor the complementary floor is proved; no zero exclusion or public frontier change follows. Optional certified-prime examples check finite identities, not population density or a net floor."),
    ("rieszPrimeCompensationScope", .str "For squarefree n=P*r*a*b, retaining two small primes gives an exact unit tent minus the sum of actual prime tents when every distinct prime pair in b lies above the reflected cutoff. If r*a is below that cutoff, the unit tent vanishes and all remaining terms have the same negative sign. Each prime in the plateau interval supplies a full log(r) of negative response; the norm costs at most card(primeFactors b)*log(r), with the original log(n)/L coefficient normalization. On unchanged lowSupport with canonical largest/least factors, saturation is discharged. Pair separation and the small-pair cutoff remain explicit arithmetic tests; no prime-count class is silently deleted. This bounds individual arithmetic coefficients, not the real phased sum or the population of opposing labels. The optional ordinary-density probe integrates the radial exponential before share quadrature, retaining the moving length, all marked factorial slots and phase. It has no arithmetic transport theorem or bound for omitted counts 15..55. The exact coupled signed boundary target, complementary floor and public zero-free frontier remain unchanged."),
    ("rieszCutoffProfileScope", .str "Every squarefree four-prime label on the unchanged retained lowSupport has a nonnegative arithmetic coefficient for u>=1/2 and N>=2. A literal five-prime family on the core has a nonpositive coefficient when the product of its large marked prime and two small marked primes is below the physical cutoff; ordering, squarefreeness and core/physical geometry are explicit. The proof keeps the exact decreasing middle-three-prime profile. Full complex phase and original allocation weights remain, so these are not real carrier floors. Exact reflected plateaux survive whenever all middle primes exceed the reflected cutoff. The optional weighted-profile probe retains finite factorial marginals, all marked slots and full phase, but its continuum density model has no arithmetic transport theorem and no estimate for omitted counts. Certified Proth products are selected regressions, not a representative prime population. The joint signed boundary difference and complementary floor remain open; no zero-free frontier changes."),
    ("rieszJointBoundaryLedgerTheorem", .str "RiemannGaussian.ZetaRieszLeastOrderOverflow.jointLow_boundary_ledger"),
    ("rieszJointBoundaryComparisonTheorem", .str "RiemannGaussian.ZetaRieszLeastOrderOverflow.fullPacket_sub_joint_boundary_bound"),
    ("rieszJointBoundaryScope", .str "The literal least-slot unit endpoint is zero for N>=101 and its coincident marked/least correction has an independent geometric source-scale bound. The joint owner/least/count tilt pays the upper least-order overflow for counts 14..55 by 110*U*C_sigma*r_up^N, where U=10001/20000, sigma=1+1/262144 and r_up=U*(262144/131071)*exp(-1/6000)<1. These estimates retain the actual coefficient, full phase and support and require no zero hypothesis. Exact finite ledgers leave lowerThresholdPacket over counts 3..55 MINUS shortOverflowPacket over counts 3..13, up to independently paid errors. The first term still contains counts 14..55; only their upper-order overflow is paid. No separate norm allowance is asserted for either retained signed term. Their joint estimate and the complementary direct-carrier floor remain open. Sparse finite actual-prime probes are identity regressions, not full prime-sum estimates. No packet decay, -3/40 floor, zero exclusion or whole-carrier frontier advance is claimed."),
    ("rieszJointPrimeScope", .str "Independent bounds for literal arithmetic errors, with every factorial order, Riesz coefficient and complex phase retained. Joining share neighbors has an exact debit to the old rest. Exterior shares, removal of largest-prime ownership, old allocation and radial localization have geometric source-scale bounds. The joint owner/least factorial tilt pays the actual omega>=56 tail by 400*U*C_sigma*r^N, with U=10001/20000, sigma=1+1/1048576 and r=U*(1048576/524287)*exp(-1/9700)<1, uniformly in height and count cutoff for N>=21. The rate is close to one and the constant is not claimed small. Exact ledgers reduce the joined and wider radial packets to the signed counts 3..55 up to independently vanishing errors. That low-count sum and the complementary direct carrier still lack independent signed bounds. Synthetic finite-mode probes have no certified quadrature error and are not actual zeta-zero configurations or prime-sum estimates. No packet decay, -3/40 floor, zero exclusion or advance of the whole-carrier source frontier is claimed."),
    ("rieszShiftedCenterScope", .str "The shifted-center operator cancels the full von Mangoldt pole on each prime leg before products. For 0<=u<=10001/20000 and abs(y)>54, its complete ordinary-prime leg of order k>=2 equals the actual multiplicity-weighted negative global zero sum, with error <=M*(3/4)^(k-1). The analytic completion and both proper-prime-power terms are paid; no exposure, simplicity or rightmost hypothesis is required for this expansion. Pointwise, the operator multiplies a prime leg by 1-C^k/p. Under the stated adaptive upper-order and polynomial physical cutoffs, the product multiplier differs from one by <=exp(3/(N+1)^14)-1. This is relative closeness, not a source-normalized arithmetic packet error. The finite negative-mode count exponential and its boundary-aware inverse support are proved, including every low-order channel. A fixed-primitives passage to the global xi product fails by Gamma growth; the local finite-divisor replacement has a regular remainder, whose arbitrary analytic-factor support transfer is refuted separately. A rescaled supported insertion process is <=exp(-N/100) for N>=100000, with a positive auxiliary lower cutoff retained; it is not identified with the actual multimode inverse or arithmetic packet. The exact full binomial zero-order marginal is (1-x)^N. For every shrinking share x_N->0 and fixed u>1/2, its generic absolute source envelope (2u)^N*(1-x_N)^N diverges. Thus a concentration/union bound alone cannot pay adaptive bad allocations at source scale; this is not divergence of the signed carrier. No broad packet is constructed, no hard-share transfer is inferred from complete-leg convergence, and no packet decay, independent floor or new zero exclusion is claimed. Prior ledgers, no-go audits and public frontiers are preserved."),
    ("rieszPoleJetMaskScope", .str "The radial analytic model has a proved positive saddle exponent; numerical integration is exploratory, not a theorem that the literal packet diverges. Actual direct and admissible reflected zero denominators lie in Re(z)>=u under an explicit temporary global rightmost hypothesis. Finite convex mixtures stay outside the source disk, and a distinct-node hard-share integral has bound 2u/(norm(z1-z2)*(N+1)); the confluent case is also evaluated. The literal polynomial packet keeps every mask frozen at N and filters total moments. Its mixed-mode multiplier is P(1/sum(q*z)), not a product of individual filter evaluations. The actual pole-jet filter is nonzero at every exposed inner denominator with Re(w)>1/2. The pole is 1/2+i*Im(rho); the real mixture 0.500023125 requires a second same-real-part zero at a compensating ordinate. That zero and ordinate relation are hypotheses, not asserted facts. Resonance forces a genuinely rightward zero, with gain at least its depth. An exact synthetic two-rightward-mode family defeats every uniform squared-displacement/gain bound while retaining disk exposure and a local radius; the three/four-mode probes are not actual zeta-zero data. No existence of a rightmost zero is inferred, and no conditional literal filtered-packet estimate is proved or refuted. The joint residual, pole-containing mixtures and one-sided cutoff boundary remain unpaid. The independent signed floor and all public RH/zero-free frontiers are unchanged."),
    ("rieszParityRestAllowanceTheorem", .str "RiemannGaussian.ZetaRieszParityRestAudit.restAllowance_tendsto_atTop"),
    ("rieszZeroParityScope", .str "The simple-zero parity transform is (1-exp(-J_r))/z^2, equal to 1/(z*(w+z)) at zero cutoff. A directly verified inverse representative is the indicator d>=s; the literal core gap is greater than 289/1000. Ten omitted shares <=7/250 cannot reach support. The first nonzero insertion is an exact positive indicator, and its reciprocal-density integral with 1/11! is below 1/1000000. The entire signed 12+ supported-inverse insertion series is now below 1/1000000, uniformly in a positive auxiliary lower cutoff. Its exact nested-kernel identity and convergent signed resummation are proved. The full one-sided cutoff defect is still open: support and empty-cofactor boundary terms and the zero auxiliary cutoff passage are not paid. Order-zero atoms still preclude direct uniform replacement, but their summed factorial mass is now paid: every prime share is >=3/250, and allocations with some order <N/200 have total mass <=39 exp(-N/400). The literal badPacket has an independent geometric source-normalized bound for all moving heights/counts. Finite-prime completion now applies on [N/200,27N/40]; all good allocations have uniform count-parity prime products under the original simple exposed-zero hypotheses. This is not yet a phase-transfer theorem through the coupled prime-log/Riesz masks. An analytic-mode counterexample proves that uniform complete-leg decay for k>=N/200 alone cannot bound the hard share projection; an arbitrarily small fixed nonzero residual still has no eventual bound in that model. This is not actual prime data or a counterexample to FullParityPacket; its full Riesz/count structure may provide further cancellation. The original factorial rectangle misses <=6 exp(-N/6000) mass, and the original allocated fraction is <=39 exp(-N/1000) throughout FullParityBox. Both actual arithmetic errors decay independently at source scale for 0<=u<=10001/20000. Thus the original packet differs by o(1) from the unchanged coefficient on exactly the same finite support, retaining the full phase, squarefree/count/physical/core/share masks. No whole-packet bound or source limit is claimed. Balanced triple boxes survive untouched in fullParityRest: its existing phase-cost allowance is >=c(2u)^N/(N+1)^4 eventually and diverges for each fixed height and 1/2<u<=exp(-11/16). This does not assert divergence of the signed rest or rule out joint cancellation. Numerical Dickman/Buchstab delay probes are exploratory, not arithmetic estimates or Lean certificates. LocalizedTypeIIBound and public zero-free/source frontiers are unchanged."),
    ("rieszCascadeScope", .str "The all-count continuum Riesz kernel has the exact repository coefficient sign, a genuinely integrable iterated Laplace functional, and an absolutely convergent sum of evaluated count transforms. Its zero-cutoff transform is 1/(w*z). Positive-cutoff renewal retains the empty-cofactor boundary source. The full cutoff contrast has a nonzero first variation under a shared prime-density perturbation. A literal all-count packet in the interior largest/least-prime share box retains the old finite factorial rectangle, original allocation, full phase and every core mask, with an exact packet/rest ledger and at most 39 distinct prime factors. The standard one-prime absolute-variation Abel allowance from the current inverse-log Chebyshev envelope grows at source scale even inside 1.95N<log n<=2.03N, at every fixed u>1/2 and nonzero height. This audits that bound; it is not a lower bound for the actual prime discrepancy, a no-go theorem for all global signed methods, or a bound for the complete packet variation. This earlier continuum/Abel audit alone provides no pointwise cutoff-sensitivity estimate, count-tail bound, discrete-to-continuum o(1) transfer, packet norm <=1/1000, signed floor or zero exclusion; the later simple-zero parity scope records the independently paid factorial errors. The strict arithmetic pass criterion remains unmet; prior paid errors, LocalizedTypeIIBound and public frontiers are unchanged."),
    ("rieszParityScope", .str "The literal direct-carrier triple rectangle and five-prime packet retain all original masks, allocation and phase. Exact hinge enumeration proves the opposite arithmetic coefficient signs. Independent geometric estimates pay the full carrier tail above 2.03N, the five-prime old allocation (boundedShare<=5*exp(-N/2200)), and freezing phases in cells of half-width exp(-N/4000), for 0<=u<=10001/20000. No zero premise is used for these savings; window and allocation errors are uniform in arbitrary heights. The signed cell main term is the actual triple-minus-quintuple density times its common cosine. The current Rosser endpoint bounds overlap at this cell resolution; their subtraction cannot prove positive counts there. This is a limitation of those bounds, not a theorem excluding primes or all short-interval methods. Numerical continuous prime-density probes are exploratory and neither actual prime counts nor Lean certificates. The signed packet floor, unmatched excess and remaining original carrier are still unbounded; no restricted contradiction, new zero-free region or change to LocalizedTypeIIBound is claimed."),
    ("rieszRectangleScope", .str "The literal second-incidence rectangle 21N<=40j<=23N and N<=100(h+1)<=4N has a proved eventual reserve >=1/160 on the original dyadic schedule for 1/2<u<=10001/20000, under the original simple exposed-zero hypotheses. Reserve means minus its contribution to ownerCompletionCorrection. All integer, narrow-window, reflected-large, nondominant, physical-prime, coprimality and allocation masks remain. The allocated fraction has joint bound 3*exp(-N/3200); allocation and every literal-mask comparison error decay at source scale uniformly in arbitrary moving heights without a zero premise. The three separate prime phases and exact finite lattice density supply the signed bound; no composite cofactor or global all-order product is completed. The original correction is partitioned explicitly. The matching uniquely owned rectangle remains negative and nondecaying; the positive correction removes a duplicate incidence. The rest of the correction and other prime-count classes still require a joint signed bound. This is a concrete component estimate, not a -3/40 whole-carrier floor, restricted contradiction, new zero-free region or general impossibility theorem. The strict whole-carrier pass criterion is unmet; LocalizedTypeIIBound and public frontiers are unchanged."),
    ("rieszSkewScope", .str "Concrete quarter-gap rank-two box only: h+1<=N/25, j-l>=N/4, with the original ownerOrders, reflected-large threshold, all integer masks, coprimality, phase and old unassigned factor. The exact factorial identity and finite nonowner partition retain every correction. Joint binomial tails give (1-boundedShare)*skewMass<=3*exp(-N/8100). On the original dyadic schedule from index 32 the normalized literal sum has norm <=3*U*skewRate^N*MajorantMass(1+1/262144), with a proved skewRate<1, for 0<=u<=U=10001/20000 and arbitrary moving heights. No zero or completion premise is used. Hence this unassigned box cannot supply any fixed positive reserve. Weight-one correction rows still include their old allocated fraction; the difference tends to zero, with no separate estimate on either part. The N/5 variant and the full correction are not ruled out or bounded. No restricted zero exclusion, general rank-two impossibility result or change to LocalizedTypeIIBound is claimed."),
    ("rieszRenewalScope", .str "Boundary-renewal test using the exact reflected-large threshold log p>=log n-L. Saturation follows from the literal exactly-two-large support; the positive side is Lambda(a), the negative side is the negative truncated hinge, and the innermost layer equals v*log n. The terminating least-prime recurrence is inserted into the original pairForm with every mask, allocation, incidence weight, factorial kernel, phase and unsaturated correction retained. Prime deletion changes the original unit log cell. The density-adjusted kernel provides no strict contraction below its saddle; the subtracting branch vanishes in the inner layer. These are infrastructure and specific transfer obstructions, not a quantitative improvement to LocalizedTypeIIBound or a general impossibility theorem for signed renewal. No fixed power saving, independent signed floor or restricted zero exclusion is proved. The public source frontier and conditional Type-II target are unchanged."),
