...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ...
    ("rosserZeroMassScope", .str "Unconditionally, the complete sum of analytic zero multiplicities divided by squared ordinates is strictly below 463/10000, retaining both ordinate signs. The reciprocal norm-square sum satisfies the same classical Rosser constant. The proof evaluates the full reflected Poisson mass at one as 2+gamma-2*log(2)-log(pi), using the proved complete critical-line verification through height 54 and the absence of zeros through height 14. Summability is proved; no RH, external zero table, prime-count estimate or short-prime supply is assumed. Higher inverse-power constants, the published low-zero verification through height near 22000 and actual prime-error estimates remain open. This auxiliary literature dependency supplies no larger zero-free region or independent RH signed floor."),
