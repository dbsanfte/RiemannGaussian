...
private def milestones : Array Milestone := #[
  ...
  {
    label := "Both horizontal sides vanish and the vertical-minus-bulk scalar converges without normalization to two pi times the xi heat detector"
    lineOne := "horizontal = 0"
    lineTwo := "vertical-bulk limit"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_separatedSelectedLaplaceVerticalSubBulkHeat_im
  }
]
