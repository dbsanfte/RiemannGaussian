...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ...
    ("vkRieszScope", .str "The literal Riesz band has an exact phase-zero Fourier lift and residue partition. Signed congruencing bounds its actual weighted mixed moments; finite sampling and residue Holder costs are explicit. A constructive falling-factorial and floor mass quantifies normalization. The actual weighted mixed maximum now interpolates into the higher Riesz moment and reverse mixed maximum; the latter retains only a squared original Riesz factor. All continuity and Lp conditions are proved. Exact complex Gram correlations and the original source limit survive. No independent saving for those weighted moments, new zero-free width, or RH proof is asserted."),
