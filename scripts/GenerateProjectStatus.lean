...
private def milestones : Array Milestone := #[
  ...
  {
    label := "Normalized paired-eta L1 flux recovers exact positive zero multiplicity"
    lineOne := "paired-eta L1 flux"
    lineTwo := "2 pi multiplicity"
    role := "unconditional"
    theoremName :=
      ``RiemannGaussian.tendsto_pairedEtaNormalizedNumeratorRadialFlux
  }
]
