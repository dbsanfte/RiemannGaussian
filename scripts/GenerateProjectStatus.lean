...
private def milestones : Array Milestone := #[
  ...
  {
    label := "The decomposed near-edge nonnegative scalar converges without normalization to two pi times the xi heat detector"
    lineOne := "five-term scalar"
    lineTwo := "limit = 2pi xi heat"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_separatedSelectedLaplaceBoundaryHeatFunctional_im
  }
]
