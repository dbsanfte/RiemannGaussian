...
private def milestones : Array Milestone := #[
  ...
  {
    label := "A single diagonal sequence of paired-eta log-norm radial variations converges to the complete detector"
    lineOne := "eta log-norm flux"
    lineTwo := "limit = detector"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_pairedEtaBoundaryHeatLogNormRadialFluxDiagonal
  }
]
  ...
  let statusJson := Json.mkObj [
    ...
    ("frontier", Json.mkObj [
      ...
      ("target", .str (
        ...
        "for the heat-weighted radial variation of log|E|^2, where E is the " ++
        "convergent paired-eta series, that forces the exact diagonal detector " ++
        "limit to vanish without discarding its zero contributions"))
    ]),
