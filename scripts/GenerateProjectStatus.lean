...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ...
    ("rieszFixedCofactorScope", .str "At the original source scale, every fixed squarefree nonunit composite cofactor has a vanishing full prime-insertion band. The clipped part has an explicit inverse-order plus geometric allowance with every factorial shift retained, and the part above the cutoff is zero. For N>=60 the actual carrier splits into one-large-prime and at-least-two-large-prime classes. The one-large-prime cofactor divides 30030; its prime possibilities are 2,3,5,7,11,13, with their exact semiprime coefficient retained. The joint signed semiprime and multiple-large-prime estimate remains open, as does the assembly of the finite cofactor deletion into a reduced source theorem. The separate nonlinear allowance is now uniform on each closed half-plane Re(s)>=sigma>1/2. Completed response and boundary pass exactly through the full filter; their combined bound remains open. No new zero-free region or RH proof is claimed."),
