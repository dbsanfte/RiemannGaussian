...
private def milestones : Array Milestone := #[
  ...
  {
    label := "Literal paired-eta L1 flux has a two-stage limit equal to the complete detector"
    lineOne := "eta L1 flux windows"
    lineTwo := "limit = detector"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.exists_pairedEtaBoundaryHeatNormalizedNumeratorRadialFlux_twoStageLimit
  }
]
