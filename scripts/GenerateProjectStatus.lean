...
private def milestones : Array Milestone := #[
  ...
  {
    label := "Actual nontrivial zeros satisfy 1/450000<Re(rho)<1-1/450000 whenever 1000000<=abs(Im(rho)) and log(abs(Im(rho))+2)<=320000. Literal zeta nonvanishing includes the closed right edge. The complete all-family Gaussian cost is at most 47500 against a selected source at least 48000; the existing exact family discharges all premises. On exp(300000)<=abs(t)<=exp(310000), exact function comparisons beat denominators 4.8594, 19.62 and 51.34 in the classical, Littlewood and Vinogradov--Korobov shapes. The existing eventual component remains, and overlaps combine by maximum with all height conditions. Full literature and historical novelty review remains; RH is open."
    lineOne := "zero-free: explicit band"
    lineTwo := "width 1/450000"
    role := "unconditional"
    theoremName :=
      ``RiemannGaussian.ZetaGaussianBandExclusion.nonvanishing
  },
  {
...
private def gaussianPoleRemainderToolkit : Json :=
  Json.mkObj [
    ...
    ("nextProofTarget", .str "The gaussianPhaseBandToolkit now combines the full family and closes a rational cost surplus on an explicit height band. Next complete the literature comparison, widen the interval and transport it into the arithmetic chain; no coefficient search."),
...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ("role", .str "The complete Gaussian phase-family inequality proves an explicit literal zero-free band with a rational source surplus."),
    ...
    ("analyticParameters", .str "u=1/450000, x=u/1000>0, B=4u^2 and derivative order k=9. The center is strictly right of one and no Euler boundary limit is assumed."),
    ("rationalSurplus", .str "The genuine Gaussian source has F_B(x+u)>=152000 and F_B(x)<=199575. The full band cost is at most 47500; an actual zero in the proposed width would contribute at least 48000, including its multiplicity and favorable reserve."),
    ("actualRegion", .str "For every actual nontrivial zero rho=beta+i*t, 1000000<=abs(t) and log(abs(t)+2)<=320000 imply 1/450000<beta<1-1/450000. Literal zeta nonvanishing includes the closed right edge."),
    ("explicitHeightThreshold", .bool true),
    ("comparisonInterval", .str "All heights abs(t)=exp(L) with 300000<=L<=310000 lie in the actual band. Exact inequalities compare 1/450000 with 1/((24297/5000)L), log(L)/((981/50)L), and 1/((2567/50)L^(2/3)log(L)^(1/3))."),
    ("comparisonScope", .str "The three named comparison functions are strictly smaller throughout the matching interval. Their constants are reported in Bellotti--Trudgian--Yang 2026, including Yang thesis improvements. The thesis landing page was retrieved but its PDF returned a server error; its full parameter-dependent conclusions have not been inspected. Function comparisons are not external region axioms or an exhaustive world-record audit."),
    ("literature", .str "https://arxiv.org/html/2603.21490v1 ; https://doi.org/10.26190/unsworks/31825"),
    ("regionUnion", .str "The new band and the existing eventual Littlewood component give their maximum on overlaps, retaining both height domains. The old eventual threshold remains coefficient-dependent and unevaluated."),
    ("actualTheorem", .str "RiemannGaussian.ZetaGaussianBandExclusion.exact_strip"),
    ("nonvanishingTheorem", .str "RiemannGaussian.ZetaGaussianBandExclusion.nonvanishing"),
    ("familyTheorem", .str "RiemannGaussian.ZetaGaussianBandExclusion.family_margin"),
    ...
    ("unionTheorem", .str "RiemannGaussian.ZetaGaussianBandComparison.union_with_eventual"),
    ("nextProofTarget", .str "Complete literature comparison, widen or sharpen the explicit interval using the full all-family budget, and transport the new band into the arithmetic chain with its height windows intact."),
    ("limitations", .str "No exhaustive world-record or historical novelty claim, new RH proof, or independent fixed-ordinate ordinary-prime floor is established. The region is a band plus the existing eventual component, not all-height RH."),
    ("documentation", .str "docs/zeta-gaussian-phase-band.md"),
    ("publicTheorems", .arr #[
...
run_cmd do
  ...
  let statusJson := Json.mkObj [
    ...
    ("squarefreeLocalWindowToolkit", Json.mkObj [
      ...
      ("limitations", .str "The local window margin must be proved for the chosen center. Its explicit Gaussian-band instantiation remains the next arithmetic step. This interface alone supplies no additional zero exclusion or independent ordinary-prime floor; RH remains open."),
