...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ...
    ("fordSelectedIterationScope", .str "The actual all-endpoint moment sequence uses the proved rounded rank and maximal depth, including its final rank-k step to defect at most k-1. ShortPrimeSupply remains an explicit unproved arithmetic premise of the moment bound. The scalar recurrence, strict defect positivity and cumulative potential estimate H(d_J)<=H(d_0)-2J/k+67/(50k) are proved independently of that premise, for k>=1000 and an active prefix. The final defect exceeding k-1 implies that prefix condition. Potential endpoint evaluation, the closed coefficient bound and dense short-prime supply remain open. No new zero-free region, RH signed floor or numerical zero certificate follows."),
