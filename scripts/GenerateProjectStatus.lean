...
private def milestones : Array Milestone := #[
  ...
  {
    label := "The live detector is a divisor-aware xi log-norm boundary-minus-gradient functional"
    lineOne := "xi log-norm gradient"
    lineTwo := "divisor-aware limit"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_separatedSelectedLaplaceXiLogNormGradientScalarHeat
  }
]
