...
private def milestones : Array Milestone := #[
  ...
  {
    label := "The live xi-logarithmic-derivative frontier is a fully real heat-gradient scalar retaining the detector limit"
    lineOne := "real xi'/xi scalar"
    lineTwo := "heat-gradient form"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_separatedSelectedLaplaceXiLogDerivativeScalarHeat
  }
]
