...
private def milestones : Array Milestone := #[
  ...
  {
    label := "Xi symmetry reflects the live log-norm boundary without changing its detector limit"
    lineOne := "reflected xi boundary"
    lineTwo := "s -> 1 from below"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_separatedSelectedLaplaceXiLogNormReflectedGradientScalarHeat
  }
]
