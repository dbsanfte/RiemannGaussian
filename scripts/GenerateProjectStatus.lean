...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ...
    ("vkRieszScope", .str "The literal Riesz band has an exact phase-zero Fourier lift and residue partition. Signed congruencing bounds its actual weighted mixed moments; finite sampling and residue Holder costs are explicit. A constructive nonempty block justifies normalization. Exact complex Gram correlations and the original source limit survive. No independent saving for those weighted moments, new zero-free width, or RH proof is asserted."),
