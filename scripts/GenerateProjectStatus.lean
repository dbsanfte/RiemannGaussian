...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ...
    ("fordSelectedIterationScope", .str "For k>=1000, the actual selected defect is at most (3/8)*k^2*exp(1/2-2*(J+1)/k+1.69/k) throughout the full published upper order range. The actual all-endpoint moment inherits this bound with its recursive coefficient; ShortPrimeSupply remains an explicit unproved arithmetic premise. At omega=0.06, V^(k+1)<=W=k^(4.11*k) is proved. For j+1<=1.97*k the actual defect decreases by at least 0.01916*k, paying the entire early step coefficient by W^(Delta_j-Delta_(j+1)); later steps retain both factors. All these scalar estimates are independent of prime supply. The closed coefficient product and dense short-prime supply remain open. No new zero-free region, RH signed floor or numerical zero certificate follows."),
