...
private def milestones : Array Milestone := #[
  ...
  {
    label := "For every actual nontrivial zero rho=beta+i*t with abs(t)>=1000000, d(t)<beta<1-d(t), where L=log(abs(t)+2), C(L)=L+2052*log(L)+30240 and d(t)=min(1/450000,221/(250*C(L))). No upper height ceiling. Literal zeta nonvanishing includes the closed right edge. The preceding explicit curve is contained and the new one is strictly wider for L>320000. The complete retained cost is below the original source. The squarefree arithmetic chain inherits the larger explicit radius; the independent signed prime floor and RH remain open."
    lineOne := "zero-free: explicit curve"
    lineTwo := "no upper height ceiling"
    role := "unconditional"
    theoremName :=
      ``RiemannGaussian.ZetaGaussianRetainedRegion.nonvanishing
  },
  {
...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ("vaughanScope", .str "Classical Vaughan decomposition retains the literal von Mangoldt weights and both strict factor cutoffs. Chebyshev prime-power density and inverse-square-root weights improve the independent prefix budget to C_y*sqrt(U+1)*sqrt(V+1)*sum norm(p_k), including the full product multiplier. For every fixed complex polynomial, 0<u<1 and abs(y)>1, both cutoffs may reach floor(u^(-N)/b_N), with normalized allowance C(p,y)*(u/b_N+u^(N+1)); every positive denominator b_N tending to infinity is allowed. The canonical denominator N+1 gives floor(u^(-N)/(N+1)), eventually larger than every geometric cutoff with base r>=1 and u*r<1, including the preceding cutoff. The original finite-band error, divisor-log majorant, all product phases and shared primes are retained. The cofactor kappa_V is nonnegative, bounded by log(n), and additive on coprime products. The full negative-multiplicity source survives in the original band under each hypothetical right-half zero. Its independent cofinal signed lower bound and RH remain open. No arbitrary-coefficient Type II estimate, comparable-factor range, optimal or undamped cutoff, height-uniform constant or larger zero-free region is claimed. These use classical prime-density and partial-summation tools. The squarefree projection is now paid at the same larger cutoff: averaging every reciprocal Euler correction gives an O(sqrt(U)) marked mass at Re(s)>=1/2, and the full coprime Mobius-prime cross term has the original square-root product budget. The logarithmic projected prefix also decays independently. The entire nonsquarefree contribution to the original finite band tends to zero for every fixed polynomial, abs(y)>1 and 0<u<1, without assuming a zeta zero. The surviving squarefree band retains the full conditional negative-multiplicity source; its signed lower bound remains open. Decay controls the signed sum, not the sum of absolute values."),
    ...
    ("vaughanDocumentation", .str "docs/zeta-squarefree-vaughan-projection.md"),
    ...
    ("analyticParameters", .str "q>=1, w=1/(450000*q), x=w/1000>0, B=4*w^2 and k=9. Write L=log(abs(t)+2), C(L)=L+2052*log(L)+30240 and choose q=max(1,C(L)/397800). The strip half-width remains delta_9+x; no Euler boundary limit is assumed."),
    ("rationalSurplus", .str "Exact homogeneity retains the selected source >=48000*q. The complete all-family cost is <=36922*q+L/36+57*log(L)+840 for every abs(t)>=1000000 and q>=1, with no upper height constraint. The actual dilation bounds this by 47972*q, giving a strict surplus."),
    ("actualRegion", .str "For every actual nontrivial zero rho=beta+i*t with abs(t)>=1000000, d(t)<beta<1-d(t), where L=log(abs(t)+2), C(L)=L+2052*log(L)+30240 and d(t)=min(1/450000,221/(250*C(L))). No upper height ceiling. Literal zeta nonvanishing includes the closed right edge. The preceding explicit curve is contained and the new one is strictly wider for L>320000."),
    ("explicitHeightThreshold", .bool true),
    ("comparisonInterval", .str "L_* is the unique root on L>=250000 of (981/50)L=450000 log(L), with 288000<L_*<289000. The exact ceiling is L_max=log(exp(320000)-2), with 310000<L_max<320000. The band and headline envelope comparison hold exactly on [L_*,L_max]; strict improvement holds on (L_*,L_max]. Both signs of height and the closed right edge are covered."),
    ("comparisonScope", .str "The headline envelope, all classical denominators >=2, all Littlewood denominators >=981/50, VK denominators >=48, the MTY intermediate expression and the Ford expression for every leading constant >=1/2 are compared on the exact interval. The literature table records versions, height ranges, edges, source status and unevaluated eventual thresholds. The Yang thesis full PDF and newer independent certificate were unavailable; BTY v1 Theorem 2 has unresolved proof status. No external zero-free proof is imported and no exhaustive record claim is made."),
    ("literature", .str "https://arxiv.org/html/2603.21490v1 ; https://doi.org/10.26190/unsworks/31825"),
    ("regionUnion", .str "The improved explicit curve and existing eventual Littlewood component give their maximum on each overlap. Both height conditions remain; the eventual threshold is coefficient-dependent and unevaluated."),
    ...
    ("actualTheorem", .str "RiemannGaussian.ZetaGaussianRetainedRegion.exact_strip_min"),
    ("nonvanishingTheorem", .str "RiemannGaussian.ZetaGaussianRetainedRegion.nonvanishing"),
    ("familyTheorem", .str "RiemannGaussian.ZetaGaussianRetainedRegion.family_margin"),
    ...
    ("unionTheorem", .str "RiemannGaussian.ZetaGaussianRetainedRegion.union_with_eventual"),
    ("comparisonTheorem", .str "RiemannGaussian.ZetaGaussianBandFrontier.nonvanishing_and_comparison"),
    ...
    ("nextProofTarget", .str "Use the larger explicit region and squarefree radius to attack the independent cofinal floor of the complete signed prime-versus-integer carrier. Retain all phases, arithmetic signs, boundaries and cross terms; the negative source limit alone is not that floor."),
    ...
    ("documentation", .str "docs/zeta-gaussian-retained-region.md"),
    ("arithmeticTheorem", .str "RiemannGaussian.SquarefreeGaussianRetainedRegion.exists_response_bound"),
    ("arithmeticRadius", .str "For each abs(y)>=500002, let H=log(2*abs(y)+5), C(H)=H+2052*log(H)+30240, m(y)=min(1/450000,221/(250*C(H))). The full closed radius 1+m(y)/2 is available for the original squarefree quotient and all marked responses. The full doubled window and pole exclusions are discharged. This radius is strictly larger than the preceding one when H>320000."),
    ...
    ("publicTheorems", .arr #[
      ...
      .str "RiemannGaussian.ZetaGaussianBandFrontier.nonvanishing_and_comparison",
