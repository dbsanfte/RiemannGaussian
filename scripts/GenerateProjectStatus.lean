...
private def milestones : Array Milestone := #[
  ...
  {
    label := "Every zeta zero is a limit of finite positive-Laplace eta zeros satisfying exact Fourier cancellations"
    lineOne := "finite eta roots"
    lineTwo := "Laplace moments = 0"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.exists_pairedEtaFiniteLaplaceMomentZero_sequence_tendsto
  }
]
