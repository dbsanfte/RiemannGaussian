...
private def milestones : Array Milestone := #[
  ...
  {
    label := "A single diagonal sequence of literal paired-eta L1 fluxes converges to the complete detector"
    lineOne := "eta L1 diagonal"
    lineTwo := "limit = detector"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_pairedEtaBoundaryHeatNormalizedNumeratorRadialFluxDiagonal
  }
]
...
private def renderSvg (moduleCount declarationCount theoremCount : Nat) : String :=
  ...
  "<svg xmlns=\"http://www.w3.org/2000/svg\" role=\"img\" " ++
    ...
    "  <text class=\"frontier\" x=\"20\" y=\"220\">Inventory, not proximity meter. Open: " ++
      "global arithmetic control of normalized eta L1 flux; no current theorem implies RH.</text>\n" ++
    "</svg>\n"
