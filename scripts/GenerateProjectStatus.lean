...
private def milestones : Array Milestone := #[
  ...
  {
    label := "The holomorphic regular correction is asymptotically inert and the regular-subtracted vertical-minus-bulk scalar retains the xi heat-detector limit"
    lineOne := "regular correction = 0"
    lineTwo := "xi frontier retained"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_separatedSelectedLaplaceVerticalSubBulkHeat_sub_regularCorrection_im
  }
]
