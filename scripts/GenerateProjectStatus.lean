...
private def gaussianPhaseBandToolkit : Json :=
  Json.mkObj [
    ...
    ("rieszCentralJointTransportTheorem", .str "RiemannGaussian.ZetaRieszCentralPair.eventually_norm_refined_sub_centralJoint_le"),
    ("rieszCentralJointSourceTheorem", .str "RiemannGaussian.ZetaRieszCentralPair.tendsto_centralJoint_exposed"),
    ("rieszCentralPairLogTheorem", .str "RiemannGaussian.ZetaRieszCentralPair.centralPairResponse_eq_log_sum"),
    ("rieszCentralUnpairedDegreeTheorem", .str "RiemannGaussian.ZetaRieszCentralPair.eventually_centralUnpaired_three_primes"),
    ("rieszThreePrimeCoefficientTheorem", .str "RiemannGaussian.ZetaRieszTriplePrime.actual_three_prime_coefficient_bounds"),
    ("rieszThreePrimeMidpointTheorem", .str "RiemannGaussian.ZetaRieszTriplePrime.norm_actual_three_prime_le_midpoint_gap"),
    ("rieszCentralThreeBoundsTheorem", .str "RiemannGaussian.ZetaRieszCentralPrimeLayers.central_three_coefficient_bounds"),
    ("rieszCentralPrimeLayersTheorem", .str "RiemannGaussian.ZetaRieszCentralPrimeLayers.eventually_centralJoint_eq_prime_layers"),
    ("rieszCentralThreePhaseBoundTheorem", .str "RiemannGaussian.ZetaRieszCentralPrimeLayers.re_normalized_three_ge_negative_phase"),
    ("rieszCentralPrimeLayersScope", .str "The central transport has error at most 2E_N on 1/2<=u<exp(-2/3), with the whole completed prime head retained. For u>=1/2 and N>=20, the actual pair clip is one and the central correction has coefficient log(n). Twenty is only the clip threshold, not a starting order for all earlier transports. Nonzero unpaired survivors eventually have at least three distinct prime factors. For L>0 and log(n)<=2L, actual three-prime coefficients are nonnegative and bounded by log(n)/2 and (log(n)/L)*(2L-log(n)). The actual annulus discharges the logarithmic premise. The exact whole carrier retains the complete head, central pair correction, three-prime response and four-or-more-prime response. The known coefficient signs leave cosine oscillations intact. The source-normalized three-prime lower bound pays only negative real observations with the half-logarithm budget; this unevaluated phase cost is not a subunit source-scale floor. The whole joint floor remains open. All previous global fallback ranges remain, and no RH proof, new zero-free region, numerical zero bound or historical novelty is claimed."),
    ("rieszCofactorConvolutionTheorem", .str "RiemannGaussian.ZetaRieszHeadOrders.completedHead_eq_convolution"),
    ("rieszHeadPhysicalLengthTheorem", .str "RiemannGaussian.ZetaRieszHeadOrders.length_le_two_log_two"),
    ("rieszHeadHighConvolutionTheorem", .str "RiemannGaussian.ZetaRieszHeadOrders.norm_highConvolution_le"),
    ("rieszHeadHighBoundTheorem", .str "RiemannGaussian.ZetaRieszHeadOrders.norm_highHead_le"),
    ("rieszHeadHighDecayTheorem", .str "RiemannGaussian.ZetaRieszHeadOrders.tendsto_highHead"),
    ("rieszHeadRemainingIdentityTheorem", .str "RiemannGaussian.ZetaRieszHeadOrders.completedHead_eq_remaining_add_high"),
    ("rieszHeadOrderTransportTheorem", .str "RiemannGaussian.ZetaRieszHeadOrders.norm_centralJoint_sub_orderReduced_le"),
    ("rieszHeadOrderSourceTheorem", .str "RiemannGaussian.ZetaRieszHeadOrders.tendsto_orderReducedJoint_exposed"),
    ("rieszHeadOrdersScope", .str "For every fixed P, uniformly in y, the actual high head with 16k>=15(N+j+1) has independent bound highHeadCost(P,u)*(N+1)^2*exp(-7N/3200), on 1/2<=u<exp(-2/3) and N>=2. No zero, exposure or cancellation premise is used in this estimate. The exact convolution retains finite complex cofactor moments and all shifted complete prime orders, including order zero. The sharper physical length L_N<=2N*log(2) keeps the literal damped floor and added two. Order two is only the new estimate threshold. The remaining 16k<15(N+j+1) orders keep their complex correlations. Neither prime variable is truncated and no finite cofactor set is completed. The actual joint source remains minus the full zero multiplicity. The remaining head, central prime-pair correction, three-prime negative phases and four-or-more-prime response still require a joint cofinal real floor above -1. The whole signed floor remains open. All previous global fallback ranges remain; no RH proof, new zero-free region, numerical zero bound or historical novelty follows."),
    ("rieszAdaptiveLengthTheorem", .str "RiemannGaussian.ZetaRieszHeadAdaptive.length_le_source_log"),
    ("rieszAdaptiveHeadBoundTheorem", .str "RiemannGaussian.ZetaRieszHeadAdaptiveTransport.norm_adaptiveHead_le"),
    ("rieszPairConvolutionIdentityTheorem", .str "RiemannGaussian.ZetaRieszPrimePairConvolution.pairLogResponse_eq_convolution_sub_diagonal"),
    ("rieszPairConvolutionDiagonalTheorem", .str "RiemannGaussian.ZetaRieszPrimePairConvolution.tendsto_pairDiagonal"),
    ("rieszPairOrderReflectionTheorem", .str "RiemannGaussian.ZetaRieszPairOrders.full_sub_middle_eq_twice_high"),
    ("rieszPairOuterBoundTheorem", .str "RiemannGaussian.ZetaRieszPairOrders.norm_outerPairResponse_le"),
    ("rieszPairMiddleTransportTheorem", .str "RiemannGaussian.ZetaRieszPairOrders.tendsto_centralPair_sub_middle"),
    ("rieszPairJointTransportTheorem", .str "RiemannGaussian.ZetaRieszPairOrders.tendsto_centralJoint_sub_middleJoint"),
    ("rieszSharedPrimeFormTheorem", .str "RiemannGaussian.ZetaRieszPairOrders.middleJoint_eq_unpaired_add_form"),
    ("rieszPairMiddleSourceTheorem", .str "RiemannGaussian.ZetaRieszPairOrders.tendsto_middleJoint_exposed"),
    ("rieszPairOrdersScope", .str "For every fixed P, uniformly in y, the adaptive high head 8k>=7(N+j+1) and both outer pair ranges 8k<=M or 8k>=7M, M=N+j+1, have independent allowances C(P,u)*(N+1)^2*exp(-7N/9216), on 0<u<exp(-2/3), N>=3. No zero, exposure or cancellation premise is used in these bounds. The repeated-prime diagonal decays for every 0<u<1. Exact unordered counting retains the pair half; reflection retains both complex factors. The whole source transport is restricted to 1/2<=u<exp(-2/3), and all earlier cutoffs and corrections are discharged. The remaining head 8k<7M and middle pair M<8k<7M share a finite prime moment array with the adjacent head derivative shift; the complete prime series is not truncated. The actual unpaired three-prime and four-or-more-prime responses remain. A joint cofinal real floor above -1 remains open. These are order ranges, not fractions of arithmetic mass. Other global source ranges retain their earlier fallback regimes. No RH proof, new zero-free region or numerical zero bound follows."),
    ("rieszPrimeMiddleCompletionTheorem", .str "RiemannGaussian.ZetaRieszPrimeCompletion.eventually_norm_finite_sub_complete_le"),
    ("rieszWeightedFinitePhaseTheorem", .str "RiemannGaussian.ZetaRieszPrimeCompletionPhase.tendsto_weighted_finite"),
    ...
    ("rieszMatchedSourceTheorem", .str "RiemannGaussian.ZetaRieszMatchedMiddle.tendsto_unmatched_add_matched_exposed"),
    ("rieszMatchedMiddleScope", .str "On the original annular source interval 1/2<=u<exp(-2/3), the actual unfiltered matched block has eventual signed budget -m_rho^2/8<=Re(u^(N+1)*matchedBlock)<0. The signed bound has explicit zero and exposure premises; multiplicity is not assumed one. It is not a decay theorem or an independent whole-carrier floor. The input finite-prime completion has NO zero premise: for 0<u<exp(-2/3), both omitted prime ranges give 2Z*exp(-17N/12288) error at normalization u^k, uniformly for N<=3k,128k<=65N and all moving heights. Exact normalized atom identities retain the pair half, head successor order and k/(u*L_N), which lies in[7/12,1]. Actual reciprocal-order sums are<=5/32 and the block is nonempty for N>=256; this is ONLY the structural threshold, not the eventual phase threshold. The original P=1 source is unchanged. Exact set differences retain the complementary head and pair orders and every unpaired three-prime and higher-prime label. Only unmatchedJoint+matchedBlock retains the full -m_rho source. The remaining JOINT floor and other global source ranges remain open. No RH proof, zero-free enlargement or numerical zero bound follows."),
    ("rieszCompletionOptimalTiltTheorem", .str "RiemannGaussian.ZetaRieszPrimeCompletionRate.completionExponent_ideal_le"),
    ("rieszCompletionUniqueTiltTheorem", .str "RiemannGaussian.ZetaRieszPrimeCompletionRate.completionExponent_ideal_lt"),
    ("rieszWiderPrimeCompletionTheorem", .str "RiemannGaussian.ZetaRieszPrimeCompletionRate.eventually_norm_wider_completion"),
    ("rieszWiderFinitePhaseTheorem", .str "RiemannGaussian.ZetaRieszPrimeCompletionRate.tendsto_wider_weighted_finite"),
    ("rieszWiderInclusionTheorem", .str "RiemannGaussian.ZetaRieszWiderMatched.old_matchedOrders_subset"),
    ("rieszWiderStrictInclusionTheorem", .str "RiemannGaussian.ZetaRieszWiderMatched.wider_order_not_old"),
    ("rieszWiderPersistentBoundTheorem", .str "RiemannGaussian.ZetaRieszWiderMatched.eventually_matchedBlock_persistent_bounds"),
    ("rieszWiderNondecayTheorem", .str "RiemannGaussian.ZetaRieszWiderMatched.not_tendsto_matchedBlock_zero"),
    ("rieszWiderSourceGuardTheorem", .str "RiemannGaussian.ZetaRieszWiderMatched.not_tendsto_unmatched_full_source"),
    ("rieszWiderComplementTheorem", .str "RiemannGaussian.ZetaRieszWiderMatched.middleJoint_one_eq_unmatched_add_matched"),
    ("rieszWiderSourceTheorem", .str "RiemannGaussian.ZetaRieszWiderMatched.tendsto_unmatched_add_matched_exposed"),
    ("rieszEndpointTaperIdentityTheorem", .str "RiemannGaussian.ZetaRieszEndpointTaper.reflected_sharedAtom_eq"),
    ("rieszEndpointTaperBoundTheorem", .str "RiemannGaussian.ZetaRieszEndpointTaper.norm_actual_taperedMoment_le"),
    ("rieszCompletionProductTheorem", .str "RiemannGaussian.ZetaRieszCompletionProduct.eventually_norm_complete_completion_product"),
    ("rieszReflectedErrorBoundTheorem", .str "RiemannGaussian.ZetaRieszReflectedCompletion.eventually_norm_wingError"),
    ("rieszReflectedErrorDecayTheorem", .str "RiemannGaussian.ZetaRieszReflectedCompletion.tendsto_wingError"),
    ("rieszCentralSubsetBoundTheorem", .str "RiemannGaussian.ZetaRieszReflectedCarrier.eventually_centralBlock_re_bounds"),
    ("rieszLowPrefixSumTheorem", .str "RiemannGaussian.ZetaRieszLowHeadPrefix.sum_norm_weighted_smallPrimeMoment"),
    ("rieszLowHeadCompletionTheorem", .str "RiemannGaussian.ZetaRieszLowHeadCorrection.tendsto_finite_headArray_sub_complete_exposed"),
    ("rieszCompleteHeadOrderTheorem", .str "RiemannGaussian.ZetaRieszCompletedCarrier.completeHeadOrders_eq_range"),
    ("rieszCompletedCarrierSourceTheorem", .str "RiemannGaussian.ZetaRieszCompletedCarrier.tendsto_completedCarrier_exposed"),
    ("harmonicProductContinuityTheorem", .str "RiemannGaussian.HarmonicProductContinuity.tendsto_harmonic_product_error"),
    ("rieszCompleteHeadCenteredTheorem", .str "RiemannGaussian.ZetaRieszCompleteHeadHarmonic.tendsto_completeHead_centered"),
    ("rieszCompleteHeadBoundTheorem", .str "RiemannGaussian.ZetaRieszCompleteHeadHarmonic.eventually_completeHead_re_bounds"),
    ("rieszThreeHigherTaperSourceTheorem", .str "RiemannGaussian.ZetaRieszCompleteHeadHarmonic.tendsto_three_higher_taper_budget"),
    ("rieszCompleteHeadHarmonicScope", .str "On the original exposed-zero annular range 1/2<=u<exp(-2/3), P=1 and unrestricted multiplicity m, both reflected completion products and both low-head completion corrections vanish with their full original weights. The reflected summed error is independently <=8exp(2)Z1025^2(N+1)^4exp(-5N/4096), uniformly in moving height. The low small-prime derivative sum is independently <=600sqrt(N+1)Z1025; its product decay uses the explicit exposed-zero complete-leg bound. The disjoint central complement has source-conditioned real budget[-m^2/8,0]. Complete head orders are exactly range(floor((15N+64)/32)+1) forN>=256, a structural threshold only. The Mathlib-only harmonic-product theorem applies to every convergent complex array and every moving lower-half order selection, with the harmonic kernel fixed. It yields u^(N+1)H_N+m^2h_N->0 for the actual entire negative complete head, with explicit nonnegative h_N=(N+1)/(uL_N)*sum(1/(N+1-k)). The head itself does not vanish. The full source is u^(N+1)(T3+T>=4+taperedWing+centralBlock)-m^2h_N -> -m. All prior masks, exact floor, derivative shifts, phases, source premises and costs remain. The joint signed lower bound for T3+T>=4+taperedWing and the other global source ranges remain open; no RH or zero-free enlargement is claimed."),
    ("rieszExactLengthTheorem", .str "RiemannGaussian.ZetaRieszLengthAsymptotic.tendsto_length_div_succ"),
    ("harmonicIntervalLimitTheorem", .str "RiemannGaussian.HarmonicIntervalLimit.tendsto_harmonic_difference"),
    ("rieszExactHeadCostTheorem", .str "RiemannGaussian.ZetaRieszHeadHarmonicAsymptotic.tendsto_completeHead_exact_cost"),
    ("rieszExactHeadBoundTheorem", .str "RiemannGaussian.ZetaRieszHeadHarmonicAsymptotic.eventually_completeHead_exact_bounds"),
    ("finiteWeightedUniformLimitTheorem", .str "RiemannGaussian.FiniteWeightedUniformConvergence.tendsto_nonneg_weighted_sum"),
    ("rieszCentralIntervalTheorem", .str "RiemannGaussian.ZetaRieszCentralHarmonicCost.centralOrders_eq_Icc"),
    ("rieszCentralHarmonicMassTheorem", .str "RiemannGaussian.ZetaRieszCentralHarmonicCost.tendsto_central_reciprocal"),
    ("rieszCentralUniformProductTheorem", .str "RiemannGaussian.ZetaRieszCentralProductPhase.eventually_central_product_errors"),
    ("rieszCentralExactCostTheorem", .str "RiemannGaussian.ZetaRieszCentralHarmonicCost.tendsto_centralBlock_exact_cost"),
    ("rieszHarmonicCostStrictTheorem", .str "RiemannGaussian.RieszHarmonicCostBounds.paidHarmonicCost_lt_one"),
    ("rieszPaidComponentsLimitTheorem", .str "RiemannGaussian.ZetaRieszCentralHarmonicCost.tendsto_head_add_central_exact_cost"),
    ("rieszPaidComponentsStrictBudgetTheorem", .str "RiemannGaussian.ZetaRieszCentralHarmonicCost.eventually_head_central_gt_neg_multiplicity_square"),
    ("rieszThreeUnpaidExactSourceTheorem", .str "RiemannGaussian.ZetaRieszCentralHarmonicCost.tendsto_three_unpaid_exact_source"),
    ("rieszExactHarmonicCostsScope", .str "On the original exposed-zero annulus 1/2<=u<exp(-2/3), P=1 and unrestricted multiplicity m, the entire complete head and actual symmetric central block have exact limits -m^2log(32/17)/(-2u logu) and m^2log(17/15)(1-1/(-2u logu)). The exact head component alone permits u<=3/5; the full source range is unchanged. The independent floor-defined length limit and moving harmonic-interval theorem retain all integer endpoints. The exact central set is [floor((15N+64)/32)+1,N-floor((15N+64)/32)] forN>=256, a structural threshold only. Both central prime-product errors vanish uniformly at arbitrary precision under the explicit exposed-zero premises. A Mathlib-only finite-weight theorem transports uniform complex phases through every nonnegative weight family with convergent total mass; its companion covers complex weights with bounded total variation. Their combined cost is -m^2c(u), c(u)=log(32/15)/(-2u logu)-log(17/15)<1. Thus the two actual components eventually have real value greater than -m^2. The source is linear in m, so this is not a sub-source bound for unrestricted multiplicity and simplicity is not assumed. Exactly three unpaid components remain: T3+T>=4+taperedWing, with joint source -m+m^2c(u). All prior squarefree, physical, cofactor, prime and central masks, derivative shifts and complex phases remain. The independent joint signed lower bound and other global source ranges remain open; no RH, new zero-free region or numerical zero bound follows."),
    ("rieszOddPrimeFrequencyTheorem", .str "RiemannGaussian.ZetaRieszSignedFrequency.norm_primePair_le_of_odd"),
    ("rieszThreeSineProductTheorem", .str "RiemannGaussian.ZetaRieszSignedFrequency.primePair_three_eq_sine_product"),
    ("rieszHigherPrimeFrequencyTheorem", .str "RiemannGaussian.ZetaRieszSignedFrequency.norm_higher_prime_quotient_le"),
    ("rieszNonlinearFrequencyIdentityTheorem", .str "RiemannGaussian.ZetaRieszSignedFrequency.nonlinearResponse_eq_integral"),
    ("rieszNonlinearLowFrequencyTheorem", .str "RiemannGaussian.ZetaRieszSignedFrequency.norm_nonlinear_low_frequency_le"),
    ("rieszNonlinearHighRemainderTheorem", .str "RiemannGaussian.ZetaRieszSignedFrequency.norm_nonlinear_sub_high_frequency_le"),
    ("rieszNaturalFrequencyWindowTheorem", .str "RiemannGaussian.ZetaRieszSignedFrequency.natural_frequency_window"),
    ("rieszUpperTiltClassificationTheorem", .str "RiemannGaussian.LogarithmicDeviation.exists_upper_tilt_iff"),
    ("rieszLowerTiltClassificationTheorem", .str "RiemannGaussian.LogarithmicDeviation.exists_lower_tilt_iff"),
    ("rieszUniformDeviationBoundTheorem", .str "RiemannGaussian.ZetaArithmeticDeviationBounds.exists_uniform_deviation_bound"),
    ("rieszGlobalDeviationSourceTheorem", .str "RiemannGaussian.ZetaRieszDeviationCarrier.tendsto_universal_physicalDeviation_exposed"),
    ("rieszMovingDeviationErrorTheorem", .str "RiemannGaussian.ZetaRieszDeviationCarrier.tendsto_universal_annulus_error_moving_radius"),
    ("rieszSignedFrequencyScope", .str "Every prime supplies a vanishing signed Fourier factor; odd prime count supplies one more after opposite frequencies are coupled at L-log(n)/2. The three-prime response is exactly a four-sine product. On |xi|log(n)<=1, all counts at least four also have a quadratic paired quotient allowance. Both actual nonlinear arithmetic classes retain all earlier integer masks, the fixed polynomial filter, physical floor and u^(N+1) normalization. Their joint low-frequency integral has bound u^(N+1)d^3(C3+C4)/(3pi); d=3/(8(N+1)) works at everyN. C3 and C4 explicitly retain the growing prime-log and factorial costs. This is not source-scale decay or a bound for the full tail: the cost, complementary coupled high-frequency integral and tapered wing remain unpaid. These inequalities require no zero hypothesis. The separate source decomposition retains its exposed-zero premises, unrestricted multiplicity and original local range. Independently, the exact deviation profile classifies all positive summable cutoff tilts and gives one uniform removed-tail bound across0<=u<=1, including moving radii/heights. The .45N<log(n)<=5.5N window intersects the old masks and exact X_N<n; it carries the full source for every exposed right-half zero without an extra local-u hypothesis. It does not widen the separate three-unpaid decomposition or classify signed methods. The independent whole signed bound, RH and a new zero-free conclusion remain open."),
    ("rieszFrequencyCostGrowthTheorem", .str "RiemannGaussian.ZetaRieszFrequencyDecay.nonlinearFrequencyCost_le"),
    ("rieszFrequencyBandwidthFamilyTheorem", .str "RiemannGaussian.ZetaRieszFrequencyDecay.exists_frequency_tilt"),
    ("rieszFrequencyAllowanceDecayTheorem", .str "RiemannGaussian.ZetaRieszFrequencyDecay.tendsto_frequency_allowance_moving"),
    ("rieszFrequencySectorUniformTheorem", .str "RiemannGaussian.ZetaRieszFrequencyDecay.norm_nonlinear_sub_high_frequency_universal"),
    ("rieszFrequencyMovingDeletionTheorem", .str "RiemannGaussian.ZetaRieszFrequencyDecay.tendsto_nonlinear_sub_high_frequency_moving"),
    ("rieszHighFrequencyWingSourceTheorem", .str "RiemannGaussian.ZetaRieszFrequencyDecay.tendsto_high_frequency_add_wing_source"),
    ("rieszThreeExactMarginsTheorem", .str "RiemannGaussian.TripleRieszProfile.neg_riesz_ordered_three_primes_eq_min_margins"),
    ("rieszThreePrimeEndpointTheorem", .str "RiemannGaussian.ZetaRieszPrimeEndpoint.actual_three_coefficient_le_prime_taper"),
    ("rieszThreeCanonicalEndpointFloorTheorem", .str "RiemannGaussian.ZetaRieszPrimeEndpoint.re_normalized_three_ge_largest_prime_taper"),
    ("rieszPrimeAverageLossTheorem", .str "RiemannGaussian.ZetaRieszPrimeEndpoint.average_prime_taper_no_improvement"),
    ("rieszTripleEndpointChamberTheorem", .str "RiemannGaussian.ZetaRieszPrimeEndpoint.riesz_three_primes_eq_neg_prime_gap"),
    ("rieszFrequencyDecayScope", .str "The actual three-prime and four-or-more-prime frequency costs now have an independent uniform bound 2048(N+1)^5 q^(-N) times the complete fixed-filter mass at exponent 3/2-q>1. For every positive U,r with r<=1 and 2Ur^3<1, a summable tilt q>Ur^3 makes the entire low-frequency allowance decay on d_N=3r^N/(8(N+1)). This works for arbitrary moving radii in [0,U] and arbitrary moving heights, with no zero or exposure premise. In particular, r=3/4, q=7/16 and U=1 give the explicit error C(P)(N+1)^2(27/28)^N. The interval shrinks exponentially; the larger natural 1/N sector is not proved to vanish. The retained complementary integral and tapered wing have the same source -m+m^2c(u), with full multiplicity and original exposed-zero range 1/2<=u<exp(-2/3). No independent joint bound for these terms is proved. Separately, every actual three-prime coefficient in that original annulus has the largest-prime endpoint taper and an exact ordered four-margin profile. Its full negative-phase cost is unpaid. The one-small-pair chamber has exactly the same endpoint factor as the wing, but cancellation between the two is not established. Averaging prime logarithms loses the new endpoint improvement. Original masks, physical floors, polynomial filter shifts and phases remain. No RH, new zero-free result or historical novelty is claimed; the default whole-carrier endpoint is unchanged."),
    ("rieszPrimeCountMeanTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountFrequency.prime_log_product_le_mean"),
    ("rieszPrimeCountWindowTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountFrequency.norm_actual_higher_low_frequency_le_count"),
    ("rieszPrimeCountPowerTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountFrequency.norm_higher_quotient_le_count_power"),
    ("rieszPrimeCountArithmeticTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountFrequency.norm_manyPrimeLowResponse_le"),
    ("rieszPrimeCountSavingTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountFrequency.dyadic_count_power_saving"),
    ("rieszPrimeCountUniformTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountFrequency.norm_manyPrimeLowResponse_dyadic_le"),
    ("rieszPrimeCountFamilyTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountFrequency.exists_many_prime_tilt"),
    ("rieszPrimeCountDecayTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountFrequency.tendsto_manyPrimeLowResponse_dyadic_moving"),
    ("rieszPrimeCountPartitionTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountFrequency.nonlinearResponse_sub_many_low_eq"),
    ("rieszPrimeCountReducedSourceTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountFrequency.tendsto_count_reduced_source"),
    ("rieszPrimeCountFrequencyScope", .str "The full prime-count product bound saves k^k on the natural frequency window, where the earlier estimate discarded this denominator. Every fixed-filter many-prime integral, restricted only by the original masks and k>=K>=4, has normalized bound C(P,U,q)(N+1)^2(U/q)^N/K^K for 0<=u<=U and 0<q<1/2; the positive integer tilt mass is genuinely summable. On K_j=2^(j+3), N_j=8(j+4)K_j, the count saving is at least (17/16)^N_j, giving strict geometric decay for every 0<U<17/32 and all arbitrary moving radii/heights. N_j tends to infinity and the window stays at its natural 3/(8(N_j+1)) scale. Only this many-prime sector is paid: every 3<=k<K_j count retains all frequencies, k>=K_j retains frequencies above the cutoff, and the tapered wing remains. Their exact combined source is -m+m^2c(u) under the original exposed-zero assumptions and 1/2<=u<exp(-2/3); multiplicity is unrestricted. No independent joint lower bound or zero-free conclusion is proved. The radius ceiling belongs to a component estimate, not a new source range or zero-free strip. The previous all-count shrinking-sector result remains valid, but the two overlapping deletions are not asserted to add without a further partition. The count-sensitive k^4 estimate separately permits a wider individual window. The AM-GM step is standard mathematics; no historical novelty or optimality is claimed. Default whole-carrier, zero-free and numerical-certificate endpoints are unchanged."),
    ("rieszCountEulerMomentTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountMass.squarefree_count_mass_le_exp"),
    ("rieszCountMassTailTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountMass.many_prime_mass_le"),
    ("rieszManyIntegerBoundTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountMass.norm_normalized_many_sum_le"),
    ("rieszCountCompensatorTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountMass.eventually_exp_count_le_geometric"),
    ("rieszManyIntegerDecayTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountMass.tendsto_normalized_many_sum"),
    ("rieszManyActualDecayTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountMass.tendsto_manyPrimeResponse_moving"),
    ("rieszManyIntegralIdentityTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountMass.manyPrimeResponse_eq_integral"),
    ("rieszFewWingSourceTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountMass.tendsto_few_add_wing_source"),
    ("rieszCountWidePrefixBoundTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountWindow.norm_manyPrimePrefix_le_square_count"),
    ("rieszCountWidePrefixDecayTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountWindow.tendsto_wide_prefix_moving"),
    ("rieszCountFrequencyUnionTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountWindow.tendsto_two_cutoff_source"),
    ("rieszWholePrimeCountScope", .str "The complete original signed integer contribution with k>=K_j now independently tends to zero, with K_j=2^(j+3) and N_j=8(j+4)K_j. A count-weighted finite Euler product bounds every changing squarefree selection by exp(2rM(sigma))/r^K, keeping all2^k divisor choices; M(sigma) is genuinely summable for sigma>1. With a fixed summable tilt0<q<1/2, the original factorial filter and physical coefficient give a full source-normalized bound C(P,U,q)N_j(16U/(17q))^N_j exp(2K_jM(3/2-q)). The last exponential is subexponential in N_j and is absorbed into a strict geometric rate for every fixed0<U<17/32. Radii, heights, positive physical lengths and finite arithmetic selections may move arbitrarily; the actual central-band corollary discharges their support conditions. This covers all frequencies, not just the former low-frequency sector. The exact remaining source is the full3<=k<K_j signed integral plus the tapered wing, tending to -m+m^2c(u) under the original exposed-zero assumptions and u<exp(-2/3), with unrestricted multiplicity. The independent joint bound for these two objects is open. The count threshold grows; no fixed-count class or proportion of the carrier is claimed paid. Separately, the square-count estimate pays every prefix up to the square root of its threshold times the natural window, for fixed radius ceilings below33/64; exact overlap subtraction combines it with the prior shrinking all-count sector. These estimates preserve the richer frequency data alongside whole-integer decay. Neither radius ceiling is a zero-free region. No RH, new zero-free theorem or historical novelty is claimed. Default whole-carrier, zero-free and optional numerical-certificate endpoints remain unchanged."),
    ("rieszRightCountRateTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountRightHalf.eventually_count_power_saving"),
    ("rieszRightCountDecayTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountRightHalf.tendsto_many_sum_right_half"),
    ("rieszRightCountActualDecayTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountRightHalf.tendsto_manyCountBand"),
    ("rieszRightCountSourceTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountRightHalf.tendsto_few_optimized_source"),
    ("rieszSpernerWindowTheorem", .str "RiemannGaussian.ZetaRieszSperner.divisorWindow_card_le"),
    ("rieszSpernerFactorizationTheorem", .str "RiemannGaussian.ZetaRieszSperner.exists_two_smallest_factorization"),
    ("rieszSpernerCoefficientTheorem", .str "RiemannGaussian.ZetaRieszSperner.norm_coefficient_le_allowance"),
    ("rieszSpernerFullCarrierTheorem", .str "RiemannGaussian.ZetaRieszSperner.norm_original_band_le_middle_layers"),
    ("rieszSpernerCentralTheorem", .str "RiemannGaussian.ZetaRieszSperner.norm_actual_central_coefficient_le"),
    ("rieszSpernerGainTheorem", .str "RiemannGaussian.ZetaRieszSperner.norm_actual_central_coefficient_le_divisor_saving"),
    ("rieszPrimeCountSpernerScope", .str "Two independent arithmetic savings strengthen the original carrier. On K_j=2^(j+3), M_j=(j+4)K_j, every geometric rate A<2 is eventually paid by K_j^K_j. The complete high-count integer contribution tends to zero for all moving radii in each fixed [0,U] with 0<U<1, all moving heights and positive lengths, arbitrary fixed polynomial filters and changing original squarefree subbands. The full Euler mass is genuinely summable and its exp(C K_j) cost is subexponential in M_j. The shorter moment schedule deletes a smaller class at the same order than the earlier schedule; no fixed-count class or proportion is asserted negligible. Subtracting only its intersection with the earlier optimized cofactor complement leaves the exact source -m for every right-half zero, without exposure or simplicity assumptions. Separately, two prime differences and Sperner give |c_L(n)| <= (log n)^2/(kL) times choose(k-2,floor((k-2)/2)) for every squarefree composite. The full original finite band keeps the complete factorial filter. On the actual central band the bound saves at least 2k compared with the coarse 2^k log n allowance. Existing sharper three-prime bounds remain available. This is cancellation inside each coefficient, not a proof of cancellation across integers. The joint independent bound for the remaining lower-count arithmetic sum, a new zero-free conclusion from this path, and RH remain open. The narrower central-plus-wing source is separate; no wing decay or new source range for that decomposition is claimed. No historical novelty is asserted for the standard Sperner and exponential-moment arguments. Default whole-carrier, proved zero-free and numerical-certificate endpoints remain unchanged."),
    ("rieszSpernerRootGainTheorem", .str "RiemannGaussian.ZetaRieszSpernerRate.norm_actual_central_coefficient_le_root_saving"),
    ("rieszWindowTriangleTheorem", .str "RiemannGaussian.ZetaRieszWindowGram.integral_norm_sq_signed_divisor_windows"),
    ("rieszWindowCutoffTheorem", .str "RiemannGaussian.ZetaRieszWindowGram.norm_sum_bandWeight_two_primes_sq_le_gram"),
    ("rieszWindowAllFamiliesTheorem", .str "RiemannGaussian.ZetaRieszWholeWindow.sum_bandWeight_eq_integral_primeWindowLift"),
    ("rieszWindowFullEnergyTheorem", .str "RiemannGaussian.ZetaRieszWholeWindow.integral_primeWindowLift_norm_sq"),
    ("rieszWindowWholeCarrierTheorem", .str "RiemannGaussian.ZetaRieszWholeWindow.exists_original_band_primeWindowGram"),
    ("rieszWindowUnitZeroTheorem", .str "RiemannGaussian.ZetaRieszWindowUnitDeletion.exists_central_higher_unit_window_zero"),
    ("rieszWindowUnitDeletionTheorem", .str "RiemannGaussian.ZetaRieszWindowUnitDeletion.primeWindowLift_eq_withoutUnit"),
    ("rieszWindowGramScope", .str "The full original finite Riesz carrier equals one complex interval integral across every prime-pair family. Exact rescaling keeps each prime logarithm, both physical window endpoints, all Mobius divisors, the original support and the complete polynomial factorial filter. Its norm squared is bounded by one explicit signed Gram energy, retaining cross terms between all original integers and prime pairs. All support, prime-selection, integrability and square-integrability conditions are discharged; only zero original summands are deleted. The entire two-smallest-prime factorization certificate remains available. Fixed-prime triangular and cutoff-preserving kernels remain upstream. Separately, exact binomial recurrences give the actual central coefficient allowance 2^k log(n)/(2k sqrt(k-1)). This saves 2k sqrt(k-1) relative to the coarse Boolean allowance but does not improve every previous atom-specific bound. For each actual central atom with at least four prime factors, prime ordering and log(n)<2L prove the unit cofactor-divisor window identically zero on the integration domain. This is not deletion of the separate Euler prime-head term. The central savings retain u<exp(-2/3); the whole original carrier identity has no exposure, simplicity or such central-radius restriction. An independent source-scale bound for the remaining complete signed Gram energy, a new zero-free conclusion from this arithmetic path, and RH remain open. Standard finite-interval identities, Cauchy--Schwarz and binomial estimates carry no historical novelty claim. No external divisor-moment theorem is imported. Default whole-carrier, proved zero-free and numerical-certificate endpoints remain unchanged."),
    ("rieszGcdGramTheorem", .str "RiemannGaussian.ZetaRieszWindowGcd.primeWindowGram_entry_reduce_gcd"),
    ("rieszNegativeOverlapTheorem", .str "RiemannGaussian.ZetaRieszWindowInteraction.negative_overlap_reduced_structure"),
    ("rieszCrossFamilyPrimeTheorem", .str "RiemannGaussian.ZetaRieszCrossFamilyWindow.actual_chain_overlap_requires_new_small_prime"),
    ("rieszPrimeReplacementIdentityTheorem", .str "RiemannGaussian.ZetaRieszPrimeReplacement.riesz_prime_replacement_identity"),
    ("rieszPrimeReplacementGapTheorem", .str "RiemannGaussian.ZetaRieszPrimeReplacement.norm_bandWeight_replacement_pair_le_integer_gap"),
    ("rieszReplacementPhaseTheorem", .str "RiemannGaussian.ZetaRieszReplacementPhase.norm_bandWeight_replacement_pair_le_phase"),
    ("rieszReplacementPartitionTheorem", .str "RiemannGaussian.ZetaRieszReplacementFamily.actual_band_eq_replacement_add_remainder"),
    ("rieszReplacementSavingTheorem", .str "RiemannGaussian.ZetaRieszReplacementFamily.norm_actual_band_le_mass_sub_saving"),
    ("rieszPrimeReplacementScope", .str "The complete original finite carrier has an absolute-mass bound minus an explicit nonnegative saving from disjoint prime-to-product matches. Each arbitrary prime triple selects all cofactors passing literal band, coprimality and saturation tests; prime divisibility and injectivity prevent double counting. An exact four-integer Riesz identity reduces each eligible pair to a logarithmic cutoff difference. The complete original amplitude, polynomial factorial filter and height are bounded, giving a relative integer-gap cost. A second proved cost retains the exact sine of half the relative phase, with the common cofactor cancelled and the smooth allowance on the real spectral line. Each pair keeps the better cost. The exact complex partition and unmatched remainder remain upstream. No uniform positivity or source-scale lower bound on the saving is proved, and this is not claimed to improve every previous whole-carrier estimate. The whole signed source-scale bound, a new zero-free region from this arithmetic path, and RH remain open. Supporting Gram identities retain common-prime cutoff displacement; fixed-prime-pair negative overlaps need at least three nonshared primes. Comparable cross-family overlaps with equal first prime require a new smaller prime. These restrictions do not cover all varying-first-prime correlations, and negative Mobius sign alone does not imply negative complex energy. No special coefficient family, numerical certificate, external analytic assumption or historical novelty claim is introduced. Default whole-carrier, zero-free and numerical-certificate endpoints remain unchanged."),
    ("rieszCenteredCutoffTheorem", .str "RiemannGaussian.ZetaRieszCenteredCofactor.riesz_cutoff_lipschitz_centered"),
    ("rieszCenteredTentTheorem", .str "RiemannGaussian.ZetaRieszCenteredCofactor.abs_riesz_two_primes_centered"),
    ("rieszCenteredReplacementTheorem", .str "RiemannGaussian.ZetaRieszCenteredCofactor.norm_bandWeight_replacement_pair_centered"),
    ("rieszOppositeCosineTheorem", .str "RiemannGaussian.ZetaRieszOppositePrimes.norm_unitPhase_add_eq"),
    ("rieszCenteredPhaseTheorem", .str "RiemannGaussian.ZetaRieszCenteredPhase.norm_bandWeight_prime_pair_le_half_phaseCost"),
    ("rieszPairMatchingPartitionTheorem", .str "RiemannGaussian.ZetaRieszPairMatching.sum_eq_pairs_add_remainder"),
    ("rieszPrimeMatchingCostTheorem", .str "RiemannGaussian.ZetaRieszPrimeMatching.primePairCost_bound"),
    ("rieszOptimalPrimeMatchingTheorem", .str "RiemannGaussian.ZetaRieszPrimeMatching.exists_actual_band_optimal_prime_pairs"),
    ("rieszCenteredCancellationScope", .str "Every eligible squarefree nonunit cofactor now pays half the complete opposite-phase prime-pair cost. Centering uses its actual zero signed Mobius divisor mass, including both physical cutoff edges. The complete polynomial amplitude and its real-line derivative are bounded; the relative cosine phase remains exact. The original finite band considers every pair whose actual gcd is squarefree and nonunit, with distinct prime quotients not dividing that gcd. The whole carrier is bounded by absolute mass minus an attained maximum saving over all disjoint candidate matchings, with every pair-cost premise discharged. The exact complex pair/remainder partition remains upstream. Prime-to-product profile estimates also halve, but are not included in this prime-insertion candidate graph. No uniform positive saving, pair coverage, or source-scale control of the unmatched remainder is proved. A finite relative saving need not defeat the surviving exponential allowance. A new zero-free region from this path and RH remain open. No numerical certificate, special coefficient family, external hypothesis or historical novelty claim is introduced. Default frontier, zero-free and numerical-certificate endpoints remain unchanged."),
    ("rieszTentSlopeTheorem", .str "RiemannGaussian.ZetaRieszTentSlope.primePairTent_lipschitz"),
    ("rieszQuarterCutoffTheorem", .str "RiemannGaussian.ZetaRieszTentSlope.riesz_cutoff_lipschitz_quarter"),
    ("rieszQuarterPhaseTheorem", .str "RiemannGaussian.ZetaRieszTentSlope.norm_bandWeight_prime_pair_le_quarter_phaseCost"),
    ("rieszDescendingPhaseTheorem", .str "RiemannGaussian.ZetaRieszDescendingAmplitude.norm_logAmplitude_add_le_endpoint_phase"),
    ("rieszEventualDescendingTheorem", .str "RiemannGaussian.ZetaRieszDescendingAmplitude.eventually_norm_logAmplitude_pair_above_length"),
    ("rieszDescendingPairTheorem", .str "RiemannGaussian.ZetaRieszSharpPair.norm_bandWeight_pair_le_descending"),
    ("rieszSharpPairCostTheorem", .str "RiemannGaussian.ZetaRieszSharpMatching.sharpPairCost_bound"),
    ("rieszSharpPrimeMatchingTheorem", .str "RiemannGaussian.ZetaRieszSharpMatching.exists_actual_band_sharp_prime_pairs"),
    ("rieszSharpMatchingScope", .str "The coupled two-prime tent is one-Lipschitz, quartering the full profile allowance for every composite squarefree cofactor; prime cofactors retain the half allowance. Exact factorial-coefficient amplitudes and their real-line derivatives are bounded at each local pair endpoint beyond the actual stationary points, retaining the relative cosine phase. For any fixed polynomial and 0 < u < exp(-1/2), the stationary bound applies above the moving cutoff eventually and uniformly in height. Each actual original candidate uses the smaller proved cost when eligible and a verified fallback otherwise. No additional analytical premise enters the whole-carrier theorem. The whole original carrier is bounded by absolute mass minus the attained maximum disjoint saving, with the exact complex pair/remainder partition retained upstream. No pair coverage, positive aggregate saving, or sufficient source-scale bound is proved. Fixed finite percentage savings do not establish an asymptotic result. A new zero-free region from this path and RH remain open. Default endpoints and numerical certificates are unchanged; no historical novelty claim is made."),
    ("rieszMassTransportPartitionTheorem", .str "RiemannGaussian.ZetaRieszMassTransport.sum_eq_sent_add_remaining"),
    ("rieszMassTransportRayTheorem", .str "RiemannGaussian.ZetaRieszTransportPhase.ray_remainder"),
    ("rieszMassTransportSignTheorem", .str "RiemannGaussian.ZetaRieszTransportPhase.pairCost_bandWeight_one"),
    ("rieszMassTransportDiagonalTheorem", .str "RiemannGaussian.ZetaRieszTransportCoverage.remaining_mass_sq_le_diagonal"),
    ("rieszMassTransportDecayTheorem", .str "RiemannGaussian.ZetaRieszTransportCoverage.tendsto_actual_remaining_mass"),
    ("rieszMassTransportBoundTheorem", .str "RiemannGaussian.ZetaRieszTransportSource.actual_band_le_completed_prime_transport"),
    ("rieszMassTransportErrorTheorem", .str "RiemannGaussian.ZetaRieszTransportSource.tendsto_actual_band_sub_sent"),
    ("rieszMassTransportSourceTheorem", .str "RiemannGaussian.ZetaRieszTransportSource.tendsto_actual_sent_source"),
    ("rieszMassTransportScope", .str "Equal-mass transport preserves every complex ray and lets a remaining amplitude participate in later pairs without double-counting. For the actual constant filter, equal arithmetic signs pay the cosine chord and opposite signs pay the sine chord. The full-polynomial direction identity remains available. After a complete fallback visits every distinct original pair, at most one nonzero amplitude remains; the actual diagonal-energy theorem pays its complete mass. The final source-normalized unsent mass tends to zero for each fixed 0 < u < 1, every fixed full polynomial, and arbitrary height sequences. All fallback chord costs remain in the whole bound: this is not free coverage or a source-scale bound for those costs. The fully specified transported complex sum differs from the original carrier by a vanishing normalized error and retains the full negative-multiplicity source at every hypothetical right-half zero, without exposure or simplicity. Sufficient independent control of the whole signed sent sum or accumulated chord costs, a new zero-free region from this path, and RH remain open. No external premise, numerical certificate or historical novelty claim is introduced. Default endpoints and both top-ten lists remain unchanged."),
    ("rieszCycleMidpointTheorem", .str "RiemannGaussian.ZetaRieszPairMidpoint.sent_pair_bandWeight_eq_midpoint"),
    ("rieszCycleCorrelationTheorem", .str "RiemannGaussian.ZetaRieszCycleCorrelation.area_actual_full"),
    ("rieszCycleSavingTheorem", .str "RiemannGaussian.ZetaRieszCycleSaving.twice_min_norm_le_cycleSaving"),
    ("rieszCycleOverheadTheorem", .str "RiemannGaussian.ZetaRieszPhaseBracket.bracketOverhead_sub_one_le_sq"),
    ("rieszCycleCapacityTheorem", .str "RiemannGaussian.ZetaRieszPhaseBracket.norm_bracket_le_capacity"),
    ("rieszCycleWholeBoundTheorem", .str "RiemannGaussian.ZetaRieszCycleIteration.norm_actual_band_le_exact_cycles"),
    ("rieszCycleTransportTheorem", .str "RiemannGaussian.ZetaRieszCycleSource.norm_actual_band_le_cycles_then_transport"),
    ...
    ("rieszArithmeticCycleTotalTheorem", .str "RiemannGaussian.ZetaRieszArithmeticCycles.arithmeticSavings_le_totalSaving"),
    ("rieszArithmeticCycleWholeTheorem", .str "RiemannGaussian.ZetaRieszArithmeticCycles.norm_actual_band_le_available_arithmetic_cycles"),
    ("rieszArithmeticCycleSourceTheorem", .str "RiemannGaussian.ZetaRieszArithmeticCycles.tendsto_available_arithmetic_cycle_source"),
    ("rieszPacketSavingTheorem", .str "RiemannGaussian.ZetaRieszPacketIteration.norm_actual_band_le_packet_total"),
    ("rieszExactCellAffineTheorem", .str "RiemannGaussian.ZetaRieszPrimeCells.targetProfile_eq_affine"),
    ("rieszCellAdjacentOrdersTheorem", .str "RiemannGaussian.ZetaRieszCellOrders.prime_cell_eq_adjacent_orders"),
    ("rieszCellCovarianceTheorem", .str "RiemannGaussian.ZetaRieszCellEnergy.covariance_eq_midpoint"),
    ("rieszRetainedCellBoundTheorem", .str "RiemannGaussian.ZetaRieszRetainedCells.available_cell_mass_le_arithmetic_allowance"),
    ("rieszCompletePrimeIntervalTheorem", .str "RiemannGaussian.ZetaRieszPrimeIntervals.intervalPrime_between"),
    ("rieszArithmeticCellsTheorem", .str "RiemannGaussian.ZetaRieszArithmeticCells.actual_band_eq_arithmetic_cells"),
    ("rieszArithmeticPrimeMomentsTheorem", .str "RiemannGaussian.ZetaRieszArithmeticCells.arithmeticCellResponse_eq_prime_moments"),
    ("rieszWholeCellSavingTheorem", .str "RiemannGaussian.ZetaRieszCellCycles.norm_actual_band_le_cell_saving"),
    ("rieszWholeCellSourceTheorem", .str "RiemannGaussian.ZetaRieszCellCycles.tendsto_retained_cell_source"),
    ("rieszSmallOwnerMassTheorem", .str "RiemannGaussian.ZetaRieszSmallCompositeCells.eventually_smallOwner_mass_le"),
    ("rieszSmallOwnerCellDecayTheorem", .str "RiemannGaussian.ZetaRieszSmallCompositeCells.tendsto_smallOwner_cell_mass"),
    ("rieszRemainingOwnerSupportTheorem", .str "RiemannGaussian.ZetaRieszSmallCompositeCells.remainingCells_support"),
    ("rieszRemainingOwnerSourceTheorem", .str "RiemannGaussian.ZetaRieszSmallCompositeCells.tendsto_remaining_cell_source"),
    ("rieszWingCompletionTheorem", .str "RiemannGaussian.ZetaRieszWingReserve.eventually_norm_reserve_completion"),
    ("rieszWingReserveTheorem", .str "RiemannGaussian.ZetaRieszWingReserve.eventually_re_reserve_ge"),
    ("rieszWingHighBoundTheorem", .str "RiemannGaussian.ZetaRieszWingHighOrders.eventually_norm_highWing"),
    ("rieszWingHighDecayTheorem", .str "RiemannGaussian.ZetaRieszWingHighOrders.tendsto_highWing"),
    ("rieszWingUnpaidSupportTheorem", .str "RiemannGaussian.ZetaRieszWingHighOrders.unpaidOrders_support"),
    ("rieszHarmonicAnnularWindowTheorem", .str "RiemannGaussian.ZetaRieszHarmonicWindow.exists_annular_window_error"),
    ("rieszHarmonicReserveWindowTheorem", .str "RiemannGaussian.ZetaRieszHarmonicWindow.exists_reserve_window_error"),
    ("rieszHarmonicAnnularSourceTheorem", .str "RiemannGaussian.ZetaRieszHarmonicWindow.tendsto_annularRemainder"),
    ("rieszHarmonicWindowSourceTheorem", .str "RiemannGaussian.ZetaRieszHarmonicWindow.tendsto_remainder_add_reserve"),
    ("rieszHarmonicWindowCriterionTheorem", .str "RiemannGaussian.ZetaRieszHarmonicWindow.false_of_remainder_floor"),
    ("rieszHarmonicOriginalSourceTheorem", .str "RiemannGaussian.ZetaRieszPrimeCountMass.tendsto_few_add_wing_source"),
    ("rieszJointCofactorBoundTheorem", .str "RiemannGaussian.ZetaRieszJointCofactor.exists_unpaidWing_add_compositeWing_bound"),
    ("rieszJointCofactorIdentityTheorem", .str "RiemannGaussian.ZetaRieszJointCofactor.unpaidWing_add_compositeWing"),
    ("rieszJointCofactorCoefficientTheorem", .str "RiemannGaussian.ZetaRieszJointCofactor.coupledCoefficient_eq_original"),
    ("rieszJointCofactorSourceTheorem", .str "RiemannGaussian.ZetaRieszJointCofactor.tendsto_arithmeticRemainder_add_reserve"),
    ("rieszJointCofactorCompletionTheorem", .str "RiemannGaussian.ZetaRieszJointCofactor.exists_completedWing_bound"),
    ("rieszJointCofactorBoundaryTheorem", .str "RiemannGaussian.ZetaRieszJointCofactor.exists_boundaryWing_bound"),
    ("rieszJointCofactorPrefixTheorem", .str "RiemannGaussian.ZetaRieszJointCofactor.norm_prefixWing_le"),
    ("rieszJointCofactorDiagonalTheorem", .str "RiemannGaussian.ZetaRieszJointCofactor.norm_diagonalWing_le"),
    ("rieszAllocationShareTheorem", .str "RiemannGaussian.ZetaRieszJointAllocation.boundedShare_bounds"),
    ("rieszAssignedCompanionTheorem", .str "RiemannGaussian.ZetaRieszJointAllocation.hasSum_assignedAtom"),
    ("rieszRetainedMaskTheorem", .str "RiemannGaussian.ZetaRieszMaskSupport.residualMask_eq_retained"),
    ("rieszRetainedMovingErrorTheorem", .str "RiemannGaussian.ZetaRieszMaskSupport.tendsto_arithmetic_sub_retained_moving"),
    ("rieszRetainedUniformErrorTheorem", .str "RiemannGaussian.ZetaRieszMaskSupport.eventually_all_heights_arithmetic_error"),
    ("rieszReserveExactTheorem", .str "RiemannGaussian.ZetaRieszWingReserve.tendsto_reserve_exact"),
    ("rieszRetainedExactSourceTheorem", .str "RiemannGaussian.ZetaRieszMaskSupport.tendsto_retained_exact_source"),
    ("rieszRetainedCostBoundTheorem", .str "RiemannGaussian.ZetaRieszMaskSupport.retainedCost_lt_thirtyseven_fortieths"),
    ("rieszDominantRatesTheorem", .str "RiemannGaussian.ZetaRieszDominantAllocation.rates_bounds"),
    ("rieszDominantBoundTheorem", .str "RiemannGaussian.ZetaRieszDominantAllocation.dominantSector_bound"),
    ("rieszNondominantMovingErrorTheorem", .str "RiemannGaussian.ZetaRieszDominantAllocation.tendsto_arithmetic_sub_nondominant_moving"),
    ("rieszNondominantSupportTheorem", .str "RiemannGaussian.ZetaRieszDominantAllocation.nondominant_prime_log_lt"),
    ("rieszNondominantSourceTheorem", .str "RiemannGaussian.ZetaRieszDominantAllocation.tendsto_nondominant_exact_source"),
    ("rieszNondominantDeficitTheorem", .str "RiemannGaussian.ZetaRieszDominantAllocation.eventually_nondominant_re_lt_neg_three_fortieths"),
    ("rieszReflectedLinearTheorem", .str "RiemannGaussian.ZetaRieszReflectedLinear.coefficient_eq_linear"),
    ("rieszReflectedCountTheorem", .str "RiemannGaussian.ZetaRieszReflectedLinear.linearClass_count_le_nine"),
    ("rieszOneSidedBoundTheorem", .str "RiemannGaussian.ZetaRieszOneSidedArithmetic.nondominantRemainder_lower_with_four_credit"),
    ("rieszFourCreditTheorem", .str "RiemannGaussian.ZetaRieszOneSidedArithmetic.four_phaseCost_le_quarter"),
    ("rieszBalancedMaskAuditTheorem", .str "RiemannGaussian.ZetaRieszAllowanceComplement.balanced_mem_nondominant"),
    ("rieszComplementGrowthTheorem", .str "RiemannGaussian.ZetaRieszAllowanceGrowth.eventually_complement_growth"),
    ("rieszComplementDivergenceTheorem", .str "RiemannGaussian.ZetaRieszAllowanceGrowth.complement_tendsto_atTop"),
    ("rieszAllowanceDivergenceTheorem", .str "RiemannGaussian.ZetaRieszAllowanceGrowth.improved_allowance_tendsto_atTop"),
    ("rieszAllowanceNoCofinalCeilingTheorem", .str "RiemannGaussian.ZetaRieszAllowanceGrowth.not_frequently_improved_allowance_le_three_fortieths"),
    ("rieszTypeIIWindowTheorem", .str "RiemannGaussian.ZetaRieszTypeII.exists_narrow_window_error"),
    ("rieszTypeIIPairTheorem", .str "RiemannGaussian.ZetaRieszTypeII.pairForm_eq_sum"),
    ("rieszTypeIIFiniteBoundTheorem", .str "RiemannGaussian.ZetaRieszTypeII.norm_narrow_of_typeII"),
    ("rieszTypeIIFloorTheorem", .str "RiemannGaussian.ZetaRieszTypeII.eventually_signed_floor_of_typeII"),
    ("rieszTypeIIContradictionTheorem", .str "RiemannGaussian.ZetaRieszTypeII.not_typeII_of_simple_exposed_zero"),
    ("rieszTypeIILogRateTheorem", .str "RiemannGaussian.ZetaRieszTypeII.logarithmic_envelope_eventually_exceeds"),
    ("rieszTypeIIHeightTheorem", .str "RiemannGaussian.ZetaRieszTypeII.fixed_height_outside_repo_vmvt"),
    ("rieszTypeIIScope", .str "Quantitative go/no-go test on 1/2<u<=5001/10000. Tails outside 1.95N<log n<=2.05N have an independent geometric bound. The literal prime-pair form retains all masks, allocation, factorial phase, incidence normalization and unsaturated correction. A fixed 1/1000 power saving on each unit log cell implies source-scale decay at rate 1667/1670 times a fixed polynomial, hence the eventual -3/40 floor and a simple-exposed-zero contradiction. That arithmetic estimate is an explicit unproved premise. Logarithmic error envelopes and the fixed-degree growing-height VMVT rectangle do not discharge it. No-go with current inputs; no independent surviving floor, zero-free region or RH proof is claimed."),
    ("rieszOneSidedScope", .str "The actual nondominant remainder has the independent finite lower bound Re R_j >= -B_j+(3/4)*B_4,j for every u>=0 and every height and order. The complete reflected divisor sum is evaluated when every prime logarithm lies between D/2 and D, with D=log n-L>=0; this class has at most nine prime factors on the actual window. Its exact parity and radial sign remain. The specified four-prime subfamily receives at least a 75-percent saving on its own former charge. This is not a saving on the whole carrier. The subsequent complement audit proves that this fixed improved allowance diverges for every fixed height and 1/2<u<=exp(-11/16), so it cannot supply the required cofinal floor. The independent signed floor remains open; no new zero exclusion follows."),
    ("rieszAllowanceObstructionScope", .str "Negative result for the exact fixed positive allowance, with every original mask unchanged. For every fixed real height y and 1/2<u<=exp(-11/16), B_j-B_4,j >= c(u,y)*(2u)^N_j/(N_j+1)^4 eventually, with c(u,y)>0. Balanced products of exactly three distinct primes near log n=2N survive the window, prime-count cutoff, physical cutoff and both paid sectors. Their unassigned fraction is eventually at least 1/2. The proved prime number theorem supplies actual positive-phase boxes. Both B_j-B_4,j and B_j-(3/4)*B_4,j tend to positive infinity; even a cofinal 3/40 ceiling is impossible. No hypothetical zero, exposure or simplicity is assumed. The constant and starting index are unevaluated; arbitrary moving heights are not covered. This is an obstruction theorem, not an independent signed floor or a new zero exclusion. The signed source remains unchanged and its joint lower bound remains open."),
    ("rieszAllocationScope", .str "The complete signed companion is exactly a bounded multinomial allocation theta with 0<=theta<=1 of each original integer. The finite comparison retains 1_S-theta until every off-mask correction is paid. Weighted high-count deletion, complete window tails and the balanced companion estimate discharge those corrections; the original balanced carrier is not bounded by the companion estimate. The retained multiplier is then exactly 1-theta on the original mask minus the independently paid sector. All aggregate reduction errors vanish for arbitrary moving heights; one starting index works for every real height at each fixed eligible radius and positive tolerance. The radius range is 1/2<u<=exp(-11/16). These estimates use no hypothetical zero. The independent signed floor remains open."),
    ("rieszReserveExactScope", .str "The complete positive reserve has exact complex limit m^2[log(19/17)+(1-1/(-2u log u))*log(15/13)] under the original exposed-zero hypotheses on 1/2<u<exp(-11/16). Both reciprocal marginals and integer endpoints are evaluated. The retained source is -m+m^2*c_ret(u), where c_ret(u)=log(32/13)/(-2u log u)-log(19/13). Independently, c_ret(u)<37/40 throughout the closed interval 1/2<=u<=exp(-11/16). For simple exposed zeros the retained real part is eventually below -3/40. This is source-forced negativity, not an independent arithmetic floor. The independent cofinal real floor remains open; no zero exclusion follows."),
    ("rieszDominantScope", .str "The entire remaining allocated contribution with an eligible selected prime carrying at least 13/20 of log n has independent norm bound (r_H^N+r_L^N)*M(12001/12000), where r_H=(503/1000)*(12000/5999)*exp(-1/160)<1 and r_L=exp(-1/2000)<1. It holds for N>=320, every real height, every count cutoff and 1/2<=u<=exp(-11/16). The finite summable mass M is unevaluated. Keeping the missing allocation tail coupled to the factorial kernel and actual prime cutoff pays the endpoint; coefficient-only decay is not asserted. Every prime of every nonzero surviving label has log p<13/20*log n from dyadic index 32. The original signed coefficient, phase, masks and multiplier 1-theta remain, in 7N/4<log n<=9N/4 with all surviving counts coupled. Aggregate reduction errors vanish for arbitrary moving heights. Under the original exposed-zero hypotheses on 1/2<u<exp(-11/16), the smaller carrier tends to -m+m^2*c_ret(u); c_ret(u)<37/40. For simple exposed zeros its real part is eventually below -3/40. An independent cofinal real floor at -3/40 remains open. Balanced products remain unpaid. No restricted contradiction or new zero-free region follows."),
    ("rieszJointCofactorScope", .str "The original unpaid wing plus an explicit complete signed composite companion has independent source-scale norm bound C_gamma(N+1)^2 exp(-N/64) for 1/2<=u<exp(-2/3), at each fixed abs(gamma)>1. One constant works across the radius interval; the starting order may depend on u, and the constant is unevaluated and not uniform in height. The original prime set, unpaid orders, phases and factorial weights remain. The complete cofactor response retains the mark size, and the saturation correction, clipped prime prefix, repeated-prime diagonal and unit are all paid. No zero, exposure, simplicity or cancellation premise enters this joint bound. Separate decay of either summand is not proved. The surviving arithmetic difference is the actual masked finite lower-count response MINUS the complete composite companion; the companion includes all admissible cofactors and is not presumed to be a finite subfamily or counted once per integer. Under the original exposed-zero hypotheses, on 1/2<u<exp(-11/16), the difference plus the positive reserve retains the exact source -m+m^2c(u). The finite response keeps 7N/4<log n<=9N/4. For simple exposed zeros, a cofinal real floor -eta with eta<1-c(u)+15/544 would suffice; that joint arithmetic floor remains open. The subsequent allocation and dominant-prime theorems discharge the finite-mask comparison, evaluate the full reserve and reduce the surviving support; the independent signed bound remains open. No new zero-free region or RH conclusion follows."),
    ("rieszHarmonicWingScope", .str "The active harmonic-cost route retains the exact head-plus-central limit -m^2 c(u) and the proved deletion of complete high-prime-count classes. Throughout 1/2<u<exp(-2/3), all lower-count terms outside 25N/16<log n<=5N/2 have a geometric source-scale bound. On the smaller interval 1/2<u<exp(-11/16), this window narrows to 7N/4<log n<=9N/4; both bounds are independent of zeros and uniform in height, count threshold and radius up to the stated ceiling for every fixed full complex filter. In the smaller interval, exposed-zero phases and independent prime completion give an eventual positive wing reserve at least (15/544)m^2. The high-leg wing with 4N<=5(N+1-k) is bounded by C(N+1)^2 exp(-N/1024) and vanishes under the exposed-zero premises. Only wing orders k<=floor(13N/32), 5(N+1-k)<4N remain, with every original mask and taper. The narrowed lower-count sum and this middle wing remain coupled; their sum plus the positive reserve tends to -m+m^2c(u). For simple exposed zeros an independent cofinal real floor -eta with eta<1-c(u)+15/544 would suffice. That joint arithmetic floor remains open. No simplicity, exposure, numerical threshold, new zero-free region or RH conclusion is silently inferred. The full original harmonic interval remains the goal; the new wing bounds cover only its stated subinterval."),
    ("rieszOwnerLengthCeilingTheorem", .str "RiemannGaussian.ZetaRieszOwnerWindow.eventually_length_le_source_log"),
    ("rieszOwnerGeneralMassTheorem", .str "RiemannGaussian.ZetaRieszOwnerMass.exists_growing_owner_mass_decay"),
    ("rieszOwnerGeneralSourceTheorem", .str "RiemannGaussian.ZetaRieszOwnerCells.exists_remaining_source"),
    ("rieszOwnerFifthMassTheorem", .str "RiemannGaussian.ZetaRieszOwnerBounds.eventually_fifth_owner_mass_le"),
    ("rieszOwnerFifthSourceTheorem", .str "RiemannGaussian.ZetaRieszOwnerBounds.tendsto_fifth_remaining_source"),
    ("rieszOwnerTenthMassTheorem", .str "RiemannGaussian.ZetaRieszOwnerBounds.eventually_tenth_owner_mass_le"),
    ("rieszOwnerTenthSourceTheorem", .str "RiemannGaussian.ZetaRieszOwnerBounds.tendsto_tenth_remaining_source"),
    ("rieszOwnerWindowSupportTheorem", .str "RiemannGaussian.ZetaRieszOwnerCells.remainingCells_support"),
    ("rieszOwnerWindowScope", .str "The original full carrier has independent composite-owner absolute-mass bounds. The exp(N/5) cutoff replaces exp(N/10) for 0<u<exp(-2/3); its common base is (3/2) exp(-9403/23040)<1. The exp(N/10) cutoff now works for 0<u<exp(-16/25), with common base (3/2) exp(-10427/25600)<1. Every bound keeps the full complex polynomial filter and is uniform in all heights, including moving heights. More generally, every 1/2<u<1 except u=exp(-1/2) admits some positive exponent delta(u), chosen analytically, whose complete composite-owner absolute mass decays. This does not assert the same fixed cutoff throughout that interval, optimality of the cofactor exponent, or failure of other methods at the scalar contact. The exact damped floor cutoff remains linked to u and the entire original divisor majorant pays the support without a separate cofactor-count cost. The corresponding whole-cell norm mass vanishes and the full pole-jet negative multiplicity source survives without exposure or simplicity. Every source claim retains its stated radius hypothesis. The actual starting order is unevaluated. The remaining cells are semiprimes or composite owner cofactors larger than the chosen exponential cutoff. Their joint signed bound remains open; these component bounds do not establish a new zero-free region or RH. Earlier estimates remain available, default endpoints and both top-ten lists are unchanged."),
    ("rieszSmallOwnerScope", .str "The actual full original carrier now has an independently controlled growing composite-owner component. If a is the cofactor left after removing the largest prime, all composite a with log a <= N/10 have geometrically vanishing source-normalized absolute mass for 1/2 <= u < exp(-2/3). The bound keeps every complex polynomial and is uniform over all heights, including arbitrary moving heights. Its constant is u times the existing factorial-tilt constant, and its geometric base is (3/2) exp(-631/1536), proved strictly below one. The physical saturation condition becomes automatic at sufficiently large orders; no numerical starting order is claimed. The complete norm mass of every corresponding prime-interval cell is also controlled, so neither internal nor inter-cell cancellation is needed for this paid component. The remaining cells consist exactly of prime owner cofactors (semiprimes) or composite owner cofactors with log a > N/10. Their joint signed response retains the full negative-multiplicity pole-jet source in this range, without exposure or simplicity. A sufficient independent bound for that remaining response, a new zero-free region from this path and RH remain open. This is a component decay theorem, not zero exclusion. The full-range exact cell decomposition remains available outside this narrower quantitative range. Default endpoints and both top-ten lists remain unchanged."),
    ("rieszPrimeCellScope", .str "Exact packet cycles retain full signed packet sums and actual remaining fractions. Literal divisor cells have exactly affine profiles; signed boundary corrections charge only crossed hinges. The two adjacent factorial prime moments keep the original band mask, full complex filter and signed covariance. Unique largest-prime ownership partitions the complete original carrier with no dropped or duplicated term. Exact regrouping of the arithmetic-cycle residual into cells never increases the earlier arithmetic allowance. Fixed arithmetic cells are independent of height and filter and contain every prime between their endpoints, including zero-filter terms. Whole-cell cycles spend one scalar per complete prime interval, preserve the full carrier and give its finite upper bound by cell norm mass minus exact saving. No internal phase-dependent prime selection is introduced by this alternative. The full pole-jet source survives with arbitrary multiplicity, no exposure assumption and all 1/2<u<1. Independent sufficient cofinal control of the coupled prime moments or the signed sum between cells remains open; no new zero-free region or RH proof is claimed. Conditional interval-envelope transport is not an independent analytic estimate. Finite floating-point annulus probes do not establish a full-band or asymptotic bound. Default endpoints and both top-ten lists remain unchanged."),
    ("rieszArithmeticCycleScope", .str "Exact target profiles retain the signed cofactor change across a phase half-turn and pay only actual flank gaps. Prime cofactors use the proved half-divisor slope; composite squarefree cofactors use the quarter-divisor slope. Every partner keeps its actual full-polynomial non-Riesz amplitude, original support, separate oriented ray capacity and actual previously retained fraction. The central original atom is unrestricted, so cross-prime-count cycles are allowed. Literal gcd, prime, nonunit-squarefree and complex-orientation tests define a nonnegative local arithmetic budget, with zero credit on failure. All local estimates and their accumulation are proved without an aggregate cancellation premise. The available initial-budget filter prevents uncredited steps from consuming later capacity. The entire original carrier has an explicit upper allowance of absolute mass minus the accumulated arithmetic saving, for every complex filter, cutoff, order and height. All inactive, unmatched and unselected mass remains included. The full pole-jet source survives at every right-half zero without exposure or simplicity. Sufficient aggregate capacity or a cofinal strictly sub-source upper allowance, a new zero-free region from this path and RH remain open. Finite floating-point physical-annulus tests are neither full-band certificates nor asymptotic results. No external premise or historical novelty claim is introduced. Default endpoints and both top-ten lists remain unchanged."),
    ("rieszWiderMatchedScope", .str "On 1/2<=u<exp(-2/3), a strictly larger actual unfiltered head/pair block has eventual budget -m_rho^2/8<=Re(u^(N+1)*matchedBlock)<=-m_rho^2/1536, with explicit zero and exposure premises and unrestricted multiplicity. This is a persistent negative contribution, not an independent subunit whole-carrier floor. Independent completion reaches N<=3k and32k<=17N, uniformly in height, with exp(-11N/30720)*(Z1025+Z8193) error; its derivative-weighted error also vanishes. The analytic tilt optimum is q=theta/ell only for this estimate, with admissibility separate. Actual order counts and reciprocal costs hold for N>=256; this is ONLY the structural threshold. The larger block contains every old matched order and has a proved new order. Its relative head cost is in[5/9,4/5] and its reciprocal-order sum in[1/16,5/16]. Exact complements keep every remaining head, pair, three-prime and higher-prime term. Only unmatchedJoint+matchedBlock retains source -m_rho; nondecay and source-change guards are proved. An exact reflected identity exposes weight 1-log(p)/L_N with an independent inverse-length envelope gain. The exponential factor and both completion-error products remain to be controlled. The remaining JOINT floor and other global source ranges remain open; no RH or zero-free enlargement follows."),
