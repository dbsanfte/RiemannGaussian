...
private def milestones : Array Milestone := #[
  ...
  {
    label := "Xi symmetry moves the complete live gradient functional into the positive critical half-strip"
    lineOne := "positive xi half-strip"
    lineTwo := "1/2 < Re s < 1"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_separatedSelectedLaplaceXiLogNormPositiveHalfStripGradientScalarHeat
  }
]
