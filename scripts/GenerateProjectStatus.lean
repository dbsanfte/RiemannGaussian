...
private def renderSvg (moduleCount declarationCount theoremCount : Nat) : String :=
  ...
  "<svg xmlns=\"http://www.w3.org/2000/svg\" role=\"img\" " ++
    ...
    "  <text class=\"frontier\" x=\"20\" y=\"220\">Inventory, not proximity meter. Open: " ++
      "derive a zero constraint from the convergent eta log derivative; no current theorem implies RH.</text>\n" ++
    "</svg>\n"
  ...
  let statusJson := Json.mkObj [
    ...
    ("frontier", Json.mkObj [
      ...
      ("target", .str (
        ...
        "boundary-plus-gradient functional on 1/2 < Re s < 1 that forces " ++
        "its detector limit to vanish"))
    ]),
