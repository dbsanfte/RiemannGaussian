...
private def milestones : Array Milestone := #[
  ...
  {
    label := "A growing diagonal sequence of finite paired-eta log-norm variations converges to the complete detector"
    lineOne := "finite eta log flux"
    lineTwo := "limit = detector"
    role := "bridge"
    theoremName :=
      ``RiemannGaussian.tendsto_pairedEtaBoundaryHeatFiniteLogNormRadialFluxDiagonal
  }
]
...
private def renderSvg (moduleCount declarationCount theoremCount : Nat) : String :=
  ...
  "<svg xmlns=\"http://www.w3.org/2000/svg\" role=\"img\" " ++
    ...
    "  <text class=\"frontier\" x=\"20\" y=\"220\">Inventory, not proximity meter. Open: " ++
      "global arithmetic control of finite eta log variation; no current theorem implies RH.</text>\n" ++
    "</svg>\n"
  ...
  let statusJson := Json.mkObj [
    ...
    ("frontier", Json.mkObj [
      ...
      ("target", .str (
        ...
        "for the selected growing finite paired-eta log-norm variation " ++
        "sequence that forces its exact diagonal detector limit to vanish " ++
        "without discarding its zero contributions"))
    ]),
