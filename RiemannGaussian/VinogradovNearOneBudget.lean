...
/-!
# Unconditional coverage of every original near-one dyadic block

The displacement is of order n^(-2), while the height exponent is of
order n^(-3). Small blocks use their actual damped mass, middle blocks
use the proved Vinogradov moment with constructed parameters, and long
blocks use the two paid classical derivative profiles. The explicit
height threshold discharges every root and degree condition.
-/

namespace RiemannGaussian.VinogradovNearOneBudget
...
def delta (n : ℕ) : ℝ := 1 / (65536 * (2 * (n : ℝ) + 1) ^ 2)
...
theorem delta_le (n : ℕ) : delta n ≤ 1 / 4096 := by
  ...
  unfold delta
  apply (div_le_iff₀ (by positivity : 0 < 65536 * (2 * (n : ℝ) + 1) ^ 2)).mpr
...
/-- The selected degree pays twice the target line displacement. -/
theorem saving_ge {n k : ℕ} (hk : 1 ≤ k) (hkn : k ≤ 2 * n + 1) :
    2 * delta n ≤ 1 / (32768 * (k : ℝ) ^ 2) := by
  have hkpos : (0 : ℝ) < k := by exact_mod_cast hk
  have hkr : (k : ℝ) ≤ 2 * (n : ℝ) + 1 := by exact_mod_cast hkn
  have hsq : (k : ℝ) ^ 2 ≤ (2 * (n : ℝ) + 1) ^ 2 := by nlinarith
  have h := one_div_le_one_div_of_le
    (by positivity : 0 < 32768 * (k : ℝ) ^ 2)
    (mul_le_mul_of_nonneg_left hsq (by norm_num : (0 : ℝ) ≤ 32768))
  have he : 2 * delta n = 1 / (32768 * (2 * (n : ℝ) + 1) ^ 2) := by
    unfold delta
    field_simp
    ring
  rw [he]
...
theorem middle_block_bound (n j : ℕ) (hn : 12 ≤ n) {s : ℂ}
  ...
  obtain ⟨k, M, hk, hkn, hkr, hMX, hXM, hkt, htk⟩ :=
    exists_window_parameters n (2 ^ j) hn ht hlo hhi
  have hσ := (line_bounds n).1.trans hline
  have h := VinogradovDyadic.dyadic_block_bound k M j hk hkr hMX hXM s
    (by linarith) hkt htk
  have hsaving := saving_ge (by omega : 1 ≤ k) hkn
  have hdelta := delta_pos n
  have hlead : 1 - s.re - 1 / (32768 * (k : ℝ) ^ 2) ≤ 0 := by
...
theorem block_bound (n j : ℕ) (hn : 12 ≤ n) {s : ℂ} (hline : line n ≤ s.re)
...
theorem canonical_block_bound (n : ℕ) (hn : 12 ≤ n) {s : ℂ}
...
end RiemannGaussian.VinogradovNearOneBudget
