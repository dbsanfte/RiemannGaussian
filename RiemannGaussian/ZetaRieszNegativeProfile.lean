...
/-!
# A proved negative conditioned profile in the original Riesz carrier

At the original initial scale pair a=0,b=1, replace the complete
conditioned-energy allowance by an independently proved B*p^beta, beta<0.
Both homogeneous moment premises and every later conditioned-energy premise
are discharged. The actual Riesz coefficients, complex fibre correlations,
attained-frequency sampling cost and positive lower block mass remain intact.

The prime threshold, coupled finite cutoff p^S<=X and actual padded quotient
threshold remain explicit. Constants and the negative exponent are not
numerically evaluated. The remaining signed correlation and sampling costs
still need a net source-scale saving. This theorem proves neither RH nor a
larger zero-free region.
-/

namespace RiemannGaussian.ZetaRieszNegativeProfile
...
/-- The independently proved negative profile replaces the complete initial conditioned-energy allowance in the original Riesz carrier by a decreasing prime-power bound. -/
theorem exists_original_band_negative_profile (k u : ℕ) (hk : 2 ≤ k) (hu : k ≤ u) :
  ...
  let lam : ℝ := (((k * (2 * u + 1) : ℕ) : ℝ) - 1 / (3 * (k : ℝ)))
  obtain ⟨C, hC, N₀, hbudget⟩ := VinogradovImprovedNormalization.exists_improved_rounded_budget k u hk hu
  obtain ⟨beta, hbetalow, hbeta, S, hS, B, hB, hallow⟩ := exists_initial_allowance_saving
    hk hu hC (VinogradovNegativeProfile.first_exponent_gt_critical hk) hbudget
  ...
  have hiteration := mixed_le_scale_allowance_of_scaled_bounds (a := 0) (b := 1) (xi := 0)
    (H := 1) hk hu (by omega) (by omega) (by omega) hXpos hC
    (VinogradovNegativeProfile.first_exponent_gt_critical hk).le hp
...
end RiemannGaussian.ZetaRieszNegativeProfile
