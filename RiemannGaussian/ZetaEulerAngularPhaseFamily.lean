...
-/
import RiemannGaussian.ZetaSharpAngularPhaseFamily
import RiemannGaussian.ZetaEulerAngularBound

/-!
# Direct Euler growth in every complete phase-family zero bound

The eta-free Gaussian allowance reaches the actual selected zero and
all countably many prime-phase channels. The removed cost is exactly
`2*mass*log(4/delta)/(pi*delta)` in the complete source budget. Full
multiplicity, frequency and radial corrections are retained.
-/

namespace RiemannGaussian.ZetaEulerAngularPhaseFamily
...
def totalAllowance (k : ℕ) (x t : ℝ) (a ω : ℕ → ℝ) : ℝ :=
  ∑' n, tail a n * ZetaEulerAngularBound.allowance k x (ω n * t)
...
theorem summable_allowance {a ω : ℕ → ℝ} (ha : ∀ n, 0 ≤ a n) (hs : Summable a)
    ...
    (hscale : 1 ≤ scale t) :
    Summable (fun n ↦ tail a n * ZetaEulerAngularBound.allowance k x (ω n * t)) := by
  ...
  · have hw := hω n hn
    have hfreq : 2 ≤ |ω n * t| := by
      ...
      nlinarith
    have hnew := ZetaEulerAngularBound.allowance_nonneg k hk hfreq hx
    have hb := ZetaEulerAngularBound.allowance_add_twelve_le_previous k hk x hfreq
...
/-- The exact removed logarithmic allowance survives the whole infinite
channel sum, with its full nonconstant coefficient mass. -/
theorem totalAllowance_add_gain {a ω : ℕ → ℝ}
    (ha : ∀ n, 0 ≤ a n) (hs : Summable a) (hω : ∀ n, n ≠ 0 → 1 ≤ ω n)
    (hlog : Summable (fun n ↦ tail a n * Real.log (ω n)))
    (k : ℕ) (hk : 2 ≤ k) {x t : ℝ} (hx : 0 < x) (ht : 2 ≤ |t|)
    (hscale : 1 ≤ scale t) :
    totalAllowance k x t a ω + ZetaEulerLogProfile.gain k * mass a =
      ZetaSharpAngularPhaseFamily.totalAllowance k x t a ω := by
  have hn := summable_allowance ha hs hω hlog k hk hx ht hscale
  have hm := (tail_summable ha hs).mul_left (ZetaEulerLogProfile.gain k)
  have he : (∑' n, (tail a n * ZetaEulerAngularBound.allowance k x (ω n * t) +
      ZetaEulerLogProfile.gain k * tail a n)) =
      ∑' n, tail a n * ZetaNearOneSharpAngularBound.allowance k x (ω n * t) := by
    apply tsum_congr
    intro n
    have h := ZetaEulerAngularBound.allowance_add_gain k x (ω n * t)
    linear_combination tail a n * h
  rw [hn.tsum_add hm, tsum_mul_left] at he
  exact he
...
theorem source_le_budget {a ω : ℕ → ℝ} (ha : ∀ n, 0 ≤ a n) (hs : Summable a)
  ...
  have hpoint (n : ℕ) : a n * D n + (if n = 1 then a 1 * S else 0) ≤
      (if n = 0 then a 0 * (1 / x + 448 * localZetaLogHeight 0) else 0) +
        2 * (tail a n * ZetaEulerAngularBound.allowance k x (ω n * ρ.1.im)) / (Real.pi * delta k) := by
    by_cases hn0 : n = 0
    · subst n
    ...
    · by_cases hn1 : n = 1
      · subst n
        have hz := mul_le_mul_of_nonneg_left
          (ZetaEulerAngularBound.neg_logDeriv_re_le_sub_zero k hk ρ ht hx hx' hnear) (ha 1)
      ...
      · have hw := hω n hn0
        have hfreq : 2 ≤ |ω n * ρ.1.im| := by
          ...
          nlinarith
        have hz := mul_le_mul_of_nonneg_left
          (ZetaEulerAngularBound.neg_logDeriv_re_le k hk hfreq hx hx') (ha n)
...
/-- The complete actual source budget improves by an exact positive
amount, with no altered zero source, prime kernel or radial correction. -/
theorem budget_add_gain {a ω : ℕ → ℝ}
    (ha : ∀ n, 0 ≤ a n) (hs : Summable a) (hω : ∀ n, n ≠ 0 → 1 ≤ ω n)
    (hlog : Summable (fun n ↦ tail a n * Real.log (ω n)))
    (k : ℕ) (hk : 2 ≤ k) {x t : ℝ} (hx : 0 < x) (ht : 2 ≤ |t|)
    (hscale : 1 ≤ scale t) :
    budget k x t a ω + 2 * ZetaEulerLogProfile.gain k * mass a / (Real.pi * delta k) =
      ZetaSharpAngularPhaseFamily.budget k x t a ω := by
  have h := totalAllowance_add_gain ha hs hω hlog k hk hx ht hscale
  unfold budget ZetaSharpAngularPhaseFamily.budget
  linear_combination (2 / (Real.pi * delta k)) * h
...
end RiemannGaussian.ZetaEulerAngularPhaseFamily
