...
/-!
# Sharper unconditional zero exclusion from the Gaussian endpoint

Retaining the exact shifted Gaussian endpoint sharpens the pole estimate
in the genuine Fermi budget, giving eventual width `9/(50*log(abs(t)))`.
The first global `3/20` margin and the original phase coefficients suffice.
The unchanged whole-divisor allowance discharges the remaining tail cost. The resulting height
threshold is existential, not numerically evaluated.
-/

namespace RiemannGaussian.GaussianFermiSharpZeroFree
...
/-- At the proved larger interior margin, the sharper Gaussian surplus
...
theorem margin_lt_one_sub_re_of_allowance_lt_one (ρ : NontrivialZetaZero)
    ...
    9 / (50 * Real.log |ρ.1.im|) < 1 - ρ.1.re := by
  classical
  by_contra hnot
  have hd : 1 - ρ.1.re ≤ 9 / (50 * Real.log |ρ.1.im|) := le_of_not_gt hnot
  let t := |ρ.1.im|
  let L := Real.log t
  let H := 48 * t
  ...
  have hscale := GaussianFermiSharpProfile.scale_admissible
    ...
    (show L * m ≤ 2 / 5 by change Real.log t * zetaFermiZeroMargin (48 * t) ≤ 2 / 5; linarith [hμ.2])
  have hB : 0 < B := by dsimp [B]; positivity
  have heB : B / 2 + B / 2 = B := by ring
  have hrho : 1 / 2 < ρ.1.re := by
    have hsmall : 9 / (50 * Real.log |ρ.1.im|) < (1 / 2 : ℝ) := by
  ...
  have hsurplus := GaussianFermiSharpProfile.exact_scaled_profile_surplus hL hμ.1 hμ.2 hd
...
/-- The sharper Gaussian endpoint bound proves eventual right-edge
coefficient `9/50`. The actual margin formula and original uniform
whole-divisor allowance are both discharged. -/
...
eventual `9/50` margin, by genuine horizontal reflection. -/
theorem exists_eventual_strip :
    ∃ T : ℝ, 1 ≤ T ∧ ∀ ρ : NontrivialZetaZero, T ≤ |ρ.1.im| →
      9 / (50 * Real.log |ρ.1.im|) < ρ.1.re ∧
        ρ.1.re < 1 - 9 / (50 * Real.log |ρ.1.im|) := by
...
theorem exists_eventual_nonvanishing :
    ∃ T : ℝ, 1 ≤ T ∧ ∀ s : ℂ, T ≤ |s.im| →
      1 - 9 / (50 * Real.log |s.im|) ≤ s.re → riemannZeta s ≠ 0 := by
  ...
  have hlog : 100000 ≤ Real.log |s.im| := by
    ...
    simpa only [Real.log_exp] using h
  have hsmall : 9 / (50 * Real.log |s.im|) < (1 / 2 : ℝ) := by
    apply (div_lt_iff₀ (by positivity)).mpr
    linarith
  ...
  have h := hzero ρ ht0
  change 9 / (50 * Real.log |s.im|) < 1 - s.re at h
  linarith

/-- The new literal region is strictly wider than the preceding `4/25`
region, with exact ratio `9/8` between their eventual widths. -/
theorem exists_eventual_improved_region :
    ∃ T : ℝ, 1 ≤ T ∧ ∀ s : ℂ, T ≤ |s.im| →
      4 / (25 * Real.log |s.im|) < 9 / (50 * Real.log |s.im|) ∧
        (1 - 9 / (50 * Real.log |s.im|) ≤ s.re → riemannZeta s ≠ 0) := by
...
end RiemannGaussian.GaussianFermiSharpZeroFree
