...
-/
import RiemannGaussian.GaussianFermiCurvatureProfile
import RiemannGaussian.ZetaLogRegionBand

/-!
# Unconditional zero exclusion from Gaussian endpoint curvature

The actual `9/50` region supplies a common margin for every zero in the
complete height band. Retaining curvature in the shifted Gaussian pole
interval gives a strict surplus at target `3/16` with the original phase
row. The existing whole-divisor allowance discharges the outside error.
Every eventual premise is proved; the final height is existential.
-/

namespace RiemannGaussian.GaussianFermiCurvatureZeroFree
...
/-- At the proved common `9/50` band margin, the Gaussian curvature
surplus excludes the larger right edge once the original tail allowance
is below one. All three premises are discharged below. -/
theorem margin_lt_one_sub_re_of_allowance_lt_one (ρ : NontrivialZetaZero)
    (hlog : 100000 ≤ Real.log |ρ.1.im|)
    (hband : ∀ η : NontrivialZetaZero, |η.1.im| ≤ 48 * |ρ.1.im| →
      9 / (50 * Real.log (48 * |ρ.1.im|)) ≤ η.1.re ∧
        η.1.re ≤ 1 - 9 / (50 * Real.log (48 * |ρ.1.im|)))
    (hold : zetaPoleReserveZeroMargin (48 * |ρ.1.im|) ≤
      9 / (50 * Real.log (48 * |ρ.1.im|)))
    (htail : allowance (4 / (25 * (Real.log |ρ.1.im|) ^ 2)) (48 * |ρ.1.im|) < 1) :
    3 / (16 * Real.log |ρ.1.im|) < 1 - ρ.1.re := by
  classical
  by_contra hnot
  have hd : 1 - ρ.1.re ≤ 3 / (16 * Real.log |ρ.1.im|) := le_of_not_gt hnot
  let t := |ρ.1.im|
  let L := Real.log t
  let H := 48 * t
  let m := 9 / (50 * Real.log H)
  let B := 4 / (25 * L ^ 2)
  ...
  have hμ : 1799 / 10000 ≤ L * m ∧ L * m ≤ 9 / 50 :=
    GaussianFermiCurvatureProfile.normalized_margin_bounds ht hlog
  have hmpos : 0 < m := pos_of_mul_pos_left (show 0 < m * L by nlinarith [hμ.1]) hL.le
  have hmu : m < 1 / 4 := by
    have h := mul_le_mul_of_nonneg_right hL1 hmpos.le
    nlinarith [hμ.2]
  have hscale := GaussianFermiSharpProfile.scale_admissible hL1 hmpos.le (by linarith [hμ.2])
  have hB : 0 < B := by dsimp [B]; positivity
  have heB : B / 2 + B / 2 = B := by ring
  have hrho : 1 / 2 < ρ.1.re := by
    have hsmall : 3 / (16 * Real.log |ρ.1.im|) < (1 / 2 : ℝ) := by
  ...
  have hbudget := resonant_pair_phase_bound Finset.univ phaseContactExactCoefficients
    ...
    (show B / 2 + B / 2 ≤ 1 by simpa only [heB] using hscale.2) hband ρ hrho
  ...
  have hsurplus := GaussianFermiCurvatureProfile.exact_scaled_profile_surplus hL hμ.1 hμ.2 hd
  rw [show 1 - ρ.1.re - m = (1 - m) - ρ.1.re by ring] at hsurplus
  have hcost := exact_phase_cost_le hγ (by linarith : 4000 ≤ Real.log |ρ.1.im|)
    hmpos hmu.le hscale.2 hscale.1
    (show 1 / 10 ≤ Real.log |ρ.1.im| * m by change 1 / 10 ≤ L * m; linarith [hμ.1])
...
/-- The actual `3/16` right-edge exclusion has every common-band,
comparison-margin and whole-divisor-tail premise discharged. -/
theorem exists_eventual_right_margin :
  ...
  obtain ⟨T₁, hT₁, hband⟩ := exists_eventual_sharp_common_margin
  ...
  have hcommon := hband (48 * |ρ.1.im|) (by linarith)
  have he := (hformula (48 * |ρ.1.im|) (by rw [abs_of_pos h48]; linarith)).1
  rw [abs_of_pos h48] at he
  have hμ := GaussianFermiCurvatureProfile.normalized_margin_bounds htpos hlog
  have hscale := GaussianFermiSharpProfile.scale_admissible
    (by linarith : 1 ≤ Real.log |ρ.1.im|) hcommon.1.le (by linarith [hμ.2])
  have hlogH : 0 < Real.log (48 * |ρ.1.im|) := by
    have h := Real.log_le_log htpos (show |ρ.1.im| ≤ 48 * |ρ.1.im| by linarith)
    linarith
  have hold : zetaPoleReserveZeroMargin (48 * |ρ.1.im|) ≤
      9 / (50 * Real.log (48 * |ρ.1.im|)) := by
    apply (zetaPoleReserve_margin_le_fermi _).trans
    rw [he]
    apply (div_le_div_iff₀ (by positivity) (by positivity)).mpr
    nlinarith
  have holdscale : zetaPoleReserveZeroMargin (48 * |ρ.1.im|) ^ 2 ≤
      4 / (25 * Real.log |ρ.1.im| ^ 2) :=
...
/-- Both edges of the actual nontrivial zero strip have the eventual
`3/16` logarithmic margin, by genuine horizontal reflection. -/
theorem exists_eventual_strip :
    ∃ T : ℝ, 1 ≤ T ∧ ∀ ρ : NontrivialZetaZero, T ≤ |ρ.1.im| →
      3 / (16 * Real.log |ρ.1.im|) < ρ.1.re ∧
        ρ.1.re < 1 - 3 / (16 * Real.log |ρ.1.im|) := by
...
/-- The new region feeds back into the complete actual height divisor,
including the bounded-height zeros needed by every common-margin budget. -/
theorem exists_eventual_common_margin :
    ∃ H₀ : ℝ, 1 ≤ H₀ ∧ ∀ H : ℝ, H₀ ≤ H →
      0 < 3 / (16 * Real.log H) ∧ 3 / (16 * Real.log H) < 1 / 4 ∧
        ∀ ρ : NontrivialZetaZero, |ρ.1.im| ≤ H →
          3 / (16 * Real.log H) < ρ.1.re ∧ ρ.1.re < 1 - 3 / (16 * Real.log H) := by
  have hregion : ∃ T : ℝ, 1 ≤ T ∧ ∀ ρ : NontrivialZetaZero, T ≤ |ρ.1.im| →
      (3 / 16 : ℝ) / Real.log |ρ.1.im| < ρ.1.re ∧
        ρ.1.re < 1 - (3 / 16 : ℝ) / Real.log |ρ.1.im| := by
    simpa only [div_div] using exists_eventual_strip
  simpa only [div_div] using exists_eventual_common_log_margin (by norm_num : (0 : ℝ) < 3 / 16) hregion

/-- Literal zeta nonvanishing on the larger closed right edge. The
height threshold is proved finite but is not numerically evaluated. -/
theorem exists_eventual_nonvanishing :
    ∃ T : ℝ, 1 ≤ T ∧ ∀ s : ℂ, T ≤ |s.im| →
      1 - 3 / (16 * Real.log |s.im|) ≤ s.re → riemannZeta s ≠ 0 := by
  ...
  have hlog : 100000 ≤ Real.log |s.im| := by
    ...
    simpa only [Real.log_exp] using h
  have hsmall : 3 / (16 * Real.log |s.im|) < (1 / 2 : ℝ) := by
    apply (div_lt_iff₀ (by positivity)).mpr
    linarith
  ...
  have h := hzero ρ ht0
  change 3 / (16 * Real.log |s.im|) < 1 - s.re at h
  linarith

end
end RiemannGaussian.GaussianFermiCurvatureZeroFree
