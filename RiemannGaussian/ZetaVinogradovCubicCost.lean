...
The cubic-profile bounds discharge both actual analytic discs. Their
complete signed cost, including the unchanged leading Euler-center
logarithm, has exact limit 11200*C/(3*pi). No arithmetic or analytic
payment is assumed in the resulting actual-zero margin.
-/

namespace RiemannGaussian.ZetaVinogradovCubicCost
...
open ZetaVinogradovCubicDisc (profile)
...
by the actual cubic zeta bound above the expanded-window threshold. -/
theorem margin_of_budget (n : ℕ) (hn : 48 ≤ n) (ρ : NontrivialZetaZero)
  ...
  exact ZetaAngularDiscBudget.margin_of_budget ρ (delta_pos n)
    ...
    (fun _ hz ↦ ZetaVinogradovCubicDisc.norm_translated_le n hn ht hx hx' hz)
    (ZetaVinogradovCubicDisc.analyticOnNhd_translated n hn ht₂ hx hx')
    (fun _ hz ↦ ZetaVinogradovCubicDisc.norm_translated_le n hn ht₂ hx hx' hz) hpay
...
/-- The actual profile at the original height costs six second logarithms. -/
theorem profile_normalized :
    Tendsto (fun t : ℝ ↦ profile (order t) t / level t) atTop (𝓝 6) := by
  have h := ((level_atTop.const_div_atTop (Real.log 8192)).add growth_normalized).add
    (tendsto_const_nhds (x := (1 : ℝ)))
  norm_num only [zero_add, show (5 : ℝ) + 1 = 6 by norm_num] at h
  apply h.congr'
...
theorem profile_double_normalized :
    Tendsto (fun t : ℝ ↦ profile (order t) (2 * t) / level t) atTop (𝓝 6) := by
...
theorem allowance_normalized {C : ℝ} (hC : 0 < C) :
  ...
  norm_num only [show (6 : ℝ) + 2 / 3 = 20 / 3 by norm_num] at h₁ h₂
  constructor
  · convert h₁ using 1
    funext t
    unfold allowance
...
theorem width_mul_budget_tendsto {C : ℝ} (hC : 0 < C) :
  ...
  rw [show (4 * C) * (8 * (20 / 3 : ℝ) + 2 * (20 / 3)) / Real.pi =
    800 * C / (3 * Real.pi) by ring] at h
  apply h.congr'
  filter_upwards [level_atTop.eventually (eventually_gt_atTop (0 : ℝ))] with t ht
  unfold budget ZetaAngularDiscBudget.budget allowance
  field_simp

/-- The entire signed cost has exact limit 11200*C/(3*pi); every
moving-radius correction vanishes on the same natural degree schedule. -/
theorem cost_tendsto {C : ℝ} (hC : 0 < C) :
    Tendsto (cost C) atTop (𝓝 (11200 * C / (3 * Real.pi))) := by
  ...
  rw [show (14 : ℝ) * (800 * C / (3 * Real.pi)) = 11200 * C / (3 * Real.pi) by ring] at h
  convert h using 1
  funext t
  unfold cost
  ring
...
end RiemannGaussian.ZetaVinogradovCubicCost
