...
-/
/-
Adapted from Anthropic's multiplicity-aware rank--trace development in
Zeta23. Import paths and the enclosing namespace are adjusted for
RiemannGaussian; the mathematical proof is retained.
-/
...
namespace RiemannGaussian.HermitianRankTrace.Multiplicity
...
lemma affine_le_gc {c t x : ℝ} (_ht0 : 0 ≤ t) (htc : t ≤ c) (_hx : 0 ≤ x) :
...
lemma affine_eq_gc {c x : ℝ} (_hc : 0 ≤ c) (_hx : 0 ≤ x) :
...
lemma sq_sub_ge_gc {c p n : ℝ} (_hp : 0 ≤ p) (hn : 0 ≤ n) (_hc : 0 ≤ c) :
...
section Schur
open RiemannGaussian.HermitianRankTrace
...
section Transfer
open RiemannGaussian.HermitianRankTrace Polynomial
...
lemma kc_mono {c p p' : ℝ} (_hc : 0 ≤ c) (hpp : p ≤ p') : kc c p ≤ kc c p' := by
  unfold kc
...
theorem sum_kc_two_nat (m : ι → ℕ) (hm : ∀ j, 1 ≤ m j) :
...
theorem sum_kc_three_nat (m : ι → ℕ) (hm : ∀ j, 1 ≤ m j) :
...
end RiemannGaussian.HermitianRankTrace.Multiplicity
