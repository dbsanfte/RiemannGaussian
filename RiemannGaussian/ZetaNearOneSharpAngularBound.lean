...
/-!
# The actual signed angular bound with exact Gaussian reconstruction

The left boundary retains the full height-dependent Gaussian correction.
The right boundary uses the existing exact Euler reciprocal allowance.
The full complex identity, zero multiplicities, and radial correction
are transported to the entire available disc radius.
-/

namespace RiemannGaussian.ZetaNearOneSharpAngularBound
...
def allowance (k : ℕ) (x t : ℝ) : ℝ :=
  profile k t + ZetaGaussianSharpDisc.correction k t + Real.log (1 + 1 / x)
...
/-- Every eligible channel saves at least twelve logarithmic units
relative to the previous actual angular allowance. -/
theorem allowance_add_twelve_le_previous (k : ℕ) (hk : 2 ≤ k) (x : ℝ) {t : ℝ}
  ...
  have hc := (ZetaGaussianSharpDisc.correction_bounds k hk ht).2
  unfold allowance ZetaNearOneJensen.allowance
  linarith
...
theorem boundary_moment_le (k : ℕ) (hk : 2 ≤ k) {t x r : ℝ}
  ...
  · intro z hz _
    have hn := ZetaGaussianSharpDisc.norm_translated_le k hk ht hx hx'
...
end RiemannGaussian.ZetaNearOneSharpAngularBound
