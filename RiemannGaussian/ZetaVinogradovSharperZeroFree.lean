...
-/
import RiemannGaussian.ZetaVinogradovSharperCost
import RiemannGaussian.ZetaVinogradovAngularZeroFree

/-!
# A wider VK zero-free family from the shorter actual moments

The stronger Dirichlet saving reaches actual zeta zeros through the
complete signed angular budget. Every 0<C<3*pi/38080 is admissible,
including C=1/4500, with a finite unevaluated starting height. The
coefficient ceiling is exactly 44/17 times the earlier signed ceiling.
...
namespace RiemannGaussian.ZetaVinogradovSharperZeroFree
noncomputable section
open Filter ZetaVinogradovSharperScale
...
def coefficientLimit : ℝ := 3 * Real.pi / 38080

/-- A concrete member of the proved coefficient interval. -/
theorem rational_coefficient_lt : (1 / 4500 : ℝ) < coefficientLimit := by
...
theorem eventually_parameters {C : ℝ} (hC : 0 < C) (hClim : C < coefficientLimit) :
    ∀ᶠ t : ℝ in atTop, 48 ≤ order t ∧
      ...
      ZetaVinogradovSharperCost.cost C t < 1 := by
  have hlim : 38080 * C / (3 * Real.pi) < 1 := by
    apply (div_lt_one (by positivity : 0 < 3 * Real.pi)).mpr
    unfold coefficientLimit at hClim
    linarith
  filter_upwards [ZetaVinogradovSharperScale.eventually_parameters,
    level_atTop.eventually (eventually_gt_atTop (0 : ℝ)),
    (width_div_delta_tendsto C).eventually (gt_mem_nhds (by norm_num : (0 : ℝ) < 1 / 28)),
    (ZetaVinogradovSharperCost.cost_tendsto hC).eventually (gt_mem_nhds hlim)]
...
theorem cost_abs (C t : ℝ) :
    ZetaVinogradovSharperCost.cost C |t| = ZetaVinogradovSharperCost.cost C t := by
  simp [ZetaVinogradovSharperCost.cost, (abs_invariance C t).1,
    (abs_invariance C t).2.2, ZetaVinogradovSharperCost.budget,
    ZetaAngularDiscBudget.budget, ZetaVinogradovSharperDisc.profile,
...
theorem exists_eventual_right_margin {C : ℝ} (hC : 0 < C)
  ...
  exact ZetaVinogradovSharperCost.margin_of_budget (order ρ.1.im) hn ρ ht hw hs hcost
...
44/17, after the actual moment and analytic costs have been paid. -/
theorem coefficient_gain :
    coefficientLimit = (44 / 17 : ℝ) * ZetaVinogradovAngularZeroFree.coefficientLimit := by
  unfold coefficientLimit ZetaVinogradovAngularZeroFree.coefficientLimit
  ring

/-- The displayed rational member contains every earlier signed VK
coefficient; its larger threshold is still stated explicitly as unevaluated. -/
theorem previous_limit_lt_rational :
    ZetaVinogradovAngularZeroFree.coefficientLimit < (1 / 4500 : ℝ) := by
  unfold ZetaVinogradovAngularZeroFree.coefficientLimit
...
1/4500. Absorbing the older VK component loses none of its coverage. -/
theorem exists_eventual_union_compact {C A : ℝ} (hC : 1 / 4500 ≤ C)
    (hClim : C < coefficientLimit) (hA : 0 < A)
    (hAlim : A < ZetaLogLogZeroFree.coefficientLimit) :
    ∃ T : ℝ, 4 ≤ T ∧ ∀ ρ : NontrivialZetaZero, T ≤ |ρ.1.im| →
      max (width C ρ.1.im)
...
end RiemannGaussian.ZetaVinogradovSharperZeroFree
