...
import RiemannGaussian.MontgomeryVaughan.Spacing
import RiemannGaussian.MontgomeryVaughan
...
Montgomery–Vaughan weighted Hilbert inequality, RiemannGaussian/MontgomeryVaughan.lean), and polarization
...
/-- The conclusion of step 3 (`RiemannGaussian.MontgomeryVaughan.eigen_bound`, Eigen.lean) as a `Prop` with a general
...
lemma star_dotProduct_mulVec_eq {A : Matrix ι ι ℂ} (hA : A.IsHermitian) (x : ι → ℂ) :
  ...
  conv_lhs => rw [← HermitianRankTrace.specMap_id hA]
  unfold HermitianRankTrace.specMap
...
Montgomery–Vaughan weighted Hilbert inequality (diagonal / literature form, RiemannGaussian/MontgomeryVaughan.lean). -/
