...
-/
import RiemannGaussian.VinogradovShortResonance
import RiemannGaussian.VinogradovDampedSaving

/-!
# The original Dirichlet blocks with the stronger moment saving

The complete factorial moment cost and all Gaussian factors are already
paid. The literal Taylor remainder, shift average and every decreasing
weight retain the saving 1/(1600*k^2) for k>=12, improved to 1/(512*k^2)
for k>=48. The full block still has coefficient five; every partial
block keeps its complete boundary term 2*M^2.
-/

namespace RiemannGaussian.VinogradovShortDirichlet
noncomputable section
open scoped BigOperators
open VinogradovShortResonance VinogradovRectanglePowerSaving
...
theorem product_bound (k : ℕ) (hk : 12 ≤ k)
    ...
    ‖∑ b : Fin M, ∑ c ∈ B, dirichletTerm t z ((b.val + 1) * c)‖ ≤
      ((3 : ℝ)) * (M : ℝ) ^ (2 - saving k) := by
  let alpha := 2 - saving k
  have hsaving := saving_bounds (by omega : 1 ≤ k)
...
theorem feature_block_bound (k : ℕ) (hk : 12 ≤ k)
    (M : ℕ) (hM : 1 ≤ M) (hrM : (4 * k + 1) * k ≤ M) (s : ℂ) (hσ : 0 ≤ s.re)
...
end RiemannGaussian.VinogradovShortDirichlet
