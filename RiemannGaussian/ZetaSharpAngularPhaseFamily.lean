...
/-!
# Exact Gaussian savings in the complete phase-family zero bound

The actual sharpened logarithmic derivative estimates are summed over
all eligible frequencies. Infinite support remains permitted. The
complete budget saves at least `24 * mass / (pi * delta)` before any
asymptotic limit, and retains the selected source and radial correction.
-/

namespace RiemannGaussian.ZetaSharpAngularPhaseFamily
...
/-- The complete sum of the actual improved angular allowances. -/
def totalAllowance (k : ℕ) (x t : ℝ) (a ω : ℕ → ℝ) : ℝ :=
  ∑' n, tail a n * ZetaNearOneSharpAngularBound.allowance k x (ω n * t)

/-- The improved full allowance is genuinely summable under the same
logarithmic frequency moment as the preceding phase theorem. -/
theorem summable_allowance {a ω : ℕ → ℝ} (ha : ∀ n, 0 ≤ a n) (hs : Summable a)
    (hω : ∀ n, n ≠ 0 → 1 ≤ ω n)
    (hlog : Summable (fun n ↦ tail a n * Real.log (ω n)))
    (k : ℕ) (hk : 2 ≤ k) {x t : ℝ} (hx : 0 < x) (ht : 2 ≤ |t|)
    (hscale : 1 ≤ scale t) :
    Summable (fun n ↦ tail a n * ZetaNearOneSharpAngularBound.allowance k x (ω n * t)) := by
  apply (ZetaAngularPhaseAllowance.summable_allowance ha hs hω hlog k hx hscale).of_norm_bounded
  intro n
  by_cases hn : n = 0
  · simp [tail, hn]
  · have hw := hω n hn
    have hfreq : 2 ≤ |ω n * t| := by
      rw [abs_mul, abs_of_nonneg (by linarith : 0 ≤ ω n)]
      nlinarith
    have hnew := ZetaNearOneSharpAngularBound.allowance_nonneg k hk hfreq hx
    have hb := ZetaNearOneSharpAngularBound.allowance_add_twelve_le_previous k hk x hfreq
    rw [Real.norm_eq_abs, abs_of_nonneg (mul_nonneg (tail_nonneg ha n) hnew)]
    exact mul_le_mul_of_nonneg_left (by linarith) (tail_nonneg ha n)

/-- All nonconstant channels save at least twelve logarithmic units,
including when the family has infinitely many frequencies. -/
theorem totalAllowance_add_gain_le_previous {a ω : ℕ → ℝ}
    (ha : ∀ n, 0 ≤ a n) (hs : Summable a) (hω : ∀ n, n ≠ 0 → 1 ≤ ω n)
    (hlog : Summable (fun n ↦ tail a n * Real.log (ω n)))
    (k : ℕ) (hk : 2 ≤ k) {x t : ℝ} (hx : 0 < x) (ht : 2 ≤ |t|)
    (hscale : 1 ≤ scale t) :
    totalAllowance k x t a ω + 12 * mass a ≤
      ZetaAngularPhaseAllowance.totalAllowance k x t a ω := by
  have hn := summable_allowance ha hs hω hlog k hk hx ht hscale
  have hm := (tail_summable ha hs).mul_left 12
  have ho := ZetaAngularPhaseAllowance.summable_allowance ha hs hω hlog k hx hscale
  have hb := (hn.add hm).tsum_le_tsum (fun n ↦ show
      tail a n * ZetaNearOneSharpAngularBound.allowance k x (ω n * t) + 12 * tail a n ≤
        tail a n * ZetaNearOneJensen.allowance k x (ω n * t) from by
    by_cases hz : n = 0
    · simp [tail, hz]
    · have hw := hω n hz
      have hf : 2 ≤ |ω n * t| := by
      ...
      have h := mul_le_mul_of_nonneg_left
        (ZetaNearOneSharpAngularBound.allowance_add_twelve_le_previous k hk x hf)
        (tail_nonneg ha n)
      nlinarith only [h]) ho
  simpa only [totalAllowance, ZetaAngularPhaseAllowance.totalAllowance, mass,
    hn.tsum_add hm, tsum_mul_left] using hb
...
theorem source_le_budget {a ω : ℕ → ℝ} (ha : ∀ n, 0 ≤ a n) (hs : Summable a)
  ...
  have hE := summable_allowance ha hs hω hlog k hk hx ht hscale
  ...
  have hpoint (n : ℕ) : a n * D n + (if n = 1 then a 1 * S else 0) ≤
      (if n = 0 then a 0 * (1 / x + 448 * localZetaLogHeight 0) else 0) +
        2 * (tail a n * ZetaNearOneSharpAngularBound.allowance k x (ω n * ρ.1.im)) / (Real.pi * delta k) := by
    by_cases hn0 : n = 0
    · subst n
    ...
    · by_cases hn1 : n = 1
      · subst n
        have hz := mul_le_mul_of_nonneg_left
          (ZetaNearOneSharpAngularBound.neg_logDeriv_re_le_sub_zero k hk ρ ht hx hx' hnear) (ha 1)
      ...
      · have hw := hω n hn0
        have hfreq : 2 ≤ |ω n * ρ.1.im| := by
          ...
          nlinarith
        have hz := mul_le_mul_of_nonneg_left
          (ZetaNearOneSharpAngularBound.neg_logDeriv_re_le k hk hfreq hx hx') (ha n)
...
/-- The saved boundary mass reaches the actual complete source budget
without removing a selected zero term or changing the prime kernel. -/
theorem budget_add_gain_le_previous {a ω : ℕ → ℝ}
    (ha : ∀ n, 0 ≤ a n) (hs : Summable a) (hω : ∀ n, n ≠ 0 → 1 ≤ ω n)
    (hlog : Summable (fun n ↦ tail a n * Real.log (ω n)))
    (k : ℕ) (hk : 2 ≤ k) {x t : ℝ} (hx : 0 < x) (ht : 2 ≤ |t|)
    (hscale : 1 ≤ scale t) :
    budget k x t a ω + 24 * mass a / (Real.pi * delta k) ≤
      ZetaAngularPhaseFamily.budget k x t a ω := by
  have h := totalAllowance_add_gain_le_previous ha hs hω hlog k hk hx ht hscale
  have hb := div_le_div_of_nonneg_right (mul_le_mul_of_nonneg_left h (by norm_num : (0 : ℝ) ≤ 2))
    (mul_pos Real.pi_pos (delta_pos k)).le
  unfold budget ZetaAngularPhaseFamily.budget
  simp only [add_div, mul_add] at hb
  simp only [div_eq_mul_inv] at hb ⊢
  nlinarith only [hb]

/-- A strict surplus in this explicit finite-height budget excludes
an actual zero at that same height for every eligible phase family. -/
theorem margin_lt_one_sub_re {a ω : ℕ → ℝ} (ha : ∀ n, 0 ≤ a n) (hs : Summable a)
    (hp : ∀ t, 0 ≤ zetaPhaseKernel a ω t) (hω0 : ω 0 = 0) (hω1 : ω 1 = 1)
    (hω : ∀ n, n ≠ 0 → 1 ≤ ω n)
    (hlog : Summable (fun n ↦ tail a n * Real.log (ω n)))
    (k : ℕ) (hk : 2 ≤ k) (ρ : NontrivialZetaZero) {r u : ℝ}
    (ht : 2 ≤ |ρ.1.im|) (hscale : 1 ≤ scale ρ.1.im) (hr : 0 < r) (hu : 0 < u)
    (hsmall : 4 * (r + 1) * u < delta k)
    (hcost : cost k r u ρ.1.im a ω < PhasePoleMargin.margin (a 0) (a 1) r) :
    u < 1 - ρ.1.re := by
  by_contra h
  exact (not_le_of_gt hcost) (margin_le_cost_of_zero_near ha hs hp hω0 hω1 hω hlog
    k hk ρ ht hscale hr hu hsmall (le_of_not_gt h))

end
end RiemannGaussian.ZetaSharpAngularPhaseFamily
