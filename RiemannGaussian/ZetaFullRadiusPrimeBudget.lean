...
/-!
# The actual prime budget with the entire local radius

The full-radius signed estimate reduces both oscillatory allowances and
retains the selected multiplicity with its exact endpoint correction.
...
namespace RiemannGaussian.ZetaFullRadiusPrimeBudget
...
full available radius. -/
def budget (k : ℕ) (x t : ℝ) : ℝ :=
  1344 * localZetaLogHeight 0 +
    (8 * allowance k x t + 2 * allowance k x (2 * t)) / delta k

/-- The source-normalized full-radius budget retains the exact
...
theorem source_le_budget (k : ℕ) (hk : 2 ≤ k) (ρ : NontrivialZetaZero)
  ...
  have hzero := ZetaNearOneFullRadius.neg_logDeriv_re_le_sub_zero k hk ρ ht hx hx' hnear
  have ht2 : 2 ≤ |2 * ρ.1.im| := by rw [abs_mul]; norm_num; linarith
  have hdouble := ZetaNearOneFullRadius.neg_logDeriv_re_le k hk ht2 hx hx'
...
end RiemannGaussian.ZetaFullRadiusPrimeBudget
