...
-/
import RiemannGaussian.ZetaVinogradovSharperBound
import RiemannGaussian.ZetaVinogradovFullDisc

/-!
# The full analytic disc supplied by the stronger actual growth bound

The shorter moment descent enlarges the available radius by sixteen at
the same index. Both sides of the one-line and the complete unit height
buffer retain the original numerical profile coefficient. Analyticity,
not zero-freeness, is proved on the entire disc.
-/

namespace RiemannGaussian.ZetaVinogradovSharperDisc
noncomputable section
open Complex Metric Set VinogradovSharperBudget VinogradovScaleSelection
...
theorem strip_bound_abs (n : ℕ) (hn : 48 ≤ n) {s : ℂ}
  ...
  by_cases hσ : s.re ≤ 1
  · exact ZetaVinogradovSharperBound.bound_strip_abs n hn hlo hσ ht
...
end RiemannGaussian.ZetaVinogradovSharperDisc
