...
-/
import RiemannGaussian.ZetaVinogradovCubicCost
import RiemannGaussian.ZetaVinogradovSharperZeroFree

/-!
# A wider VK zero-free family from the retained cubic block profile

The retained cubic block saving reaches actual zeta zeros through the
complete signed angular budget. Every 0<C<3*pi/11200 is admissible,
including C=1/1250, with a finite unevaluated starting height. The
coefficient ceiling is exactly 17/5 times the previous shorter-moment ceiling.
...
namespace RiemannGaussian.ZetaVinogradovCubicZeroFree
...
def coefficientLimit : ℝ := 3 * Real.pi / 11200

/-- A concrete member of the proved coefficient interval. -/
theorem rational_coefficient_lt : (1 / 1250 : ℝ) < coefficientLimit := by
...
theorem eventually_parameters {C : ℝ} (hC : 0 < C) (hClim : C < coefficientLimit) :
    ∀ᶠ t : ℝ in atTop, 48 ≤ order t ∧
      ...
      ZetaVinogradovCubicCost.cost C t < 1 := by
  have hlim : 11200 * C / (3 * Real.pi) < 1 := by
  ...
  filter_upwards [ZetaVinogradovCubicScale.eventually_parameters,
    ...
    (ZetaVinogradovCubicCost.cost_tendsto hC).eventually (gt_mem_nhds hlim)]
...
theorem cost_abs (C t : ℝ) :
    ZetaVinogradovCubicCost.cost C |t| = ZetaVinogradovCubicCost.cost C t := by
  simp [ZetaVinogradovCubicCost.cost, (abs_invariance C t).1,
    (abs_invariance C t).2.2, ZetaVinogradovCubicCost.budget,
    ZetaAngularDiscBudget.budget, ZetaVinogradovCubicDisc.profile,
...
theorem exists_eventual_right_margin {C : ℝ} (hC : 0 < C)
  ...
  exact ZetaVinogradovCubicCost.margin_of_budget (order ρ.1.im) hn ρ ht hw hs hcost
...
17/5, after the actual moment and analytic costs have been paid. -/
theorem coefficient_gain :
    coefficientLimit = (17 / 5 : ℝ) * ZetaVinogradovSharperZeroFree.coefficientLimit := by
  unfold coefficientLimit ZetaVinogradovSharperZeroFree.coefficientLimit
  ring

/-- The displayed rational member contains every previous shorter-moment VK
coefficient; its larger threshold is still stated explicitly as unevaluated. -/
theorem previous_limit_lt_rational :
    ZetaVinogradovSharperZeroFree.coefficientLimit < (1 / 1250 : ℝ) := by
  unfold ZetaVinogradovSharperZeroFree.coefficientLimit
...
1/1250. Absorbing the older VK component loses none of its coverage. -/
theorem exists_eventual_union_compact {C A : ℝ} (hC : 1 / 1250 ≤ C)
    (hClim : C < coefficientLimit) (hA : 0 < A)
    (hAlim : A < ZetaLogLogZeroFree.coefficientLimit) :
    ∃ T : ℝ, 4 ≤ T ∧ ∀ ρ : NontrivialZetaZero, T ≤ |ρ.1.im| →
      max (width C ρ.1.im)
...
end RiemannGaussian.ZetaVinogradovCubicZeroFree
