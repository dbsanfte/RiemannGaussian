...
/-!
# Original Dirichlet sums with fully specified degree costs

The polynomial-depth mean-value descent now enters the literal imaginary
powers, original shift average, all partial blocks, and damped zeta terms.
Every coefficient is a defined function of degree, and the sole starting
condition is `k*(k+1) <= M`. The coefficient can be very large; the theorem
does not yet give the uniform VK growth estimate or a new zero-free region.
-/

namespace RiemannGaussian.VinogradovQuantitativeDirichlet
noncomputable section
open scoped BigOperators
open VinogradovRelativeResonance VinogradovRectanglePowerSaving
...
theorem product_bound (k : ℕ) (hk : 12 ≤ k)
    ...
    ‖∑ b : Fin M, ∑ c ∈ B, dirichletTerm t z ((b.val + 1) * c)‖ ≤
      (productMultiplier k + 1) * (M : ℝ) ^ (2 - 1 / (256 * (k : ℝ) ^ 2)) := by
  let alpha := 2 - 1 / (256 * (k : ℝ) ^ 2)
  have hden : 1 ≤ 256 * (k : ℝ) ^ 2 := by
    have hk1 : (1 : ℝ) ≤ k := by exact_mod_cast (show 1 ≤ k by omega)
    have hs := one_le_pow₀ (n := 2) hk1
    nlinarith only [hs]
  have halpha : 0 ≤ alpha := by
    have hdiv : 1 / (256 * (k : ℝ) ^ 2) ≤ 1 := (div_le_one (zero_lt_one.trans_le hden)).mpr hden
    dsimp only [alpha]
    linarith only [hdiv]
  let C := productMultiplier k
  have hM1 : 1 ≤ M := hM
...
theorem partial_block_bound (k : ℕ) (hk : 12 ≤ k)
    ...
    ‖block (dirichletTerm t z) L‖ ≤
      (productMultiplier k + 1) * L * (M : ℝ) ^ (-(1 / (256 * (k : ℝ) ^ 2))) + 2 * (M : ℝ) ^ 2 := by
  let C := productMultiplier k + 1
  have hM1 : 1 ≤ M := hM
  have hMr : (0 : ℝ) < M := by exact_mod_cast hM1
  have hz : 0 < z := (pow_pos hMr 4).trans_le hzlo
  let delta := 1 / (256 * (k : ℝ) ^ 2)
...
/-- The original full block has a genuine power saving with a defined
coefficient and an evaluated starting scale at every degree. -/
theorem block_bound (k : ℕ) (hk : 12 ≤ k)
    (M : ℕ) (hM : 1 ≤ M) (hrM : (k + 1) * k ≤ M)
    (t z : ℝ) (htlo : (M : ℝ) ^ (2 * k - 2) ≤ t) (hthi : t ≤ (M : ℝ) ^ (2 * k))
    (hzlo : (M : ℝ) ^ 4 ≤ z) (hzhi : z ≤ 2 * (M : ℝ) ^ 4) :
    ‖block (dirichletTerm t z) (M ^ 4)‖ ≤
      (productMultiplier k + 3) * (M : ℝ) ^ (4 - 1 / (256 * (k : ℝ) ^ 2)) := by
  let C := productMultiplier k + 1
  ...
  calc
    ...
    _ ≤ (productMultiplier k + 3) * (M : ℝ) ^ (4 - delta) := by
      dsimp only [C]
      nlinarith only [hpow]
...
theorem weighted_block_bound (k : ℕ) (hk : 12 ≤ k)
    ...
    ‖∑ n ∈ Finset.range (N + 1), (w n : ℂ) * dirichletTerm t z n‖ ≤
      ((productMultiplier k + 1) * (M : ℝ) ^ (-(1 / (256 * (k : ℝ) ^ 2)))) *
...
theorem feature_block_bound (k : ℕ) (hk : 12 ≤ k)
    (M : ℕ) (hM : 1 ≤ M) (hrM : (k + 1) * k ≤ M) (s : ℂ) (hσ : 0 ≤ s.re)
...
end RiemannGaussian.VinogradovQuantitativeDirichlet
