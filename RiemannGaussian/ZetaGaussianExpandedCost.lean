...
/-!
# Complete Gaussian costs for the enlarged dilation range
...
allowance, for every `q ≥ 9/100`. The larger response multiplier is paid
with height coefficient `1/35`; the constant-channel overhead is `564`.
No upper height constraint is used and no new arithmetic premise is added.
-/

namespace RiemannGaussian.ZetaGaussianExpandedCost
noncomputable section
open ZetaGaussianScaledBandBudget (width shift gaussianScale)
open ZetaGaussianExpandedScale ZetaGaussianStripExplicit ZetaGaussianStripPhaseFamily
...
theorem inverse_halfWidth_le {q : ℝ} (hq : 9 / 100 ≤ q) :
...
theorem constant_terms_le {q : ℝ} (hq : 9 / 100 ≤ q) :
  ...
  linarith [constant_upper (q := q) (by linarith), completion_le hq]

/-- The entire left allowance retains its actual logarithmic height and log-log term. -/
theorem left_allowance_le {q t : ℝ} (hq : 9 / 100 ≤ q) (ht : 1000000 ≤ |t|) :
...
private theorem extra_terms_le {q t : ℝ} (hq : 9 / 100 ≤ q) (ht : 1000000 ≤ |t|) :
...
private theorem response_terms_le {q t : ℝ} (hq : 9 / 100 ≤ q) (ht : 1000000 ≤ |t|) :
...
theorem budget_le {a ω : ℕ → ℝ} (ha0 : 0 ≤ a 0) (ha0u : a 0 ≤ 37 / 200)
    ...
    {q t : ℝ} (hq : 9 / 100 ≤ q) (ht : 1000000 ≤ |t|) :
    budget 9 (gaussianScale q) (shift q) t a ω ≤
      36922 * q + scale t / 35 + 57 * Real.log (scale t) + 840 := by
  ...
  have hconstant := (mul_le_mul_of_nonneg_left (constant_terms_le hq) ha0).trans
    (mul_le_mul_of_nonneg_right ha0u (show 0 ≤ 199575 * q + 564 by positivity))
...
end RiemannGaussian.ZetaGaussianExpandedCost
