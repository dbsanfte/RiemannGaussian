...
/-!
# A kernel-checked unwrapped theta bound at height fifteen
...
namespace RiemannGaussian.GammaThetaFifteen
...
private def argument (i : ℕ) : ℚ :=
  if i = 0 then 30 / 31 else 30 / (4 * (i - 1 : ℕ) + 1)

private def lower (i : ℕ) : ℚ :=
  ([7690, 15374, 14056, 12793, 11618, 10552, 9600, 8760, 8023] : List ℚ)[i]?.getD 0 / 10000
...
private def expression : Expr :=
  .add (.add (.add (.mul (.const (31 / 4)) (.var 0))
    (.mul (.const (15 / 4)) (.log (.const (1861 / 16))))) (.const (-15 / 2)))
    (.add (.neg (angleSum 8)) (.neg (.mul (.const (15 / 2)) (.log (.var 9)))))

private theorem expression_eq : Expr.eval realEnv expression = thetaApproximation 8 15 := by
...
private def checked : Bool :=
  ...
  | .error _ => false
  | .ok B => decide ((-7 / 5 : ℚ) ≤ B.lo.toRat ∧ B.hi.toRat ≤ -13 / 10)
...
theorem approximation_bounds : -(7 / 5 : ℝ) ≤ thetaApproximation 8 15 ∧
    thetaApproximation 8 15 ≤ -(13 / 10 : ℝ) := by
...
/-- The actual unwrapped theta phase at height fifteen lies strictly
between minus three halves and zero. Its analytic error is paid explicitly. -/
theorem theta_bounds : -(3 / 2 : ℝ) < theta 15 ∧ theta 15 < 0 := by
  have hh := abs_phase_sub_shiftedApproximation_le (by norm_num : (0 : ℝ) < 1 / 4)
    8 (by norm_num) (15 / 2)
  have he : theta 15 - thetaApproximation 8 15 =
      phase (1 / 4) (15 / 2) - shiftedApproximation (1 / 4) 8 (15 / 2) := by
...
end RiemannGaussian.GammaThetaFifteen
