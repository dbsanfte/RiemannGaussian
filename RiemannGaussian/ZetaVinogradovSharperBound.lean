...
/-!
# Stronger actual Vinogradov growth on a wider near-one strip

The shorter-moment savings cover every original dyadic block. The same
Euler reconstruction now bounds zeta on a strip sixteen times wider at
the same natural index and unchanged starting height, with coefficient
8192. Both height signs and the closed line Re(s)=1 are retained.
-/

namespace RiemannGaussian.ZetaVinogradovSharperBound
noncomputable section
open DirichletDyadicBlocks ZetaDyadicTruncation
open VinogradovSharperBudget VinogradovScaleSelection
...
theorem bound_band_abs (n : ℕ) (hn : 48 ≤ n) {s : ℂ}
    ...
    (ht : (heightThreshold n : ℝ) ≤ |s.im|) :
    ‖riemannZeta s‖ ≤ 8192 * |s.im| ^ (3072 * (1 - s.re) ^ (3 / 2 : ℝ)) *
      Real.log |s.im| := by
  ...
  have he : growth n ≤ 3072 * (1 - s.re) ^ (3 / 2 : ℝ) := by
...
end RiemannGaussian.ZetaVinogradovSharperBound
