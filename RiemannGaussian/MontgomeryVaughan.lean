...
-/
/-
RiemannGaussian/MontgomeryVaughan.lean — the Montgomery–Vaughan weighted Hilbert inequality: literature (diagonal, y = x)
form ⇒ the bilinear (x, z) form H-MV of RiemannGaussian/MontgomeryVaughan/Hypotheses.lean.

Purpose (trust reduction): RiemannGaussian.MVHilbert C — what [prop:PP]/[prop:cross] consume —
...
As in RiemannGaussian.MVHilbert we allow any admissible δ (δ_r > 0, δ_r ≤ |λ_r − λ_s| for s ≠ r); the right
...
immaterial (∃ C).  Result: `RiemannGaussian.MVHilbert_of_diag` sends
`MVDiag C` to `MVHilbert (2 * C)`, and `RiemannGaussian.exists_MVHilbert_of_diag`
provides the corresponding existential form.
-/
import RiemannGaussian.MontgomeryVaughan.Hypotheses
...
Same binder shape as RiemannGaussian.MVHilbert with z := x. -/
def MVDiag (C : ℝ) : Prop :=
...
/-- The off-diagonal sesquilinear Hilbert form associated with `freq`. -/
...
theorem MVHilbert_of_diag {C : ℝ} (hC : 0 ≤ C) (h : MVDiag C) : MVHilbert (2 * C) := by
  ...
  rw [MontgomeryVaughan.sum_eq_B]
  have hdiag : ∀ y : ι → ℂ, ‖MontgomeryVaughan.B freq y y‖ ≤ C * MontgomeryVaughan.N2 δ y := by
    intro y
    have := h ι freq δ y hinj hδ hsep
    rw [MontgomeryVaughan.sum_eq_B] at this
    exact this
  exact MontgomeryVaughan.norm_B_le_sqrt hC hδ hdiag x z

/-- Existential form: `(∃ C > 0, MVDiag C) → ∃ C > 0, MVHilbert C`. -/
