...
/-!
# Actual zeta growth with the retained cubic block profile

Every original dyadic block is paid at exponent 40*Delta_n^(3/2),
including the expanded small-scale window. Direct Euler reconstruction
retains coefficient 8192 and both signs of height. The starting height
is the explicit T_(8n); no moment or growth hypothesis remains.
-/

namespace RiemannGaussian.ZetaVinogradovCubicBound
noncomputable section
open DirichletDyadicBlocks ZetaDyadicTruncation
open VinogradovCubicBudget VinogradovScaleSelection
open VinogradovSharperBudget (delta line delta_pos line_bounds delta_le_four_succ)
...
theorem bound_strip_abs (n : ℕ) (hn : 48 ≤ n) {s : ℂ}
  ...
  · have hsneg : s.im < 0 := lt_of_not_ge hs
    ...
    have ht' : (heightThreshold (8 * n) : ℝ) ≤ (conj s).im := by simpa [abs_of_neg hsneg] using ht
...
theorem bound_abs (n : ℕ) (hn : 48 ≤ n) {s : ℂ} (hline : s.re = line n)
    (ht : (heightThreshold (8 * n) : ℝ) ≤ |s.im|) :
...
theorem bound_band_abs (n : ℕ) (hn : 48 ≤ n) {s : ℂ}
    (hlo : line n ≤ s.re) (hhi : s.re ≤ line (n + 1))
    (ht : (heightThreshold (8 * n) : ℝ) ≤ |s.im|) :
    ‖riemannZeta s‖ ≤ 8192 * |s.im| ^ (320 * (1 - s.re) ^ (3 / 2 : ℝ)) *
      Real.log |s.im| := by
  ...
  have he : growth n ≤ 320 * (1 - s.re) ^ (3 / 2 : ℝ) := by
    have h := growth_eq_displacement n
    nlinarith
  have hbT : (rootBase (8 * n) : ℝ) ≤ heightThreshold (8 * n) := by
    exact_mod_cast rootBase_le_heightThreshold (show 1 ≤ 8 * n by omega)
  have hb : (16 : ℝ) ≤ rootBase (8 * n) := by exact_mod_cast rootBase_ge_sixteen (8 * n)
...
end RiemannGaussian.ZetaVinogradovCubicBound
