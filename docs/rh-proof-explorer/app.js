(function () {
  ...
  const certificateLinks = () => data.certificateVerification ?
    `<a class="small-link" href="${esc(data.certificateVerification.runUrl)}" target="_blank" rel="noopener">Successful exhaustive certificate run ↗</a><a class="small-link" href="hosted-audit.json" target="_blank" rel="noopener">Hosted full-target axiom audit ↗</a><a class="small-link" href="verification.json" target="_blank" rel="noopener">Exact certificate revision and input fingerprint ↗</a>` : "";
  ...
  function render(fit = false) {
    ...
    for (const i of visible) {
      ...
      html += `</text><text class="node-meta" x="${p.x + 13}" y="${p.y + p.height - 10}">${terminal ? "TERMINAL THEOREM" : n.verifiedDataBoundary ? "GENERATED DATA BOUNDARY" : n.generated ? "GENERATED HELPER" : esc(n.kind.toUpperCase())}${count && !terminal && !n.verifiedDataBoundary && !n.generated ? ` · ${count} project refs` : ""}</text>`;
  ...
  function showNode(i, center = false) {
    ...
    html += certificateLinks();
    if (release) html += `<a class="small-link" href="${esc(release.runUrl)}" target="_blank" rel="noopener">${data.certificateVerification ? "Presentation CI for" : "CI verification of"} ${release.revision.slice(0, 12)} ↗</a>`;
  ...
  $("metadata-guide").href = docURL(data.guideDocument || "docs/theorem-explorer.md");
  if (release) { $("revision-link").textContent = `${data.certificateVerification ? "Presentation commit" : "Verified commit"} ${release.revision.slice(0, 10)} ↗`; $("revision-link").href = release.runUrl; }
  loadHash();
